# Shallow variant of manual-mobile-mirror.ps1 — clones ONLY the target
# branch at --depth 1 to avoid the full-history clone stalling on the
# network (the public Agentrix-Claw repo has heavy APK/build history).
#
# Usage:
#   pwsh ./scripts/public-build/manual-mobile-mirror-shallow.ps1 -Branch build/companion-ball-revival-2026-05-30
#
# Requires the `public_claw` remote already configured with a working PAT.
...
if ($remoteHasBranch) {
    Write-Host "Remote branch exists — shallow-cloning only $Branch (depth 1)."
    Invoke-Git clone --depth 1 --branch $Branch --single-branch $publicUrl $Mirror
} else {
    Write-Host "Remote branch missing — shallow-cloning default branch then creating orphan."
    Invoke-Git clone --depth 1 $publicUrl $Mirror
}
...
try {
    ...
    & git -c "user.name=agentrix-bot" -c "user.email=bot@agentrix.local" commit -m "manual mirror (shallow): mobile clan sprites + PK train from CutaGames/Agentrix@$Branch" 2>&1 | ForEach-Object { Write-Host $_ }
