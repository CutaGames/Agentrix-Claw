...
- [ ] 4. 移动端收益中心（需求 2）
  ...
  - [ ] 4.2 端到端验证 + APK
    - 用任务 3.3 测试用户在 APK 真机查看：收益卡/收益中心数字与后端一致。
    - 镜像 Agentrix-Claw 触发 APK，记 build 号，等真机验证。
    - _Requirements: 2.5_
...
- [ ] 6. 拉新裂变接线（需求 4）
  - [ ] 6.1 user_referrals + ReferralFlywheelService
...
- [ ] 7. 收益兑付（需求 5）
  - [ ] 7.1 AXP 抵扣闭环 + 兑付页
    - `PetEarningsService.getRedeemOptions(userId)`（来自 `AXP_SPEND_SOURCES` 的抵扣场景）；`POST /pet-earnings/redeem` → `AxpService.spend(source, amount, refId=businessOrderId)`（用任务 1.1 幂等），1 AXP=$0.001 折算。
...
- [ ] 8. 萌宠半自主接活（需求 6）
  ...
  - [ ] 8.2 移动端"萌宠帮我赚"+ 端到端验证
    - 收益中心/萌宠屏推荐卡 + 一键接活确认。
    - 端到端：授权萌宠接一个真实可结算集市任务 → task_complete AXP 入账并在收益中心可见；限额耗尽被拦截。后端部署 + APK。
    - _Requirements: 6.5_

- [ ] 9. 飞轮指标（需求 7）
  - [ ] 9.1 PetEarningsMetricsService + admin 端点
