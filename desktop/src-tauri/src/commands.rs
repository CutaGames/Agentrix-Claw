...
/// Sprint G-2 / US-G2-6 — sanity-check the saved ball position against the
/// currently available monitors. If the position lies outside ALL monitors
/// (e.g. user unplugged the secondary display the ball was snapped to),
/// reset to the bottom-right of the primary monitor and persist.
///
/// Returns `true` when the position was reset, `false` when it was already valid.
pub fn validate_ball_position(app: AppHandle) -> Result<bool, String> {
    let monitors = app.available_monitors().map_err(|e| e.to_string())?;
    if monitors.is_empty() {
        return Ok(false);
    }

    let mut saved = BALL_POS.lock().map_err(|e| e.to_string())?;
    let Some(pos) = saved.clone() else {
        return Ok(false);
    };
    ...
    let in_range = monitors.iter().any(|m| {
        let mp = m.position();
        let ms = m.size();
        let x_min = mp.x as f64;
        let y_min = mp.y as f64;
        let x_max = x_min + ms.width as f64;
        let y_max = y_min + ms.height as f64;
        // Allow small overshoot for snap-to-edge half-hide (~50px tolerance).
        pos.x >= x_min - 100.0 && pos.x < x_max + 100.0 && pos.y >= y_min - 100.0 && pos.y < y_max + 100.0
    });
    ...
    // Out of range — reset to bottom-right of the primary monitor.
    let primary = monitors.first().expect("checked non-empty");
    let pp = primary.position();
    let ps = primary.size();
    let new_x = pp.x as f64 + ps.width as f64 - 100.0;
    let new_y = pp.y as f64 + ps.height as f64 - 100.0;
...
pub fn start_llm_sidecar(
    ...
    // Kill our previously-recorded sidecar if running.
    stop_llm_sidecar().ok();

    // ALSO kill any orphan llama-server.exe holding the target port. Without
    // this, when the user upgrades across app launches the old process from
    // the previous session keeps the port and the new sidecar's bind silently
    // fails — the new arguments (including --ctx-size) never take effect and
    // chat requests still hit the stale server. Reported by users seeing
    // `n_ctx: 4096` after we bumped the default to 16384 in v0.2.4.
    kill_orphan_llama_servers(port);
...
/// Kill any orphan llama-server processes that may still be holding the target
/// port from a previous app launch. Best-effort — if the platform commands are
/// missing we silently fall through and the spawn step will surface the bind
/// failure instead.
fn kill_orphan_llama_servers(port: u16) {
    #[cfg(target_os = "windows")]
    {
        // 1) Kill any llama-server.exe by image name (covers most cases).
        //    /F = force, /IM = image name. This is OK because llama-server is a
        //    process we own end-to-end; no other Agentrix component spawns it.
        let _ = Command::new("taskkill")
            .args(["/F", "/IM", "llama-server.exe"])
            .creation_flags(0x08000000)
            .output();

        // 2) As a belt-and-braces measure, find anything listening on the
        //    target port via netstat and kill that PID too.
    ...
    #[cfg(not(target_os = "windows"))]
    {
        // 1) Kill any llama-server processes by name.
        let _ = Command::new("pkill")
            .args(["-f", "llama-server"])
            .output();

        // 2) Kill anything bound to the port via lsof (macOS / Linux).
