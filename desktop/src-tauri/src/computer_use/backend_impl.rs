...
impl ComputerUseBackend for DefaultBackend {
    ...
    fn window_tree(&self) -> Result<Vec<WindowNode>, ComputerUseError> {
        ...
        for w in windows {
            ...
            // xcap 0.0.14 lacks an is_focused() probe — leave false; the JS
            // layer can compare window_tree() against active_window_id from
            // the existing desktop_bridge_get_active_window command.
            out.push(WindowNode {
                id: w.id().to_string(),
                title,
                app_name,
