...
export type DesktopActionKind =
  ...
  | "world-creation-task"
  // Computer Use (Phase B) — direct mouse / keyboard / screen control.
  // The desktop client invokes the corresponding Tauri command and serialises
  // the work through the same desktop-sync pipeline as filesystem ops.
...
export function classifyDesktopRisk(kind: DesktopActionKind, payload?: Record<string, string>): ApprovalRiskLevel {
  ...
  // World Creation (v6) — opening the Tier_C creator for a dispatched task is a
  // local UI action (no filesystem/network/process), so it needs no approval.
  if (kind === "world-creation-task") {
    return "L0";
  }

  // Computer Use read-only / pointer-only ops: no approval (still gated by
  // the user-facing Enable Computer Use toggle).
  if (kind === "computer-use-screenshot" || kind === "computer-use-window-tree"
      || kind === "computer-use-move"
      || kind === "computer-use-browser-list-tabs") {
    return "L0";
  }

  // Git read-only ops: no approval needed
  if (kind === "git-status" || kind === "git-diff" || kind === "git-log") {
    return "L0";
  }
  // Git write ops: per-call approval
  if (kind === "git-commit" || kind === "git-push" || kind === "git-pull" || kind === "git-checkout") {
    return "L2";
  }

  // Action-tier Computer Use: per-call approval. Browser navigate is treated
  // as L0 — opening a URL is low-risk (just like clicking a link in a chat
  // message). Per-call approval was creating UX friction without security
  // benefit, since the user already chose to invoke the agent.
  if (kind === "computer-use-browser-navigate") {
    return "L0";
  }
  if (kind === "computer-use-click" || kind === "computer-use-type"
      || kind === "computer-use-key" || kind === "computer-use-browser-eval"
