...
export async function respondDesktopApproval(
  ...
  try {
    ...
    if (err?.name === "AbortError") {
      // Timeout — return optimistic success so the UI doesn't hang.
      // The backend will still process the approval; next state poll will sync.
      return {
        ok: true,
        approval: {
        ...
        } as unknown as DesktopRemoteApproval,
      };
    }
    throw err;
  } finally {
