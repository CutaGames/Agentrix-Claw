...
import type { PetSkinWithVariants, DesktopAppMode } from "../../../shared/types/pet-skin-variant";
import { resolveVariantModelUrl, APP_MODE_TO_VARIANT } from "../../../shared/types/pet-skin-variant";
...
export async function refreshPetRenderers(): Promise<PetRendererId> {
  // Sprint DA #2: GPU-tier-aware renderer selection.
  // If hardware profile indicates low-end GPU, skip VRM entirely.
  const gpuCap = getGpuRendererCap();

  for (const candidate of RENDERER_PRIORITY) {
    // Skip VRM on low-end GPUs (Intel UHD 620 and below)
    if (candidate === "vrm" && gpuCap === "rive-only") continue;
    if (candidate === "live2d" && gpuCap === "rive-only") continue;
...
/**
 * Sprint DA #2: Determine GPU capability for renderer selection.
 * Per desktop-prd-v4 §4.1:
 *   - <= Intel UHD 620 → 'rive-only' (skip VRM)
 *   - Mid GPU → 'vrm-low' (VRM allowed)
 *   - High GPU → 'vrm-high' (VRM + PBR)
 *
 * Uses cached hardware profile from hardwareProfile.ts.
 */
type GpuRendererCap = "rive-only" | "vrm-low" | "vrm-high";

function getGpuRendererCap(): GpuRendererCap {
  try {
    const raw = localStorage.getItem("agentrix_hardware_profile_v1");
    if (!raw) return "vrm-low"; // default to mid if unknown
    const cached = JSON.parse(raw) as { profile: { gpu_vram_mb: number | null; recommended_tier: string } };
    const vram = cached.profile.gpu_vram_mb;
    const tier = cached.profile.recommended_tier;

    if (tier === "unsupported" || tier === "light") return "rive-only";
    if (tier === "enthusiast" || (vram && vram >= 6000)) return "vrm-high";
    return "vrm-low";
  } catch {
    return "vrm-low";
  }
 ...
 *
 * Sprint G-2 / US-G2-7: when no custom skin is configured, the SDK seeds
 * a default kitsune VRM URL from `VITE_DEFAULT_KITSUNE_VRM` (or the
 * production CDN path). PetVRM safely fallbacks to .glb if the file is
 * served as such, and to PetCanvas PNG if the file is missing entirely.
 */
let _booted = false;

const DEFAULT_KITSUNE_VRM_URLS: Record<DesktopAppMode, string> = {
  // Production CDN paths — v3 regenerated GLBs (2026-05-18).
  // PetVRM gracefully falls back to plain GLB rendering when the file lacks
  // VRM extensions. If the asset 404s, PetVRM `loadError` triggers and the
  // renderer chain falls back to PetCanvas PNG.
  "living-agent":   "https://agentrix.top/downloads/pets/kitsune-default-v3.glb",
  "pro-mode":       "https://agentrix.top/downloads/pets/kitsune-pro-v3.glb",
  "economy-panel":  "https://agentrix.top/downloads/pets/kitsune-economy-v3.glb",
};

function maybeSeedDefaultVrm(): void {
  try {
    const existing = localStorage.getItem("agentrix_pet_vrm_url");
    if (existing && existing.length > 0) return; // user already has a custom skin
    const url = DEFAULT_KITSUNE_VRM_URLS["living-agent"];
    localStorage.setItem("agentrix_pet_vrm_url", url);
    window.dispatchEvent(new CustomEvent("agentrix:pet-vrm-changed", { detail: { url } }));
    void refreshPetRenderers();
  } catch {
    // localStorage unavailable in private mode — fine, fallback renderer wins.
  }
}
...
export function bootPetSdk(): void {
  ...
  // Sprint G-2 / US-G2-7: seed the default kitsune VRM URL when missing.
  maybeSeedDefaultVrm();
}
...
// ── 6. Multi-Form Variant System ─────────────────────────────────────
//    Automatically switches the pet's 3D model based on the current
//    desktop app mode (Living Agent / Pro Mode / Economy Panel).

let _currentAppMode: DesktopAppMode = 'living-agent';
let _activeSkinVariants: PetSkinWithVariants | null = null;

/**
 * Set the current app mode. Called by App.tsx when mode changes.
 * Triggers variant switch if multi-form skin is active.
 */
export function setAppMode(mode: DesktopAppMode): void {
  if (_currentAppMode === mode) return;
  _currentAppMode = mode;
  applyVariantForCurrentMode();
}

export function getAppMode(): DesktopAppMode {
  return _currentAppMode;
}

/**
 * Register the active skin's variant data. Called when a multi-form
 * skin is equipped or when variant generation completes.
 */
export function setActiveSkinVariants(skin: PetSkinWithVariants | null): void {
  _activeSkinVariants = skin;
  if (skin) {
    applyVariantForCurrentMode();
  }
}

export function getActiveSkinVariants(): PetSkinWithVariants | null {
  return _activeSkinVariants;
}

/**
 * Resolve and apply the correct variant model for the current app mode.
 * Dispatches `agentrix:pet-vrm-changed` so PetVRM/FloatingBall picks it up.
 */
function applyVariantForCurrentMode(): void {
  if (!_activeSkinVariants || !_activeSkinVariants.hasMultiForm) return;

  const modelUrl = resolveVariantModelUrl(_activeSkinVariants, _currentAppMode);
  if (!modelUrl) return;

  // Dispatch the same event that petCreator.setActivePet uses
  window.dispatchEvent(
    new CustomEvent("agentrix:pet-vrm-changed", {
      detail: {
        url: modelUrl,
        mode: _currentAppMode,
        variantMode: APP_MODE_TO_VARIANT[_currentAppMode],
      },
    }),
  );
}
...
/**
 * Listen for app mode changes from the UI layer.
 * Components dispatch `agentrix:app-mode-changed` when switching modes.
 */
if (typeof window !== 'undefined') {
  window.addEventListener("agentrix:app-mode-changed", (e: Event) => {
    ...
    if (detail?.mode) {
      ...
      // Sprint G-2 / US-G2-7: when no custom skin variants are bound, swap
      // the default kitsune VRM URL so the renderer matches the form too.
      // Custom skins (PetSkinWithVariants) take precedence — see
      // applyVariantForCurrentMode() above.
      try {
        if (!_activeSkinVariants) {
          const url = DEFAULT_KITSUNE_VRM_URLS[detail.mode];
          if (url) {
            const current = localStorage.getItem("agentrix_pet_vrm_url");
            // Only overwrite when the current URL is one of OUR defaults, so
            // we don't trample a user-uploaded skin.
            const isDefault = !current || Object.values(DEFAULT_KITSUNE_VRM_URLS).includes(current);
            if (isDefault && current !== url) {
              localStorage.setItem("agentrix_pet_vrm_url", url);
              window.dispatchEvent(new CustomEvent("agentrix:pet-vrm-changed", { detail: { url } }));
            }
          }
