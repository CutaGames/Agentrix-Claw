...
import type { PetVariantMode } from "../../../shared/types/pet-skin-variant";
import { PET_VARIANT_MODE_LABELS, DEFAULT_VARIANT_PROMPT_MODIFIERS } from "../../../shared/types/pet-skin-variant";
...
export default function PetCreatorPanel({ onClose }: Props) {
  ...
  const handleGenerateVariants = async (task: PetTaskSummary) => {
    if (!task.prompt) {
      setError("无法生成变体：原始 prompt 为空");
      return;
    }
    setError(null);
    setSubmitting(true);

    const modes: PetVariantMode[] = ['living', 'pro', 'economy'];
    const results: string[] = [];

    for (const variantMode of modes) {
      try {
        const modifier = DEFAULT_VARIANT_PROMPT_MODIFIERS[variantMode];
        const variantPrompt = `${task.prompt}. Form variant: ${PET_VARIANT_MODE_LABELS[variantMode].en}. Style: ${modifier}`;
        const result = await submitPetTask({
          mode: "text",
          provider: (task.provider as PetProvider) || "meshy",
          style: (task.style as PetStyle) || "chibi",
          prompt: variantPrompt,
          enableAnimation: true,
        });
        const tid = result?.taskId || result?.task?.taskId;
        if (tid) results.push(tid);
      } catch (err: any) {
        console.warn(`[PetCreator] variant ${variantMode} submit failed:`, err?.message);
      }
    }

    if (results.length > 0) {
      setActiveTaskId(results[0]);
    }
    await refresh();
    setSubmitting(false);
  };
        ...
        <div style={{ flex: "0 0 360px", padding: 20, overflowY: "auto", borderRight: "1px solid rgba(255,255,255,0.06)" }}>
          ...
          {mode === "text" ? (
            ...
            <Section title="双图繁殖 (Breed)">
              ...
              />
              <p style={hintStyle}>
                选择两只已有皮肤的参考图 URL，AI 将融合它们的视觉特征生成新宠物。
              </p>
            </Section>
          )}
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  {(activeTask.vrmUrl || activeTask.outputUrl) && (
                    <div style={{ marginTop: 12, display: "flex", gap: 8, flexWrap: "wrap" }}>
                      ...
                      <button
                        onClick={() => handleGenerateVariants(activeTask)}
                        style={{
                          ...btnStyle,
                          background: "#8b5cf6",
                          color: "white",
              ...
              {(activeTask.vrmUrl || activeTask.outputUrl) && (
                ...
                >
                  ...
                  <div style={{ color: "rgba(255,255,255,0.75)" }}>
                    ...
                    <span style={{ opacity: 0.7 }}> — 立即替换桌面上的灵狐桌宠，跟着你一起工作</span>
                    <br />
                    ...
                    <div style={{ paddingLeft: 16, marginTop: 4, opacity: 0.7 }}>
                      ...
                      <br />· 通过 <code style={{ background: "var(--bg-overlay-medium)", padding: "1px 4px", borderRadius: 3 }}>https://gltf-viewer.donmccurdy.com</code> 在线预览
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(180px, 1fr))", gap: 12 }}>
            {tasks.map((t) => (
              <div
                ...
                style={{
                  cursor: "pointer",
                  background: t.taskId === activeTask?.taskId ? "rgba(108,92,231,0.18)" : "var(--bg-card)",
      ...
      {options.map((o) => (
        <button
          ...
          style={{
            ...btnStyle,
            flex: 1,
            background: value === o.value ? "#6C5CE7" : "var(--bg-overlay-light)",
