...
function TemplateCard({ template }: { template: GreetingTemplate }) {
  return (
    <div style={cardStyle}>
      <div style={{ fontSize: 13, fontWeight: 700, color: "var(--text-card)" }}>{template.label_zh || template.label_en}</div>
      <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 4 }}>{template.category}</div>
      {template.axp_cost > 0 && (
  ...
  return (
    <>
      <div style={{ padding: "8px 0 12px" }}>
        <div style={{ fontSize: 14, fontWeight: 800, color: "var(--text-card)" }}>
      ...
      {season.status === "submitting" && token && (
        <div style={submitBlock}>
          <div style={{ fontSize: 12, color: "var(--text-card)", marginBottom: 6, fontWeight: 700 }}>📸 参赛</div>
          <input
            type="text"
            value={imageUrl}
        ...
        <div style={gridStyle}>
          {entries.map((entry, idx) => (
            <div key={entry.id} style={entryCard}>
              ...
              <div style={{ padding: 6 }}>
                <div style={{ fontSize: 11, color: "var(--text-card)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                  {entry.caption || "—"}
                </div>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 4 }}>
                  <span style={{ fontSize: 10, color: "var(--text-muted)" }}>🗳 {entry.voteCount}</span>
...
const sectionLead: CSSProperties = { fontSize: 12, color: "var(--text-muted)", marginBottom: 14, lineHeight: 1.5 };
const sectionHeading: CSSProperties = { fontSize: 11, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: 0.5, margin: "12px 0 8px", fontWeight: 700 };
const muted: CSSProperties = { color: "var(--text-muted)", textAlign: "center", padding: "24px 0", fontSize: 12 };
const emptyBox: CSSProperties = {
  padding: 16, textAlign: "center", color: "var(--text-muted)", fontSize: 12,
  background: "var(--bg-card)", borderRadius: 10, border: "1px solid rgba(255,255,255,0.06)",
};
...
const rowMeta: CSSProperties = { fontSize: 11, color: "var(--text-muted)", marginBottom: 8 };
...
const smallBtn: CSSProperties = {
  ...
  background: "var(--bg-overlay-light)", border: "1px solid rgba(255,255,255,0.1)", color: "var(--text-card)",
};
...
const inputStyle: CSSProperties = {
  ...
  color: "var(--text-card)", fontSize: 12, boxSizing: "border-box" as any,
};
...
const entryImagePlaceholder: CSSProperties = {
  ...
  fontSize: 28, background: "var(--bg-card)",
};
