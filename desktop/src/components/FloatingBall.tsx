  ...
  try {
    ...
    const menu = await Menu.new({
      items: [
        ...
        { id: "ball-new-chat", text: "🆕 新对话", action: () => { void dispatchSafe("new-chat"); } },
        { id: "ball-voice", text: "🎤 语音输入", action: () => { void dispatchSafe("voice-start"); } },
        { item: "Separator" } as any,
        // ── 我的萌宠 ─────────
        { id: "ball-wardrobe", text: "🐾 我的萌宠 (衣柜)", action: () => { void dispatchSafe("open-wardrobe"); } },
        { id: "ball-pet", text: "✨ 创建新萌宠", action: () => { void dispatchSafe("open-pet-creator"); } },
        { id: "ball-pet-growth", text: "📊 成长 / 成就 / 相册", action: () => { void dispatchSafe("open-pet-growth"); } },
        ...
        {
          id: "ball-world-creator",
          text: "🌐 World 创作器 (Tier_C)",
          action: () => { window.dispatchEvent(new CustomEvent("agentrix:open-creator-studio", { detail: { tab: "world" } })); },
        },
        ...
        { item: "Separator" } as any,
        // ── 设置 ─────────
        { id: "ball-settings", text: "⚙️ 设置", action: () => { void dispatchSafe("open-settings"); } },
      ],
    });
    await menu.popup();
  } catch (err) {
      ...
      >
        ...
        {([
          ...
          <div
            ...
            onMouseEnter={(e) => (e.currentTarget.style.background = "var(--bg-overlay-light)")}
    ...
    {!compact && contextMenu && (
      ...
      >
        {[
          ...
          { divider: true } as any,
          // ── 我的萌宠 ─────────────
          { label: "🐾 我的萌宠 (衣柜)", action: () => { void import("../services/desktopBus").then(({ dispatchUiAction }) => dispatchUiAction("open-wardrobe")); } },
          { label: "✨ 创建新萌宠", action: () => { void import("../services/desktopBus").then(({ dispatchUiAction }) => dispatchUiAction("open-pet-creator")); } },
          { label: "📊 成长 / 成就 / 相册", action: () => { void import("../services/desktopBus").then(({ dispatchUiAction }) => dispatchUiAction("open-pet-growth")); } },
          { divider: true } as any,
          // ── 创作工坊 ─────────────
          ...
          (item as any).divider ? (
            <div key={`div-${idx}`} style={{ height: 1, background: "var(--bg-overlay-medium)", margin: "4px 0" }} />
          ) : (
            <div
              key={item.label}
              onClick={item.action}
