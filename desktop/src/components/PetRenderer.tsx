...
interface Props {
  ...
  /** Disable the drop-shadow aura halo (for pet-companion floating window). */
  noHalo?: boolean;
}
...
export default function PetRenderer({ size, style, showLevelBadge, noHalo }: Props) {
  ...
  if (activeRendererId === "vrm" && snapshot.vrmUrl) {
    return (
      <Suspense fallback={<PetCanvas size={size} style={style} showLevelBadge={showLevelBadge} noHalo={noHalo} />}>
  ...
  return <PetCanvas size={size} style={style} showLevelBadge={showLevelBadge} noHalo={noHalo} />;
}
