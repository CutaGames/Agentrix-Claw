...
export default function VoiceButton({
  ...
  useEffect(() => {
    ...
    // Sprint v0.3.6: catch-up dispatch — if the pet menu set a
    // pending-voice-start flag before this VoiceButton mounted,
    // pick it up now. The flag is consumed atomically so a fast
    // event won't double-trigger.
    try {
      const pending = window.localStorage.getItem("agentrix_pending_voice_start");
      if (pending) {
        const stamp = Number(pending);
        // Only honor the flag if it's < 5 s old (otherwise it's stale).
        if (Number.isFinite(stamp) && Date.now() - stamp < 5000) {
          window.localStorage.removeItem("agentrix_pending_voice_start");
          // Defer one tick so any in-flight setup completes.
