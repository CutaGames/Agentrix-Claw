  ...
  return (
    <div style={hostStyle}>
      ...
      <div style={petLayerStyle}>
        <PetRenderer size={BALL_SIZE - 8} style={{
          ...petStyle,
          // Breathing animation in idle, bounce when active — gives life to
          // the pet even when nothing is happening.
          animation: state === "idle"
            ? "agentrix-pet-breathe 4s ease-in-out infinite"
            : state === "thinking"
              ? "agentrix-pet-breathe 6s ease-in-out infinite"
              : state === "speaking"
                ? "agentrix-pet-bounce 1.2s ease-in-out infinite"
                : state === "recording"
                  ? "agentrix-pet-shake 0.4s ease-in-out infinite"
                  : "agentrix-pet-breathe 4s ease-in-out infinite",
        }} />
      </div>
      ...
      {/* Agent Ops: crypto-native B2B 专业工作台 (tasks/尽调/monitors/security/deliverables/metrics/packages/team). */}
      <AgentOpsPanel visible={agentOpsOpen} initialTab={agentOpsTab} onClose={() => setAgentOpsOpen(false)} />

      {/* Proactive companion bubbles — show pet greetings/suggestions in main window too */}
      <div style={{ position: "absolute", bottom: "100%", left: "50%", transform: "translateX(-50%)", marginBottom: 8, zIndex: 10 }}>
        <PetProactiveBubble />
      </div>

      {/* Pet animation keyframes — needed in main window for breathing/bounce effects */}
      <style>{`
