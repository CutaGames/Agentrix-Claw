...
export default function WardrobePanel({ onClose }: Props) {
  ...
  const [listingSkin, setListingSkin] = useState<PetSkinSummary | null>(null);
      ...
      >
                  ...
                  {skins.map((s) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {s.source === "generated" && (
                          <button
                            data-testid={`wardrobe-list-${s.id}`}
                            onClick={() => setListingSkin(s)}
                            className="rounded-md px-2 py-1 text-xs font-medium bg-purple-500/20 text-purple-300 hover:bg-purple-500/30 transition"
                          >
                            🛍 上架
                          </button>
                        )}
        ...
        {listingSkin && (
          <MarketplaceListingModal
            ...
            onSuccess={() => {
              setTimeout(() => setListingSkin(null), 2000);
            }}
          />
        )}
      </div>
