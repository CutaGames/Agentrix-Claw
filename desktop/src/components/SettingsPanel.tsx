...
import { isAnalyticsOptedIn, optInAnalytics, optOutAnalytics } from "../services/analytics";
...
export default function SettingsPanel({ ttsEnabled, onTtsToggle, onClose, models = [], selectedModel = "", onModelChange }: Props) {
  ...
  const [telemetryOptIn, setTelemetryOptIn] = useState<boolean>(() => isAnalyticsOptedIn());
  ...
  // v0.7.16 — read current app version dynamically from Tauri so the Settings
  // panel reflects whatever installer the user actually has installed,
  // instead of the hard-coded "v0.3.0" that has been wrong for ~10 versions.
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        // Not running in Tauri (web preview) — leave blank.
      }
    })();
    return () => { cancelled = true; };
  }, []);
  // Auto-check for updates once when Settings opens — non-blocking, silent
  // failure. User can still manually retry via the button.
  useEffect(() => {
    let cancelled = false;
    const t = setTimeout(async () => {
      try {
        ...
        // silent — keep status at idle so user can manually click
      }
    }, 800);
    ...
    <div style={overlay} onClick={onClose}>
      <div style={panel} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={section}>
          ...
          <ToggleRow
            label="Light mode (实验性 / experimental)"
            value={theme === "light"}
            onChange={handleToggleTheme}
          />
          {/* Sprint Pre-launch P-3 (2026-05-23) — User experience-mode
              picker. Default Simple keeps non-coder users from being
              overwhelmed by tier router / risk levels / 9 deep panels. */}
          <UserModeRow />
        </div>
        ...
        {/* Privacy / Telemetry — Sprint G-2 / US-G2-4 */}
        <div style={section}>
          ...
          <div style={{ fontSize: 11, color: "var(--text-dim)", padding: "4px 0", lineHeight: "16px" }}>
            匿名启动 / 登录 / 首次对话事件，帮助我们改进 Agentrix。崩溃报告独立机制，仅含设备指纹（始终上报）。
          </div>
        </div>
        ...
        <div style={{ ...section, borderBottom: "none" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "4px 0" }}>
            <span style={{ fontSize: 12, color: "var(--text-dim)" }}>
              Agentrix Desktop {currentVersion ? `v${currentVersion}` : ""}
            </span>
            {updateStatus === "idle" && (
...
// Sprint Pre-launch P-3 (2026-05-23) — User experience mode (Simple /
// Standard / Pro). See `services/userMode.ts` and the positioning doc
// `docs/agentrix-positioning-2026-05.zh-CN.md` for rationale.
function UserModeRow() {
  const mode = useUserModeStore((s) => s.mode);
  const setMode = useUserModeStore((s) => s.setMode);
  const options: Array<{ id: UserMode; label: string; description: string }> = [
    { id: "simple", label: "Simple · 简单", description: "默认。隐藏高级开关,接近 ChatGPT 体验" },
    { id: "standard", label: "Standard · 标准", description: "显示工具调用、Plan/Agent/Ask 切换、跨端" },
    { id: "pro", label: "Pro · 高级", description: "全部高级开关 / tier 路由 / 记忆 wiki" },
  ];
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8, paddingTop: 8 }}>
      <div style={{ fontSize: 12, color: "var(--text-dim)" }}>使用深度 / Experience depth</div>
        ...
        {options.map((option) => {
          ...
          return (
            ...
            >
              ...
              <div style={{ fontSize: 10, opacity: active ? 0.92 : 0.7, marginTop: 2 }}>
                {option.description}
              </div>
            </button>
          );
        ...
        {RECOMMENDED_MODELS.map((rm) => {
          ...
          return (
            <div key={rm.id} style={{
              ...
              background: downloaded ? "rgba(16,185,129,0.06)" : "var(--bg-card)",
              ...
              {isDownloading && (
                <>
                  <div style={{
                    height: 4, background: "var(--bg-overlay-medium)", borderRadius: 2,
            ...
            {models.map((m) => {
              ...
              return (
                <div key={m.path} style={{
                  ...
                  background: isDefault ? "rgba(16,185,129,0.06)" : "var(--bg-card)",
