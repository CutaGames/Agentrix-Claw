...
export interface TaskTimelineEntry {
  ...
  finishedAt?: number;
  output?: string;
  /** Multi-Agent v1 — when set, render an AgentIdentityCard at the row start. */
  actor?: {
    agentId?: string | null;
    role?: string;
    displayName?: string;
  ...
  /** Multi-Agent v1 — sub-task id for [sub-task #...] anchors + retry/rollback. */
  subTaskId?: string;
  /** Multi-Agent v1 — total cost USD for agent_result rows. */
  costUsd?: number;
  /** Multi-Agent v1 — duration ms; if set, prefer over startedAt/finishedAt diff. */
  durationMs?: number;
}
...
interface Props {
  ...
  /** Multi-Agent v1 — fired when user clicks Retry on a failed sub-task row. */
  onRetrySubTask?: (subTaskId: string) => void;
  /** Multi-Agent v1 — fired when user clicks Rollback on a sub-task row. */
  onRollbackSubTask?: (subTaskId: string) => void;
}
...
export default function TaskTimeline({ status, entries, onRetrySubTask, onRollbackSubTask }: Props) {
                    ...
                    {group.entries.map((entry) => (
                      <div key={entry.id} style={detailStepStyle}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontSize: 11, fontWeight: 500, display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                            {/* Multi-Agent v1 — show actor identity badge for sub-agent rows */}
                            {entry.actor?.agentId && (
                              <AgentIdentityCard
                            ...
                            {/* Multi-Agent v1 — cost USD chip on agent_result */}
                            {typeof entry.costUsd === "number" && entry.costUsd > 0 && (
                              <span style={costChip}>${entry.costUsd.toFixed(4)}</span>
                            )}
                          </div>
                          {entry.detail && (
                          ...
                          {/* Multi-Agent v1 — Retry / Rollback for failed/completed sub-tasks */}
                          {entry.kind === "agent_result" && entry.subTaskId && (
                            <div style={{ display: "flex", gap: 6, marginTop: 4 }}>
                              {entry.status === "failed" && onRetrySubTask && (
                                <button
                                  type="button"
                                  style={subTaskActionBtn}
                                  onClick={() => onRetrySubTask(entry.subTaskId!)}
                                >
                                  ↻ Retry
                                </button>
                              )}
                              {onRollbackSubTask && (
                                <button
                                  type="button"
                                  style={subTaskActionBtn}
                                  onClick={() => onRollbackSubTask(entry.subTaskId!)}
                                >
                                  ⏎ Rollback
                                </button>
                              )}
...
function formatDuration(entry: TaskTimelineEntry) {
  // Multi-Agent v1 — `agent_result` rows carry an authoritative durationMs;
  // prefer it over startedAt/finishedAt diff (which can drift across windows).
...
// ── Multi-Agent v1 styles ──────────────────────────────────────────────

const costChip: CSSProperties = {
  fontSize: 9,
  fontWeight: 600,
  padding: "2px 6px",
  borderRadius: 999,
...
const subTaskActionBtn: CSSProperties = {
  fontSize: 10,
  padding: "2px 8px",
  borderRadius: 6,
  border: "1px solid var(--border)",
