      ...
      {workspaces.length > 0 ? (
        workspaces.map((ws) => (
          <div key={ws.workspaceId} style={styles.card}>
            ...
            {ws.description && (
              <div style={{ fontSize: 12, color: "var(--text-muted)", marginTop: 4 }}>{ws.description}</div>
            )}
          </div>
        ))
      ) : (
...
const styles: Record<string, CSSProperties> = {
  ...
  deviceLabel: { fontSize: 11, color: "var(--text-dim)" },
  timeLabel: { fontSize: 10, color: "var(--text-muted)" },
  sectionTitle: {
    fontSize: 12, fontWeight: 700, color: "var(--text-muted)",
    textTransform: "uppercase" as const, letterSpacing: 0.5, padding: "8px 0 4px",
  },
  empty: { textAlign: "center", color: "var(--text-dim)", fontSize: 13, padding: 32 },
  onlineDot: { color: "#22c55e", fontSize: 12 },
  offlineDot: { color: "var(--text-dim)", fontSize: 12 },
  approveBtn: {
