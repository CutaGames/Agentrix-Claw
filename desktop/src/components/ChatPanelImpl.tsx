...
import WardrobeVariantPanel from "./WardrobeVariantPanel";
...
import { getDesktopDeviceId, invokeDesktopCommand } from "../services/desktop";
import {
...
import TodaysChangesPanel from "./TodaysChangesPanel";
...
import WorkspaceDiffWorkbench from "./WorkspaceDiffWorkbench";
import SelfEvolutionDashboardPanel from "./SelfEvolutionDashboardPanel";
import AgentTeamPanel from "./AgentTeamPanel";
import { useUserMode } from "../services/userMode";
import HandoffBanner from "./HandoffBanner";
import BackgroundTasksBanner from "./BackgroundTasksBanner";
...
import { detectHardwareProfile, type HardwareProfile } from "../services/hardwareProfile";
import {
...
import { useUiFeedbackStore } from "./chatPanel/uiFeedbackStore";
import { useFeedbackTickDriver } from "./chatPanel/useFeedbackTimer";
...
export default function ChatPanel({
  ...
  // Track whether we've already warned about insufficient hardware on
  // this session so we don't spam every time the user toggles modes.
  const hardwareWarnedRef = useRef(false);
  ...
  // Sprint P-7 (2026-05-22): variant picker for clan / skin / festival
  // sprite swapping (driven by `petVariant` store).
  ...
  // When the Agent Economy panel toggles, broadcast the app mode so the
  // floating-ball pet swaps to its 商人态 (economy) variant. When closed,
  // restore pro-mode (since the chat panel is open).
  useEffect(() => {
    const mode = economyPanelOpen ? "economy-panel" : "pro-mode";
  ...
  // Multi-Agent Collaboration v1 W1 (2026-05-26) — top-level Agent Team
  // panel state. Opened from ChatTitleBar More menu OR from
  // `agentrix:open-agent-team-panel` event (R3.1).
  const [agentTeamPanelOpen, setAgentTeamPanelOpen] = useState(false);
  const userMode = useUserMode();
  // Sprint Pro Mode Coding Views (2026-05-24): Pro-only Workspace Diff
  // Workbench. See positioning §3.4 (C_Path coding parity).
  const [diffWorkbenchOpen, setDiffWorkbenchOpen] = useState(false);
  // Sprint Post-launch P-2 (2026-05-24): Self-Evolution Dashboard for
  // Standard / Pro Mode. Visualizes long-memory + dreaming + wiki to
  // make A_Path differentiation #5 visible.
  const [selfEvolutionPanelOpen, setSelfEvolutionPanelOpen] = useState(false);
  // Sprint Pre-launch P-3 (2026-05-23) — "Today's changes" panel state.
  const [todaysChangesOpen, setTodaysChangesOpen] = useState(false);
  // Sprint Pre-launch P-4 (2026-05-23) — Background tasks panel state.
  const [backgroundTasksOpen, setBackgroundTasksOpen] = useState(false);
  const [offlineQueueCount, setOfflineQueueCount] = useState(0);

  // Sprint Pre-launch P-1 (2026-05-23) — high-frequency UI feedback moved to
  // a dedicated zustand store so setStreamFeedback / setActiveToolRun no longer
  // re-render the 3708-line ChatPanelImpl on every chunk / tool event /
  // approval submission. Variable names kept identical so the rest of the
  // file (and the ~80 useStreamingTurn callsites) require no changes.
  const approvalSubmitting = useUiFeedbackStore((s) => s.approvalSubmitting);
  const setApprovalSubmitting = useUiFeedbackStore((s) => s.setApprovalSubmitting);
  ...
  const handleRetryMessage = useCallback((msgId: string) => {
    ...
    // v0.7.8 — clear error-bucket for this session when user explicitly hits Retry.
    // Otherwise the v0.7.6 anti-loop bucket keeps any session above the >=3
    // failures threshold permanently, so subsequent stream errors land in
    // "连续失败 N 次" branch (no Continue button + no auto-retry guidance) even
    // though the user is intentionally re-trying. Resetting the bucket lets a
    // single recovery succeed without forcing a panel restart.
    try {
      const bucketWin = (window as any).__agentrixErrBucket;
      if (bucketWin?.delete) bucketWin.delete(`errBucket:${sessionIdRef.current}`);
    } catch {
      /* non-window env */
    }
    // v0.7.8 — also force-reset sending=false on retry. errorHandler should
    // already have done this, but if a hung native fetch never settled the
    // flag stays true and handleSend short-circuits. Belt-and-braces.
    patchSessionRuntime(sessionIdRef.current, { sending: false });
  ...
  // Sprint Pre-launch P-1 (2026-05-23) — High-frequency UI feedback now
  // lives in the dedicated `useUiFeedbackStore`. ChatPanelImpl pulls only
  // the SETTERS (which are stable references and never trigger re-renders)
  // and consumes the values inside isolated subscriber components
  // (StreamFeedbackBanner inside InputZone, etc.). This is the core fix
  // for the "audit approval feels frozen / typing lag" bug — previously
  // every setStreamFeedback / setActiveToolRun call from useStreamingTurn
  // re-rendered the entire 3708-line ChatPanelImpl, which in turn
  // reconciled all 30+ panel mounts and walked the giant `[style*=]`
  // cascade in global.css.
  const setStreamFeedback = useUiFeedbackStore((s) => s.setStreamFeedback);
  const setActiveToolRun = useUiFeedbackStore((s) => s.setActiveToolRun);
  const setSendStartedAt = useUiFeedbackStore((s) => s.setSendStartedAt);
  ...
  // Sprint Pre-launch P-1 (2026-05-23) — feedbackNow lives in the store now,
  // and ChatPanelImpl does NOT subscribe to it (that was causing the 2-second
  // full-tree re-render). The store is updated by useFeedbackTickDriver below
  // (a centralized 2 s tick that only runs when something is actually being
  // timed), and consumed by InputZone's StreamFeedbackBanner via its own hook.
  // ChatPanelImpl no longer needs the value, so we don't read it here.
  useFeedbackTickDriver();
  const {
    continuePrompt,
    setContinuePrompt,
    cancelAutoContinue,
  ...
  // Sprint Pre-launch P-1 (2026-05-23) — `feedbackElapsedSeconds` and
  // `visibleStreamFeedback` were derived state subscribing to feedbackNow,
  // which forced a full ChatPanelImpl re-render every 2 s. They now live
  // inside `useVisibleStreamFeedback` and are consumed directly by
  // InputZone's StreamFeedbackBanner.
  ...
  // Sprint Pre-launch P-1 (2026-05-23) — the 2 s `setFeedbackNow` tick that
  // used to live here is now inside `useFeedbackTickDriver()` (called near
  // the top of this component). The driver writes to the zustand store but
  // ChatPanelImpl does not subscribe to `feedbackNow`, so the tick no longer
  // re-renders the entire panel.
  ...
  const recordToolTimelineEvent = useCallback((
    ...
    patchSessionRuntime(sessionId, (current) => {
      ...
      // Only flip to "executing"/"failed" while there's actually a live or
      // recently failed tool — otherwise drop back to "idle" so the workbench
      // banner doesn't get pinned forever after a turn ends.
      const RECENT_FAIL_WINDOW_MS = 8_000;
      const now = Date.now();
      const recentFailure = timelineEntries.some((candidate) =>
        (candidate.status === "failed" || candidate.status === "rejected")
        && typeof candidate.finishedAt === "number"
        && now - candidate.finishedAt < RECENT_FAIL_WINDOW_MS
      );
      const nextTaskStatus: TaskRunState = hasRunning
        ? "executing"
        : recentFailure
          ? "failed"
          : "idle";
      return {
        ...
        desktopTaskStatus: nextTaskStatus,
  ...
  // Hardware detection — used to warn the user when picking 端侧 (local-only)
  // on a machine that doesn't meet the recommended specs. Cached for 7 days
  // by `detectHardwareProfile`.
  useEffect(() => {
    let cancelled = false;
    void detectHardwareProfile()
      .then((p) => { if (!cancelled) setHardwareProfile(p); })
      .catch(() => { /* ignore */ });
    return () => { cancelled = true; };
  }, []);

  // Show a one-shot system message when 端侧 is picked on insufficient HW.
  useEffect(() => {
    if (executionMode !== "local-only") return;
    if (!hardwareProfile) return;
    if (hardwareWarnedRef.current) return;
    const tier = hardwareProfile.recommended_tier;
    if (tier !== "unsupported" && tier !== "light") return;

    const ramGb = (hardwareProfile.ram_total_mb / 1024).toFixed(1);
    const vramGb = hardwareProfile.gpu_vram_mb ? (hardwareProfile.gpu_vram_mb / 1024).toFixed(1) : null;
    const gpu = hardwareProfile.gpu_name || "无独立 GPU";
    const lines = [
      tier === "unsupported"
        ? "⚠️ **你的硬件不建议运行本地模型**"
        : "ℹ️ **你的硬件可以勉强跑本地小模型，但会比较慢**",
      "",
      `当前配置：CPU ${hardwareProfile.cpu_cores} 核 · 内存 ${ramGb} GB · GPU ${gpu}${vramGb ? ` (${vramGb} GB VRAM)` : ""}`,
      "",
      "**推荐本地模型最低配置：**",
      "• CPU 8 核以上",
      "• 16 GB 系统内存",
      "• NVIDIA GPU 8 GB+ VRAM（或 Apple Silicon 16 GB+ 统一内存）",
      "• 30 GB 可用磁盘空间",
      "",
      tier === "unsupported"
        ? "建议切换到 **云端** 或 **智能** 模式（顶部栏切换），平台会用 Claude / GPT 等高性能云端模型为你服务，体验流畅得多。"
        : "如要继续使用本地，建议先从 Llama 3.2 3B / Gemma Nano 这类小模型起步，并在设置里关闭 Computer Use 工具集减少 prompt 体积。",
    ];
    addSystemMessage(lines.join("\n"));
    hardwareWarnedRef.current = true;
  }, [executionMode, hardwareProfile, addSystemMessage]);
  ...
  const handleSlashCommand = useCallback(async (text: string): Promise<boolean> => {
    ...
    if (trimmed === "/help") {
      addSystemMessage(
        ...
        "• `/diag` — Export stream diagnostics (网络问题排查)\n" +
    ...
    // /diag — export desktop SSE stream diagnostics for the last 5 turns.
    // v0.7.10 — gives user a one-shot dump of what the stream lifecycle
    // actually looked like (transport attempts, first byte, tool events,
    // watchdog timeouts, error sources) so we can root-cause "几秒就网络
    // 中断" without guessing. Output is also copied to clipboard.
    if (trimmed === "/diag" || trimmed === "/diagnostics") {
      try {
        const { formatStreamDiagnostics, getStreamDiagnostics } = await import("../services/streamDiagnostics");
        const text = formatStreamDiagnostics(8);
        const events = getStreamDiagnostics();
        try {
          await navigator.clipboard.writeText(text);
        } catch { /* clipboard may be denied */ }
        addSystemMessage(
          `📊 **Stream Diagnostics** (${events.length} events)\n\n` +
          "已复制到剪贴板。粘贴给开发者排查。\n\n" +
          "```\n" + text.slice(0, 4000) + (text.length > 4000 ? "\n... (truncated, full copy in clipboard)" : "") + "\n```",
        );
      } catch (err: any) {
        addSystemMessage(`❌ /diag failed: ${err?.message || err}`);
      }
      return true;
    }

    if (trimmed === "/diag clear" || trimmed === "/diagnostics clear") {
      try {
        const { clearStreamDiagnostics } = await import("../services/streamDiagnostics");
        clearStreamDiagnostics();
        addSystemMessage("📊 Stream diagnostics cleared.");
      } catch (err: any) {
        addSystemMessage(`❌ /diag clear failed: ${err?.message || err}`);
      }
      return true;
    }
  ...
  const handleSend = useCallback(
    async (overrideText?: string) => {
      ...
      // US-G2-4: one-shot first-chat telemetry
      if (localStorage.getItem("agentrix_first_chat_seen") !== "1") {
        trackEvent("desktop_first_chat", { mode: effectiveChatMode });
        try { localStorage.setItem("agentrix_first_chat_seen", "1"); } catch {}
      }
  ...
  }, [handleSend, onClose, createNewTab, closeTab, activeTabId]);

  // Handle paste events — intercept clipboard images and add as attachments
  const handlePaste = useCallback(async (e: ReactClipboardEvent<HTMLTextAreaElement>) => {
    ...
    try {
      ...
      const uploaded = await Promise.all(
        files.map((file) => uploadChatAttachment(file, token)),
      );
      setPendingAttachments((prev) => [...prev, ...uploaded]);
    } catch (error: any) {
      window.alert(error?.message || "Failed to upload pasted image");
    } finally {
      setUploadingAttachments(false);
    }
  }, [token]);

  const handleSendVoid = useCallback(() => {
    void handleSend();
  }, [handleSend]);
  ...
  const deleteSession = useCallback(async (sid: string) => {
    if (!confirm("永久删除这个对话?所有消息将无法恢复。")) return;
    // v0.7.19 — also delete server-side session + messages so backend
    // doesn't accumulate orphan rows. Best-effort: if backend call fails
    // (offline / 404), still remove the local cache so the UI is clean.
    if (token) {
      try {
        await deleteServerSessionApi(token, sid);
      } catch (err) {
        console.warn("[ChatPanel] backend deleteSession failed (continuing with local cleanup):", err);
      }
    }
  ...
  useEffect(() => {
    ...
    // Multi-Agent Collaboration v1 W1 (2026-05-26) — listen for the
    // top-level "🤖 Agent 团队" entry from ChatTitleBar More menu OR
    // any future cross-window dispatch (relay_event).
    const onOpenAgentTeamPanel = () => setAgentTeamPanelOpen(true);
    const onOpenPetVariant = () => setVariantPanelOpen(true);
    ...
    // Sprint v0.3.6: pet menu "✅ 待审批操作" → open notification center
    // (which lists pending approvals).
    const onOpenApprovals = () => setNotifOpen(true);
    // Sprint Pre-launch P-3 (2026-05-23) — "Today's changes" panel.
    const onOpenTodaysChanges = () => setTodaysChangesOpen(true);
    // Sprint Pre-launch P-4 (2026-05-23) — Background tasks panel.
    const onOpenBackgroundTasks = () => setBackgroundTasksOpen(true);
    // Sprint Pre-launch P-3 (2026-05-23) — turn-summary "look at the
    // screen" button dispatches `agentrix:turn-screenshot` with a data URL.
    // Attach it as a tool artifact on the target assistant message so it
    // renders in-bubble, just like a regular `computer_use_screenshot`.
    const onTurnScreenshot = (event: Event) => {
      const detail = (event as CustomEvent).detail || {};
      const messageId = String(detail.messageId || "");
      const dataUrl = String(detail.dataUrl || "");
      if (!messageId || !dataUrl) return;
      updateSessionMessages(sessionIdRef.current, (prev) =>
        prev.map((m) => m.id === messageId
          ? {
              ...m,
              toolArtifacts: [
                ...(m.toolArtifacts || []).filter((a) => a.toolCallId !== `verify-${messageId}`),
                {
                  toolCallId: `verify-${messageId}`,
                  toolName: "verify_screenshot",
    ...
    window.addEventListener("agentrix:open-approvals", onOpenApprovals);
    window.addEventListener("agentrix:open-todays-changes", onOpenTodaysChanges);
    window.addEventListener("agentrix:open-background-tasks", onOpenBackgroundTasks);
    window.addEventListener("agentrix:turn-screenshot", onTurnScreenshot as EventListener);
    ...
    window.addEventListener("agentrix:open-agent-team-panel", onOpenAgentTeamPanel);
    window.addEventListener("agentrix:open-pet-variant", onOpenPetVariant);
    ...
    (async () => {
      try {
        ...
        const events = [
          ...
          "agentrix:open-agent-team-panel",
    ...
    return () => {
      ...
      window.removeEventListener("agentrix:open-agent-team-panel", onOpenAgentTeamPanel);
      window.removeEventListener("agentrix:open-pet-variant", onOpenPetVariant);
      ...
      window.removeEventListener("agentrix:open-approvals", onOpenApprovals);
      window.removeEventListener("agentrix:open-todays-changes", onOpenTodaysChanges);
      window.removeEventListener("agentrix:open-background-tasks", onOpenBackgroundTasks);
      window.removeEventListener("agentrix:turn-screenshot", onTurnScreenshot as EventListener);
  ...
  useEffect(() => {
    ...
    const handleApprovalNew = (event: Event) => {
      ...
      // Pause pet window animations while approval is up — frees GPU/CPU
      // for smooth approval modal rendering.
      if (approval?.status === "pending") {
        void invokeDesktopCommand("desktop_pet_window_set_approval_active", { active: true }).catch(() => {});
      }
    };
    const handleApprovalResponse = (event: Event) => {
      ...
      // Resume pet window animations now that approval modal is dismissed.
      void invokeDesktopCommand("desktop_pet_window_set_approval_active", { active: false }).catch(() => {});
    };
  ...
  // ─────────────────────────────────────────────────────────────────────
  // Multi-Agent v1 W4.7 — aggregated Leader inject when CompanionBall
  // badge is clicked AND there are unacknowledged completions. Fired
  // by PetCompanionWindow with `{ taskIds: string[] }`.
  // Spec: design.md §7.4; tasks.md W4.7
  // ─────────────────────────────────────────────────────────────────────
  useEffect(() => {
    const handleAggregatedSummary = (e: Event) => {
      const detail = (e as CustomEvent<{ taskIds?: string[] }>).detail;
      const taskIds = detail?.taskIds ?? [];
      if (taskIds.length === 0) return;
      const targetSessionId = sessionIdRef.current;
      if (!targetSessionId) return;

      const lines = [
        `我帮你跑了 ${taskIds.length} 个 sub-task:`,
        ...taskIds.map((id) => `- [sub-task #${id.slice(0, 8)}]`),
        "",
        "需要看哪个的详情?",
      ];
      const content = lines.join("\n");
      const messageId = `agg-${targetSessionId}-${Date.now()}`;
      updateSessionMessages(
        targetSessionId,
        (prev) =>
          prev.concat({
        ...
        { persist: true, markUnread: false },
      );
    };
    window.addEventListener("agentrix:open-chat-with-summary", handleAggregatedSummary as EventListener);
    return () =>
      window.removeEventListener(
        "agentrix:open-chat-with-summary",
        handleAggregatedSummary as EventListener,
      );
  }, [updateSessionMessages]);
  ...
  //
  // Bugfix (移动一下就消失): dragging/moving the Tauri window enters an OS
  // modal move loop that fires a focus-lost event. Without a guard this
  // closed the panel as soon as the user dragged it. We suppress the
  // blur-driven close for a short window whenever the window is being
  // moved (onMoved) or a title-bar drag just started
  // (suppressAutoCloseUntilRef, set in handleTitleBarMouseDown). The panel
  // now only dismisses on an explicit click-outside, never on move/drag.
  useEffect(() => {
      ...
      try {
        ...
        if (win.label === "chat-panel") {
          unlistenMoved = await win.onMoved(() => {
            // Keep extending the suppression window while the user drags.
            suppressAutoCloseUntilRef.current = Date.now() + 800;
          });
          unlistenFocus = await win.onFocusChanged(({ payload: focused }) => {
            if (!focused && !effectiveProMode) {
              // Skip the auto-close if we just moved/started dragging the
              // window — the focus loss is caused by the OS move loop, not
              // by the user clicking away.
              if (Date.now() < suppressAutoCloseUntilRef.current) return;
              onClose();
            }
          });
        }
  ...
  // Timestamp until which blur-driven auto-close is suppressed (set while
  // dragging/moving the window). See the onFocusChanged effect above.
  const suppressAutoCloseUntilRef = useRef(0);
  useEffect(() => {
  ...
  const handleTitleBarMouseDown = useCallback((event: MouseEvent<HTMLDivElement>) => {
    ...
    // Suppress blur-driven auto-close while the OS window move loop runs.
    suppressAutoCloseUntilRef.current = Date.now() + 1500;
  ...
  const panel: CSSProperties = {
    ...
    overflow: "hidden",
    // v0.7.6 hotfix — switched hardcoded dark drop-shadow + ghost-white border
    // to themable CSS variables so Light theme actually shows a panel boundary
    // (instead of "white panel on white background" no-contrast).
    boxShadow: "var(--shadow-panel)",
    border: "1px solid var(--border)",
    animation: effectiveProMode ? "none" : "slideIn 0.2s ease-out",
  };

  // Memoized children for <MessageList> so the 2s feedbackNow re-render
  // doesn't reconcile the bubble list (which involves remark/highlight per
  // bubble). When activePlan / token / activeInstanceId change, we recompute.
  const planPanelNode = useMemo(() => {
    if (!activePlan) return undefined;
    const status = String(activePlan.status || "").toLowerCase();
    const dismissed = ["completed", "failed", "rejected", "cancelled", "canceled", "done"].includes(status);
    if (dismissed && !sending) return undefined;
    return (
      <PlanPanel
        plan={activePlan}
        onApprove={async () => {
        ...
        onDismiss={() => setPlanForSession(sessionIdRef.current, null)}
      />
    );
  }, [activePlan, sending, token, chatMode, handleSend, setPlanForSession]);

  // v0.7.6 hotfix — duplicate Context bar removed.
  // The MessageList-tail ContextVisualizer overlapped with the
  // InputZone token bar (showing the same data twice and confusing
  // users about why the readouts diverged). The InputZone bar is
  // closer to the input field and more useful, so we drop the
  // visualizer above it. ContextVisualizer is still kept as a module
  // (`/context` slash command opens it inline as a system message)
  // for users who want the detailed breakdown.
  const contextVisualizerNode = undefined;
  ...
  return (
    ...
    >
      ...
      <ChatTitleBar
        ...
        setAgentTeamPanelOpen={setAgentTeamPanelOpen}
        setDiffWorkbenchOpen={setDiffWorkbenchOpen}
        setSelfEvolutionPanelOpen={setSelfEvolutionPanelOpen}
      ...
      {/* Sprint P-7：形态变体选择(clan / skin / festival) */}
      {variantPanelOpen && (
        <div
          role="dialog"
          aria-modal="true"
          ...
          onClick={(e) => {
            if (e.target === e.currentTarget) setVariantPanelOpen(false);
          }}
        >
          <WardrobeVariantPanel onClose={() => setVariantPanelOpen(false)} />
        </div>
      )}
      ...
      <WorkspaceDiffWorkbench open={diffWorkbenchOpen} onClose={() => setDiffWorkbenchOpen(false)} />
      <SelfEvolutionDashboardPanel open={selfEvolutionPanelOpen} onClose={() => setSelfEvolutionPanelOpen(false)} />
      <AgentTeamPanel
        open={agentTeamPanelOpen}
        onClose={() => setAgentTeamPanelOpen(false)}
        userMode={userMode}
      />
            ...
            {historyEntries.length === 0 ? (
              ...
              historyEntries
                ...
                <div
                  ...
                  onMouseEnter={(e) => { if (s.id !== sessionIdRef.current) e.currentTarget.style.background = "var(--bg-card)"; }}
      ...
      {/* Sprint Pre-launch P-3 (2026-05-23) — Background tasks banner.
          Shows above the message list whenever a turn was queued for
          off-thread execution. Backed by `services/backgroundTasks.ts`
          (front-end stub for now; backend job queue lands next sprint). */}
      ...
      <MessageList
        ...
        planPanel={planPanelNode}
        contextVisualizer={contextVisualizerNode}
      />
      ...
      {/* Sprint Pre-launch P-3 (2026-05-23) — Today's changes audit panel. */}
      <TodaysChangesPanel
        open={todaysChangesOpen}
        onClose={() => setTodaysChangesOpen(false)}
      />
