...
export default function PetVRM({ url, size = 96, style, showLevelBadge = true }: Props) {
    ...
    loader.load(
      ...
      (gltf) => {
        ...
        if (vrm) {
          ...
          // Remove flat base plates (common in Hunyuan3D exports)
          root.traverse((child) => {
            if ((child as any).isMesh) {
              ...
              if (geo) {
                ...
                // If the mesh is very flat (height < 5% of width/depth), it's a base plate
                if (height < Math.max(width, depth) * 0.05 && width > 0.1) {
                  mesh.visible = false;
                }
              }
            }
  ...
  // FIX 2026-05-21 (Sprint P-1): when the VRM/GLB fails to load (e.g. a
  // 21 MB Hunyuan3D export OOM-ing in WebView2), the old code rendered
  // a half-opaque dark circle with "VRM load failed" inside it — on the
  // transparent pet-companion window this looked like a black hole on
  // the user's desktop. We now render nothing at all on error; the
  // outer PetRenderer's <Suspense fallback> already mounts a PetCanvas
  // PNG underneath, so a silent fallback chain takes over visually.
  if (loadError) {
    // Silently surface to the renderer chain so PetRenderer can pick a
    // healthier renderer next time (e.g. user picks a different skin).
    // We DO NOT render the dark "VRM load failed" placeholder anymore.
    return null;
  }
