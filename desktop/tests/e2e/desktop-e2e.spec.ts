...
// Sprint Pre-launch P-2 round 2 (2026-05-23) — beforeEach ensures the main
// window is visible and React has finished mounting before each test runs.
// J3 uses Ctrl+Shift+S which toggles the main window's visibility, and
// other tests assume it stays open afterwards. Without this guard the
// spec's later tests pass but J1/J2 fail because they ran before J3 and
// the main window is in a hide()'d state from a previous spec run.
async function ensureMainPageReady() {
  if (!page) return;
  // If body is empty, try to nudge the chat panel back open via the bridge.
  const len = await page.evaluate(() => document.body?.innerText?.length || 0).catch(() => 0);
  if (len > 0) return;
  try {
    await page.evaluate(async () => {
      // @ts-expect-error — Tauri global injected by webview
      const { invoke } = await import("@tauri-apps/api/core");
      await invoke("desktop_bridge_open_chat_panel");
    });
  } catch { /* ignore */ }
  for (let i = 0; i < 15; i++) {
    const l = await page.evaluate(() => document.body?.innerText?.length || 0).catch(() => 0);
    if (l > 0) return;
    await page.waitForTimeout(200);
  }
}
test.beforeEach(ensureMainPageReady);

// Sprint Pre-launch P-2 round 2 (2026-05-23) — beforeAll does a fair bit
// (CDP connect + open chat panel + wait for React mount), so give it a
// generous timeout instead of the default 30 s. The actual happy-path is
// usually < 5 s but Windows + WebView2 cold-start can spike.
...
test.beforeAll(async () => {
  ...
  // Sprint Pre-launch P-2 round 2 (2026-05-23) — useWindowManager hides
  // the main window when no panel is open. The pet-companion overlay
  // is the only always-visible page, but it's transparent + has no
  // body text, which made these J1/J2 sanity checks flaky. Solution:
  // ask any open page to invoke `desktop_bridge_open_chat_panel`
  // which un-hides the main window so the chat panel page becomes
  // available, then prefer that page for the test session.
  ...
  if (!mainPage) {
    // Use the pet-companion (or any) page to invoke the bridge command.
    ...
    if (anyPage) {
      ...
      // Wait up to 5s for the new page to register with CDP.
  ...
  // Sprint Pre-launch P-2 round 2 (2026-05-23) — wait for React to finish
  // mounting before we hand the page to J1/J2. The chat-panel page exists
  // immediately when the window opens but its body innerText stays at 0
  // for ~1-2 s while React boots, splash unmounts, and the first chat
  // surface paints. This was making J1/J2 flaky depending on test order.
  // Use a short polling loop so we don't bump up against the 30 s hook
  // timeout when the page is already mounted.
  for (let i = 0; i < 20; i++) {
    const len = await page.evaluate(() => document.body?.innerText?.length || 0).catch(() => 0);
    if (len > 0) break;
    await page.waitForTimeout(200);
  ...
  // Diagnostic: log which page we ended up on so test failures point at the
  // right surface. This will appear in playwright stdout.
  try {
    const t = await page.title();
    // eslint-disable-next-line no-console
    console.log(`[desktop-e2e] beforeAll picked page title=${t} url=${page.url()}`);
  } catch { /* ignore */ }
});
