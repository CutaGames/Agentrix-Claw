...
export default function App() {
  ...
  // Some OEM builds (observed on Huawei/EMUI) do not reliably auto-hide the
  // native Expo splash after the first React frame. Hide it after this root
  // commits so a healthy JS runtime can never remain covered by a black splash.
  useEffect(() => {
    try {
      // eslint-disable-next-line @typescript-eslint/no-var-requires, global-require
      const nativeSplash = require('expo-splash-screen') as typeof import('expo-splash-screen');
      void nativeSplash.hideAsync().catch((error: unknown) => {
        console.warn('Failed to hide native splash screen:', error);
      });
    } catch (error) {
      console.warn('Native splash screen module unavailable:', error);
    }
  }, []);
