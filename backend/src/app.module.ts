...
// Leverage Sports Market (LSM): 杠杆滚球预测市场 — KMarket 赔率桥接 + AXP 下注 + LP 金库
import { LeverageSportsMarketModule } from './modules/leverage-sports-market/lsm.module';
...
import { DesktopLifecycleModule } from './modules/desktop-lifecycle/desktop-lifecycle.module';
import { DesktopAdminModule } from './modules/desktop-admin/desktop-admin.module';
...
// Multi-agent collaboration v1 (W1) — worktree lane backend storage
import { WorktreeLaneModule } from './modules/worktree-lane/worktree-lane.module';
// Multi-agent collaboration v1 (W2) — agent_run spawn dispatcher
import { MultiAgentModule } from './modules/multi-agent/multi-agent.module';
// Multi-agent collaboration v1 (W5) — weekly summary + CSV + snapshot cron
import { MultiAgentSummaryModule } from './modules/multi-agent-summary/multi-agent-summary.module';
// Multi-agent collaboration v2 (W8) — Pet Arena
import { PetArenaModule } from './modules/pet-arena/pet-arena.module';
// Multi-agent collaboration v2.1 P2 #16 — Enterprise SSO scaffold
import { EnterpriseSsoModule } from './modules/enterprise-sso/enterprise-sso.module';
...
import { PetEarningsModule } from './modules/pet-earnings/pet-earnings.module';
...
// Reality AI World Engine (Phase 1: AI UGC Engine)
import { WorldEngineModule } from './modules/world-engine/world-engine.module';
// AI World Creation Platform (v6): 共享世界地图 + Plot + 分层基底 + 沙箱 + 跨端任务队列
import { WorldCreationModule } from './modules/world-creation/world-creation.module';
// P-9 Companion Redesign: Trust3_Signing_Sheet queue model
import { SignRequestModule } from './modules/sign-request/sign-request.module';
// Aeon(永曜城)— 实时多人共建世界(Phase 0: 实时同步层 spike)
import { AeonModule } from './modules/aeon/aeon.module';
// Arcade — 权威实时对战(路径 A:server-authoritative game server,首发 Pong)
import { ArcadeModule } from './modules/arcade/arcade.module';
// World Creation & Feed: 统一 Creation 注册表(深合并 Aeon 地理 + v6 ECS 内容)
import { CreationModule } from './modules/creation/creation.module';
// World 参与度/竞技/变现:游戏周榜(分数权威)+ 事件预测市场(parimutuel,AXP)
import { WorldEngagementModule } from './modules/world-engagement/world-engagement.module';
// Connector(连接器/插件库)— 目录 + 一键装 + 鉴权向导 + 玩法A闭环
import { ConnectorModule } from './modules/connector/connector.module';
// P-9 wave 10: Cross-device remote control gateway
import { RemoteControlModule } from './modules/remote-control/remote-control.module';
// P-9 wave 16: Companion redesign feature flag
import { CompanionRedesignModule } from './modules/companion-redesign/companion-redesign.module';
// Soul_Companion_Onboarding: 跨端 presence(心跳上报 + 查询 + WS presence:update 推送)
...
@Module({
  imports: [
    ...
    // Leverage Sports Market (LSM): 杠杆滚球预测市场
    LeverageSportsMarketModule,
    ...
    DesktopUpdateModule,
    // Desktop go-live: crash reports + opt-in analytics (US-G2-3, US-G2-4)
    DesktopLifecycleModule,
    // Desktop GA internal beta: admin dashboard (US-G3-2)
    DesktopAdminModule,
    ...
    PlanRunnerModule,
    AgentTaskModule,
    // Multi-agent collaboration v1 (W1) — worktree lane backend
    WorktreeLaneModule,
    // Multi-agent collaboration v1 (W2) — agent_run spawn dispatcher
    MultiAgentModule,
    // Multi-agent collaboration v1 (W5) — weekly summary + CSV + snapshot
    ...
    // Reality AI World Engine (Phase 1: AI UGC Engine)
    WorldEngineModule,
    // AI World Creation Platform (v6): 世界地图 + Plot + 分层基底 + 沙箱 + 跨端任务队列
    WorldCreationModule,
    // P-9 Companion Redesign: Trust3_Signing_Sheet queue model
    SignRequestModule,
    // Aeon(永曜城)— 实时多人共建世界(Phase 0: 实时同步层 spike)
    AeonModule,
    // Arcade — 权威实时对战(路径 A:server-authoritative,首发 Pong)
    ArcadeModule,
    // World Creation & Feed: 统一 Creation 注册表(深合并 Aeon 地理 + v6 ECS 内容)
    CreationModule,
    // World 参与度/竞技/变现:游戏周榜 + 事件预测市场
    WorldEngagementModule,
    // Connector(连接器/插件库)
    ConnectorModule,
    // P-9 wave 10: Cross-device remote control gateway
    RemoteControlModule,
    // P-9 wave 16: pet_companion_redesign feature flag
    CompanionRedesignModule,
    // Soul_Companion_Onboarding: 跨端 presence(R8 心跳/查询/presence:update 推送)
    PresenceModule,
  ],
