...
export class AuthController {
  ...
  @Public()
  @UseGuards(AuthRateLimitGuard)
  @ApiOperation({ summary: '游客 token（本地试用，World Engine 首扫体验）' })
  @ApiResponse({ status: 201, description: '签发游客 token' })
  async guest(@Body() body: { deviceId?: string }) {
    return this.authService.createGuestToken(body?.deviceId || '');
  }
