...
export class AuthService {
  ...
  /**
   * 为用户自动绑定默认 3D 萌宠（灵狐 Kitsune Q版）
   * 如果用户还没有装备的皮肤，则给他装上免费的官方默认灵狐。
   */
  private async ensureUserDefaultPet(userId: string): Promise<void> {
    const KITSUNE_DEFAULT_SKIN_ID = '069c173a-a3eb-495e-aa89-234013e9a625';
    try {
      // 直接 SQL 写入避免循环依赖
      await this.userRepository.query(
        `INSERT INTO pet_active_skins (user_id, active_skin_id, created_at, updated_at)
         VALUES ($1, $2, NOW(), NOW())
         ON CONFLICT (user_id) DO NOTHING`,
        [userId, KITSUNE_DEFAULT_SKIN_ID]
      );
    } catch (error) {
      // 不阻断登录/注册流程
      this.logger.warn(`Failed to bind default pet for user ${userId}: ${error.message}`);
    }
  }
  ...
  async register(dto: RegisterDto) {
    ...
    await this.ensureUserDefaultAgent(savedUser.id);
      await this.ensureUserDefaultPet(savedUser.id);
   ...
   * 游客 token（本地试用模式）。
   *
   * 发一个不绑定正式 user 的短期 JWT, 仅用于 World Engine 首扫体验:
   * 游客可调扫描/生成接口看到 AI 角色卡, 但生成结果只在客户端本地展示, 不落库
   * (scan.controller 在 isGuest 时跳过资产创建)。登录后才把卡片真正保存为资产。
   *
   * payload.type='guest' → JwtStrategy.validate 直接放行不查库。
   * deviceId 用于配额(每设备首扫限 1 次, 由 World Engine quota 层按 sub 限流)。
   */
  createGuestToken(deviceId: string): { access_token: string; isGuest: true; expiresInDays: number } {
    const safeDeviceId = (deviceId || '').replace(/[^a-zA-Z0-9_-]/g, '').slice(0, 64) || `anon-${Date.now()}`;
    const payload = {
      sub: `guest:${safeDeviceId}`,
      type: 'guest',
      deviceId: safeDeviceId,
    };
    // 7 天有效期: 足够完成"试用→注册"转化, 又不会长期占用
    const access_token = this.jwtService.sign(payload, { expiresIn: '7d' });
    return { access_token, isGuest: true, expiresInDays: 7 };
  }
  ...
  async login(user: any) {
    ...
    await this.ensureUserDefaultAgent(user.id);
      await this.ensureUserDefaultPet(user.id);
