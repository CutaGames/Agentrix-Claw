...
export class MarketplacePetController {
  ...
  /**
   * Public leaderboard for /market/leaderboard.
   *
   *   GET /api/v1/marketplace/leaderboard?board=gmv&limit=10
   *
   * Boards: gmv | listings | active. Default gmv.
   */
  @Public()
  @Get('leaderboard')
  async leaderboard(
    ...
    const safeBoard = (board === 'listings' || board === 'active' ? board : 'gmv') as
      | 'gmv'
      | 'listings'
      | 'active';
    const safeLimit = Math.max(1, Math.min(100, Number(limit) || 10));
    const items = await this.listingService.leaderboard(safeBoard, safeLimit);
    return { board: safeBoard, items };
  }
