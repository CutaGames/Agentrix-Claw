...
const INTIMACY_GATE: Record<ProactiveKind, number> = {
  ...
  anxiety_help: 5,
  birthday: 7,
  // Phase C / C-8: missed_you fires after a long absence. Gated at lv1 so
  // brand-new users don't get a clingy pet on day one.
  missed_you: 1,
};
...
const DEDUPE_WINDOW_MS_BY_KIND: Record<ProactiveKind, number> = {
  ...
  missed_you: 24 * 60 * 60 * 1000, // 一天最多一次
};
...
export class PetCompanionEngineService {
  ...
  /**
   * Phase C / C-8 — separate "missed_you" sweep that targets users who have
   * been **away** for 24h-7d. The main `tick()` only looks at users who were
   * active in the last 24h, so we'd never reach absent users from there.
   *
   * Runs once every 6 hours. Same dedupe / quiet-hour rules apply via
   * `evaluateUser`, so a user only gets the bubble at most once per 24h
   * (DEDUPE_WINDOW_MS_BY_KIND.missed_you).
   */
  @Cron(CronExpression.EVERY_6_HOURS, { name: 'pet-missed-you-sweep' })
  async tickMissedYou(): Promise<void> {
    try {
      const now = Date.now();
      const minAbsentMs = 24 * 60 * 60 * 1000;
      const maxAbsentMs = 7 * 24 * 60 * 60 * 1000;
      const upper = new Date(now - minAbsentMs);
      const lower = new Date(now - maxAbsentMs);
      const pets = await this.petRepo
        .createQueryBuilder('p')
        .where('p.last_interaction_at IS NOT NULL')
        .andWhere('p.last_interaction_at >= :lower', { lower })
        .andWhere('p.last_interaction_at < :upper', { upper })
        .limit(500)
        .getMany();
      this.logger.log(`[proactive] missed_you sweep scanned ${pets.length} absent pets`);
      // Flip pet emotion to `sad` for absent users (max once / day) so that
      // the next time they log in the sprite already shows the right mood —
      // even if the proactive event itself is suppressed by quiet hours.
      for (const pet of pets) {
        try {
          await this.markAbsentMood(pet, now);
          await this.evaluateUser(pet, now);
        } catch (err) {
          this.logger.warn(
            `[proactive] missed_you failed user=${pet.userId}: ${(err as Error).message}`,
          );
        }
      }
    } catch (err) {
      this.logger.error(`[proactive] missed_you sweep failed: ${(err as Error).message}`);
    }
  }

  /**
   * Mood adjustment for users who have been away. Idempotent — only flips
   * once per day per user (guarded by the same 24h dedupe window as the
   * missed_you proactive event).
   */
  private async markAbsentMood(pet: LivingPet, nowMs: number): Promise<void> {
    if (!pet.lastInteractionAt) return;
    const since = Number(pet.lastInteractionAt);
    if (!Number.isFinite(since)) return;
    const absentMs = nowMs - since;
    // Skip if already sad/sleepy or recently emotion was set
    if (pet.emotion === 'sad' || pet.emotion === 'sleepy') return;
    if (absentMs < 24 * 60 * 60 * 1000) return;
    pet.emotion = 'sad';
    pet.emotionIntensity = absentMs > 3 * 24 * 60 * 60 * 1000 ? 2 : 1;
    pet.emotionSince = String(nowMs);
    pet.emotionDecayAt = String(nowMs + 12 * 60 * 60 * 1000); // soft 12 h decay
    await this.petRepo.save(pet);
    emitDesktopSyncEvent(pet.userId, 'presence:pet.state', {
      pet_id: pet.id,
      emotion: pet.emotion,
  ...
  private generateCandidates(pet: LivingPet, nowMs: number): Candidate[] {
    ...
    // Phase C / C-8 — missed_you. Triggered when user has been absent for
    // 24h-7d. Picked up by the dedicated `tickMissedYou` cron sweep above
    // because the main `tick()` only sees recently-active users.
    if (pet.lastInteractionAt) {
      ...
      if (Number.isFinite(since)) {
        ...
        if (absentMs >= minMs && absentMs <= maxMs) {
          const days = Math.max(1, Math.floor(absentMs / (24 * 60 * 60 * 1000)));
          out.push({
            kind: 'missed_you',
            // Bumped above mood_followup so a returning sad user sees the
            // warm "missed you" message before the generic mood check-in.
            priority: 6,
            payload: {
              title: `${pet.name || '我'} 想你了`,
              body:
                days === 1
                  ? '昨天没看见你，希望你今天过得不错。'
                  : `已经 ${days} 天没见到你了，回来抱抱我吧。`,
              cta: { label: '陪它一会儿', action: 'open_pet_screen' },
            },
          });
        }
      }
