...
export class LsmUnderwritingService {
  ...
  /** 获取（或首次计算并持久化）盘口承接配比。 */
  async getAllocations(marketId: string): Promise<Allocation[]> {
    const existing = await this.marketHouse.find({ where: { marketId } });
    ...
    return this.computeAndPersist(marketId);
  }

  private async computeAndPersist(marketId: string): Promise<Allocation[]> {
