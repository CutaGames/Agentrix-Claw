...
import { AssetAdapter, LSM_ASSET_ADAPTER } from './lsm-asset.adapter';
...
export class LsmOrderService {
  ...
  constructor(
    ...
    @Inject(LSM_ASSET_ADAPTER) private readonly asset: AssetAdapter,
  ...
  /** 可成交赔率（decimal）：基础 edge + 官方金库利用率动态加成。 */
  private async tradableOddsFor(
    ...
    const protocol = await this.vaultSvc.getOrCreateProtocolVault();
  ...
  async preview(input: PreviewInput): Promise<PreviewResult> {
    ...
    const tradableOdds = (await this.tradableOddsFor(input.marketId, input.outcomeIdx))!;
  ...
  async place(input: PlaceInput): Promise<LsmOrder> {
    ...
    // 承接配比（P3）：官方金库腿排末位作为余数兜底
    const allocations = await this.underwriting.getAllocations(input.marketId);
    ...
    // 1) 用户侧 escrow 保证金（AXP spend，原子；余额不足抛错）
    await this.asset.escrow(input.userId, input.stake, {
      idemKey: `lsm:stake:${input.idemKey}`,
      kind: 'lsm_open',
      metadata: { marketId: input.marketId, outcomeIdx: input.outcomeIdx },
    });
    ...
    try {
      ...
      try {
        await this.asset.release(input.userId, input.stake, {
  ...
  private async settleOne(order: LsmOrder, isWin: boolean): Promise<void> {
    ...
    if (isWin) {
      await this.asset.credit(order.userId, winPayout, {
  ...
  private async refundOne(order: LsmOrder): Promise<void> {
    ...
    await this.asset.release(order.userId, stake, {
  ...
  async cashOut(orderId: string, userId: string): Promise<LsmOrder> {
    ...
    const currentOdds = await this.tradableOddsFor(order.marketId, order.outcomeIdx);
    ...
    if (cashout > 0) {
      await this.asset.credit(order.userId, cashout, {
