...
export class LsmVaultService {
  ...
  async getOrCreateProtocolVault(): Promise<LsmVault> {
    let v = await this.vaults.findOne({ where: { singletonKey: PROTOCOL_KEY } });
    if (!v) {
      v = await this.vaults.save(
        this.vaults.create({
          kind: LsmVaultKind.PROTOCOL,
          singletonKey: PROTOCOL_KEY,
          name: '官方金库 (Protocol Vault)',
          leaderUserId: null,
  ...
  async deposit(
    ...
    // 1) 扣减用户 AXP（出资）
    await this.asset.debit(userId, amount, { idemKey, kind: 'vault_deposit' });
    ...
    try {
      ...
      // 补偿：铸份额事务失败 → 退还出资
      try {
        await this.asset.release(userId, amount, {
  ...
  async redeem(
    ...
    const result = await this.dataSource.transaction(async (manager) => {
      ...
      return { payout: r.payout, sharesBurned: shares, nav: navAt / NAV_SCALE };
    });

    // 提交后给用户入账（earn）。失败仅记录对账（罕见）。
    if (result.payout > 0) {
      try {
        await this.asset.credit(userId, result.payout, {
          idemKey,
          kind: 'vault_redeem',
        });
      } catch (e) {
  ...
  async finalizeCloseIfReady(vaultId: string): Promise<{ closed: boolean; payouts: number }> {
    ...
    // 2) 事务外给各 LP 入账（幂等键防重复）。失败仅记录对账。
    let credited = 0;
    for (const po of plan.payouts) {
      if (po.amount <= 0) continue;
      try {
        await this.asset.credit(po.userId, po.amount, {
          idemKey: `lsm:vault-close:${vaultId}:${po.userId}`,
          kind: 'vault_close',
        });
        credited += 1;
  ...
  async listVaults(kind?: LsmVaultKind): Promise<VaultView[]> {
