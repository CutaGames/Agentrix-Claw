...
export class RuntimeDoctorService {
  ...
  private checkDesktopUpdater(currentVersion: string): RuntimeDoctorCheck {
    ...
    const manifest = this.desktopUpdateService.getUpdateManifest('windows', 'x86_64', currentVersion);
    const status: RuntimeDoctorStatus = manifest ? 'pass' : required ? 'fail' : 'warn';
    ...
    return {
      ...
      status,
      message: manifest
        ? `Updater manifest is available for windows-x86_64 version ${manifest.version}.`
        : 'Updater manifest metadata is incomplete or not newer than the requested version.',
      details: {
        required,
        currentVersion,
      ...
      remediation: manifest ? undefined : 'Configure DESKTOP_UPDATE_VERSION, DESKTOP_UPDATE_BASE_URL, signed asset name, and platform signature.',
    };
  }
