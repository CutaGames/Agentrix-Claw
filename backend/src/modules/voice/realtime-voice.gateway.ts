...
export class RealtimeVoiceGateway implements OnGatewayConnection, OnGatewayDisconnect, OnModuleInit, OnModuleDestroy {
  ...
  private cleanupStaleSessions() {
    ...
    // This gateway runs on the '/voice' namespace, but `this.server` (injected
    // via @WebSocketServer) is the ROOT io Server — so `this.server.sockets` is
    // the '/' namespace and NEVER contains voice socket IDs. Using it made
    // `socketStillConnected` always false, so the sweep nuked every live voice
    // session on its next tick (≤60s), dropping the user's audio mid-turn and
    // surfacing as "transcription timeout". Resolve the '/voice' namespace
    // explicitly; if it can't be resolved, assume the socket is connected so
    // the sweep can never kill a live session (true disconnects are handled by
    // handleDisconnect; the idle timeout below is the real backstop).
    const voiceNs = (this.server as any)?.of?.('/voice');
    const nsSockets = voiceNs?.sockets;

    for (const [sessionId, session] of this.sessions.entries()) {
      let socketStillConnected = true;
      if (nsSockets) {
        socketStillConnected =
          typeof nsSockets.has === 'function'
            ? nsSockets.has(session.socketId)
            : Boolean(typeof nsSockets.get === 'function' ? nsSockets.get(session.socketId) : true);
      }
  ...
  async handleConnection(client: AuthenticatedVoiceSocket) {
    try {
      ...
      // Diagnostic: capture WHY a voice socket drops (transport close, ping
      // timeout, client namespace disconnect, etc.) — helps distinguish a
      // client-initiated teardown from a network/keepalive failure.
      client.on('disconnect', (reason: string) => {
        this.logger.warn(`Voice socket ${client.id} disconnect reason: ${reason}`);
      });
    } catch (error) {
  ...
  async handleSessionStart(
    ...
    // Realtime voice default path is AWS cascade (proven, has quota). Gemini Live is an
    // OPT-IN premium enhancement only — enabled via VOICE_ENABLE_GEMINI (default off).
    // Platform free/paid Gemini keys are rate-limited/leaked, so defaulting to them
    // would make every session try a dead key first and degrade to cascade after a failure.
    const geminiLiveOptIn = process.env.VOICE_ENABLE_GEMINI === 'true';
    const hasCustomProvider = await this.providerConfigRepo.count({ where: { userId: client.userId, isActive: true } }) > 0;
    if (geminiLiveOptIn && !hasCustomProvider && this.geminiAdapter.isAvailable && data.duplexMode !== false) {
      try {
  ...
  handleAudioChunk(
    ...
    // Diagnostic: confirm audio actually reaches the server (first chunk + every
    // 50th) so we can tell "client never sent audio" from "STT didn't transcribe".
    const dbg = session as unknown as { _dbgChunks?: number };
    dbg._dbgChunks = (dbg._dbgChunks || 0) + 1;
    if (dbg._dbgChunks === 1 || dbg._dbgChunks % 50 === 0) {
      this.logger.log(
        `voice:audio:chunk #${dbg._dbgChunks} for ${data.sessionId} (${chunk.length}B, strategy=${session.v2Strategy || 'legacy'})`,
      );
    }
  ...
  async handleAudioEnd(
    ...
    const dbgEnd = session as unknown as { _dbgChunks?: number };
    this.logger.log(
      `voice:audio:end for ${data.sessionId} after ${dbgEnd._dbgChunks || 0} chunks (strategy=${session.v2Strategy || 'legacy'})`,
    );
  ...
  private buildStrategyCallbacks(
    ...
    return {
      ...
      onTranscriptFinal: (sessionId, text, lang, provider) => {
        this.logger.log(
          `voice:stt:final for ${sessionId} via ${provider || '?'}: "${(text || '').slice(0, 60)}" (${(text || '').length} chars)`,
        );
        client.emit('voice:stt:final', {
          sessionId,
          transcript: text,
