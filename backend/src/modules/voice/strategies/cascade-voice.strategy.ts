...
import { AwsTranscribeSTTAdapter } from '../adapters/aws-transcribe-stt.adapter';
import { edgeTTS, resolveEdgeVoice } from '../adapters/edge-tts.adapter';
import type { StreamingSTTAdapter, StreamingSTTSession } from '../adapters/voice-provider.interface';
...
interface CascadeSession {
  ...
  /** Duplex utterance aggregation: buffer finals until the user pauses long
   *  enough, so a natural mid-sentence pause doesn't trigger a premature reply. */
...
/** How long to wait (ms) after the last final transcript before treating the
 *  duplex turn as complete and sending it to the agent. Gives the user room to
 *  pause mid-thought without getting cut off. Override via VOICE_TURN_DEBOUNCE_MS. */
const DUPLEX_TURN_DEBOUNCE_MS = Number(process.env.VOICE_TURN_DEBOUNCE_MS || 2500);
...
export class CascadeVoiceStrategy implements IVoiceStreamStrategy {
  ...
  private readonly awsAdapter = new AwsTranscribeSTTAdapter();
  ...
  interrupt(sessionId: string): void {
    ...
    if (session.utteranceTimer) {
      clearTimeout(session.utteranceTimer);
      session.utteranceTimer = null;
  ...
  /**
   * Resolve the ordered list of streaming STT adapters to try.
   *
   * Controlled by env `VOICE_REALTIME_STT` (case-insensitive):
   *   - `aws`      → AWS Transcribe first, Deepgram fallback
   *   - `deepgram` → Deepgram first, AWS fallback
   *   - unset/other → prefer AWS when its creds are present (reliable paid
   *     provider the product owner wants), otherwise Deepgram.
   *
   * Only adapters reporting `isAvailable` are returned, preserving order.
   */
  private resolveStreamingOrder(): StreamingSTTAdapter[] {
    const pref = (process.env.VOICE_REALTIME_STT || '').trim().toLowerCase();

    let ordered: StreamingSTTAdapter[];
    if (pref === 'deepgram') {
      ordered = [this.deepgramAdapter, this.awsAdapter];
    } else if (pref === 'aws') {
      ordered = [this.awsAdapter, this.deepgramAdapter];
    } else {
      // Default: prefer AWS (paid/reliable) when configured, else Deepgram.
      ordered = this.awsAdapter.isAvailable
        ? [this.awsAdapter, this.deepgramAdapter]
        : [this.deepgramAdapter, this.awsAdapter];
    }

    return ordered.filter((adapter) => adapter.isAvailable);
  }

  private async initializeStreamingSTT(
    session: CascadeSession,
    excludeNames: Set<string> = new Set<string>(),
  ): Promise<void> {
    const adapters = this.resolveStreamingOrder().filter((a) => !excludeNames.has(a.name));
    if (adapters.length === 0) {
      this.logger.warn(`Streaming STT unavailable for ${session.options.sessionId} (no provider configured)`);
      return;
    }
    ...
    for (const adapter of adapters) {
      try {
        const streamingSession = await adapter.createStreamingSession(
          { lang: session.options.lang, encoding: 'linear16', sampleRate: 16000 },
          this.buildStreamingCallbacks(session, token, adapter.name, excludeNames),
        );
        session.streamingSession = streamingSession;
        this.logger.log(
          `Realtime STT provider for ${session.options.sessionId}: ${adapter.name}`,
        );
        return;
      } catch (error: any) {
        lastError = error instanceof Error ? error : new Error(String(error?.message || error));
        this.logger.warn(
          `Streaming STT provider ${adapter.name} failed to open for ${session.options.sessionId}: ${lastError.message}`,
        );
        // Try the next available provider (runtime fallback).
      }
    }
    ...
    session.activeStreamingToken = null;
    this.logger.warn(
      `All streaming STT providers failed for ${session.options.sessionId}: ${lastError?.message || 'unknown error'}`,
    );
  }

  /** Schedule (or reschedule) the duplex turn-completion flush — sends the
   *  aggregated utterance to the agent after a pause, so natural mid-sentence
   *  pauses don't trigger a premature reply. */
  private scheduleUtteranceFlush(session: CascadeSession): void {
    if (session.utteranceTimer) {
      clearTimeout(session.utteranceTimer);
    }
    session.utteranceTimer = setTimeout(() => {
      session.utteranceTimer = null;
      const utterance = session.pendingUtterance.trim();
      session.pendingUtterance = '';
      if (!utterance) return;
      // Skip filler-only turns (backchannel "嗯/啊/哦…" or punctuation) so an
      // acknowledgement during the agent's reply doesn't fire a junk turn that
      // looks like the assistant answering off-topic.
      const stripped = utterance.replace(/[\s。，,.!?！？、…·~～:：;；"'""'']+/g, '');
      const FILLER = new Set([
        '嗯', '嗯嗯', '嗯嗯嗯', '嗯哼', '啊', '呃', '哦', '哦哦', '哦哦哦', '噢', '额', '唉', '哎', '哎呀',
        'um', 'umm', 'uh', 'uhh', 'en', 'hmm', 'mm', 'ok', 'okay',
      ]);
      if (stripped.length === 0 || FILLER.has(stripped.toLowerCase())) {
        this.logger.debug(`Skipping filler-only utterance: "${utterance}"`);
        return;
      }
      void this.startAgentResponse(session, utterance);
    }, DUPLEX_TURN_DEBOUNCE_MS);
  }
  ...
  private buildStreamingCallbacks(
    ...
    return {
      onInterim: (transcript: string) => {
        ...
        // Interim results mean the user is still talking — if a turn-completion
        // timer is already pending, push it back so we don't cut them off.
        if (session.options.duplexMode && transcript?.trim() && session.utteranceTimer) {
          this.scheduleUtteranceFlush(session);
        }
      },
      onFinal: (result: { text?: string; lang?: string; provider: string }) => {
        ...
        // Aggregate consecutive finals into a single utterance and wait for a
        // real pause (DUPLEX_TURN_DEBOUNCE_MS) before sending to the agent.
        // AWS/Deepgram emit a "final" at every natural pause, which previously
        // fired the reply before the user finished their sentence.
        const now = Date.now();
        const isDupeFinal = text === session.lastFinalTranscript && now - session.lastFinalAt < 1500;
        if (!isDupeFinal) {
      ...
      onError: (error: Error) => {
        this.logger.warn(`Streaming STT error (${providerName}): ${error?.message}`);

        // Seamless failover: if another STT provider is still available, switch
        // to it for this session and replay the current turn's buffered audio so
        // the user's words aren't lost. This keeps AWS as the paid primary while
        // making its intermittent event-stream deserialization errors invisible
        // to the user (we fall through to Deepgram automatically).
        const nextExclude = new Set(excludeNames);
        nextExclude.add(providerName);
        const remaining = this.resolveStreamingOrder().filter((a) => !nextExclude.has(a.name));

        if (session.activeStreamingToken === token && remaining.length > 0) {
          const buffered = [...session.audioChunks];
          this.logger.warn(
            `Failing over realtime STT ${providerName} → ${remaining[0].name} for ` +
            `${session.options.sessionId} (replaying ${buffered.length} buffered chunks)`,
          );
          session.streamingSession = null;
          void this.initializeStreamingSTT(session, nextExclude).then(() => {
            const s = session.streamingSession;
            if (s) {
              for (const chunk of buffered) s.write(chunk);
            }
          });
          return;
        }

        // No fallback provider left — surface the error.
        session.streamingSession = null;
        this.resolvePendingTurn(session, token, false);
