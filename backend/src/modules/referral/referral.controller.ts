...
export class ReferralController {
  constructor(
    ...
    private readonly flywheelService: ReferralFlywheelService,
  ) {}
  ...
  @ApiBearerAuth()
  @ApiOperation({ summary: '新用户注册后归因并发放双边拉新奖励（前端注册成功后调用，带 ref）' })
  async trackSignup(
    @Request() req: any,
    @Body() body: { ref: string; channel?: string },
  ) {
    const inviteeUserId = req.user?.userId || req.user?.sub || req.user?.id;
    return this.flywheelService.onSignup(inviteeUserId, body?.ref, body?.channel);
  }

  @Get('my-flywheel')
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth()
  @ApiOperation({ summary: '我的拉新战绩（邀请数/已奖励/GMV 返佣累计 AXP）' })
  async myFlywheel(@Request() req: any) {
    const userId = req.user?.userId || req.user?.sub || req.user?.id;
    return this.flywheelService.getMyFlywheel(userId);
  }
