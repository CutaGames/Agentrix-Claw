...
export class SkillService {
  ...
  /**
   * Upsert a published `Skill` row from an OpenClaw Hub skill object, keyed by
   * `externalSkillId`. Hub skills usually have NO marketplace Skill row, which
   * previously made hub installs un-recordable (Bug: 装了显示成功但"我的技能"为空)。
   * Returns the local Skill.id to use as the install FK.
   */
  async ensureHubSkillRow(hub: {
    id: string;
    key?: string;
    name: string;
    displayName?: string;
    ...
    const externalId = hub.id || hub.key || hub.name;
    const existing = await this.skillRepository.findOne({ where: { externalSkillId: externalId } });
    if (existing) return existing;

    const row = this.skillRepository.create({
      name: (hub.name || hub.key || 'hub-skill').slice(0, 100),
      displayName: (hub.displayName || hub.name || '技能').slice(0, 200),
      description: (hub.description || hub.displayName || hub.name || 'OpenClaw Hub skill').slice(0, 500),
      version: hub.version || '1.0.0',
      // Hub skills are imported from the OpenClaw community catalog.
      source: 'imported' as any,
      originalPlatform: 'openclaw' as any,
      // Must be PUBLISHED so installSkillForInstance() accepts it.
      status: SkillStatus.PUBLISHED,
      inputSchema: { type: 'object', properties: {}, required: [] } as any,
      executor: { type: 'mcp', mcpServer: 'openclaw-hub' } as any,
      tags: hub.tags ?? [],
      externalSkillId: externalId,
      imageUrl: hub.imageUrl,
      thumbnailUrl: hub.thumbnailUrl,
      metadata: { hubSkillId: hub.id, hubKey: hub.key },
    });
    return this.skillRepository.save(row);
  }

  /**
   * Record a hub-skill install for a user keyed to an instance, persisting an
   * Agentrix-side `claw_installed_skill` row so it shows up in "我的技能"
   * regardless of whether the target claw is platform-hosted or self-hosted.
   * Idempotent (skips if already installed for that instance+skill).
   */
  async recordUserHubInstall(params: {
    hub: Parameters<SkillService['ensureHubSkillRow']>[0];
    instanceId: string;
    userId: string;
  }): Promise<ClawInstalledSkill> {
    const skill = await this.ensureHubSkillRow(params.hub);
    const existing = await this.clawInstalledSkillRepository.findOne({
      where: { instanceId: params.instanceId, skillId: skill.id },
    });
    if (existing) {
      // Backfill the installer if it was missing (older rows).
      if (!existing.installedByUserId) {
        existing.installedByUserId = params.userId;
        return this.clawInstalledSkillRepository.save(existing);
      }
      return existing;
    }
    const row = this.clawInstalledSkillRepository.create({
      instanceId: params.instanceId,
      installedByUserId: params.userId,
      skillId: skill.id,
      isEnabled: true,
    });
    return this.clawInstalledSkillRepository.save(row);
  }
   ...
   * 获取用户已安装的技能
   *
   * 2026-05-31 fix: previously this ONLY read `user_installed_skill` (userId
   * scope). But the mobile install path (`installSkillToInstance`) usually
   * lands the row in `claw_installed_skill` (instanceId scope) or via the
   * OpenClaw proxy — so a freshly "installed" skill never showed up in the
   * "我的技能" list (looked like the skill was lost). We now UNION both
   * sources keyed to the logged-in user (`claw_installed_skill.installedByUserId`)
   * and dedupe by skillId so a skill installed through ANY surface appears.
   */
  async findUserInstalledSkills(userId: string, page = 1, limit = 20): Promise<{
    ...
    const [userInstallations, clawInstallations] = await Promise.all([
      this.userInstalledSkillRepository.find({
        where: { userId },
        relations: ['skill'],
        order: { installedAt: 'DESC' },
      }),
      // Skills the user installed onto any of their claws/pets.
      this.clawInstalledSkillRepository.find({
        where: { installedByUserId: userId },
        relations: ['skill'],
        order: { installedAt: 'DESC' },
      }).catch(() => [] as ClawInstalledSkill[]),
    ]);

    // Merge + dedupe by skillId (user-scope row wins; it's the canonical
    // "owned" record). Skip rows whose skill relation failed to load.
    const bySkillId = new Map<string, (Skill & { isEnabled: boolean; config?: Record<string, any>; installedAt: Date })>();
    const add = (inst: { skill: Skill | null; skillId: string; isEnabled: boolean; config?: Record<string, any>; installedAt: Date }) => {
      if (!inst.skill) return;
      if (bySkillId.has(inst.skillId)) return;
      bySkillId.set(inst.skillId, {
        ...inst.skill,
    ...
    for (const inst of userInstallations) add(inst as any);
    for (const inst of clawInstallations) add(inst as any);

    const merged = Array.from(bySkillId.values()).sort(
