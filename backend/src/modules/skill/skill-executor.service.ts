...
import { evaluateToolCall } from '../agent-account/capability-gate';
...
export interface ExecutionContext {
  ...
  /**
   * 能力门控(G 组)— 当本次执行代表某个 Agent 时,由调用方(如 openclaw-proxy)
   * 从 **单一权威来源** `AgentAccount.capabilities` 解析后透传的已声明工具集。
   * 一旦提供(非 undefined),执行层即按「声明即门控」deny-by-default 拒绝未声明工具;
   * 未提供时(系统/非 Agent 内部调用)不启用门控,保持向后兼容。
   */
  agentCapabilities?: string[];
  /** 触发本次执行的 Agent 账户 id(审计 / 错误信息用)。 */
  agentId?: string;
}
...
export class SkillExecutorService {
  ...
  async execute(skillId: string, params: any, context: ExecutionContext = {}): Promise<ExecutionResult> {
    ...
    try {
      ...
      // 能力门控(G 组,7.22):代表 Agent 执行时,以 AgentAccount.capabilities
      // (单一权威来源,经 context 透传)做 deny-by-default 校验,未声明即拒。
      this.assertAgentToolDeclared(context, skill.name);
  ...
  private async executeHttpSkill(skill: Skill, params: any, context: ExecutionContext): Promise<any> {
    ...
    const response = await axios({
      ...
      url: endpoint,
      // GET/DELETE 走 query;其余走 body。带鉴权头(executor.headers,#2 ② 导入时烘焙)随请求发出。
      ...(isBodyless ? { params } : { data: params }),
      headers: {
  ...
  async executeInternal(handlerName: string, params: any, context: ExecutionContext): Promise<any> {
    // 能力门控(G 组,7.22):代表 Agent 执行内部工具/技能时,同样从单一权威来源
    // (AgentAccount.capabilities)做 deny-by-default 校验。
    this.assertAgentToolDeclared(context, handlerName);
  ...
  /**
   * 能力门控(G 组)— 执行层 deny-by-default 校验。
   *
   * 仅当 `context.agentCapabilities` 已提供(本次执行代表某 Agent)时启用门控:
   * 工具名未命中 `AgentAccount.capabilities`(单一权威来源)即抛 `ForbiddenException`。
   * 未提供时(系统/非 Agent 内部调用)不拦截,保持既有调用方向后兼容。
   */
  private assertAgentToolDeclared(context: ExecutionContext, toolName: string): void {
    if (!context || context.agentCapabilities === undefined) {
      return;
    }
    const result = evaluateToolCall(context.agentCapabilities, toolName);
    if (result.allowed) {
      return;
    }
    this.logger.warn(
      `Capability gate denied: agent=${context.agentId || 'unknown'} tool=${result.denial?.tool} (not in AgentAccount.capabilities)`,
    );
  ...
  private registerDefaultHandlers(): void {
    ...
    // ────────────────────────────────────────────────────────
    // Connector(连接器/插件库)Tools — 玩法 A:派 agent 办真事
    // ────────────────────────────────────────────────────────

    /**
     * connector_run — 用一个 builtin 连接器办一件事(查询类:加密行情/天气…),只返回结果不发奖励。
     * Params: { connectorId: string, ...args }
     */
    this.registerHandler('connector_run', async (params, _context) => {
      const { connectorId, ...args } = params || {};
      if (!connectorId) throw new BadRequestException('connectorId is required for connector_run');
      const r = await this.connectorService.runBuiltin(connectorId, args);
      return { success: true, connectorId, summary: r.summary, data: r.data };
    });

    /**
     * connector_errand — 派 agent 出门办一件真事(玩法 A)。办成后发 aeon_reality_reward AXP +
     * 写世界新闻("对话里派 agent 办成真事 → 游戏里产出 AXP")。
     * Params: { connectorId: string, ...args }
     */
    this.registerHandler('connector_errand', async (params, context) => {
      const { connectorId, ...args } = params || {};
      if (!connectorId) throw new BadRequestException('connectorId is required for connector_errand');
      if (!context.userId) throw new ForbiddenException('User context required to send an agent errand');
      const agentName = typeof context.metadata?.instanceName === 'string' ? context.metadata.instanceName : undefined;
      return this.connectorService.runErrand(context.userId, connectorId, args, agentName);
    });
    ...
    /**
     * creator_studio — Unified creative task tool. Handles poster generation,
     * PPT creation, multi-form pet variants, skin marketplace listing, NFT minting,
     * and photo/scan-to-3D. Routes to the appropriate device (desktop/cloud/mobile)
     * based on action type and device availability.
     */
    this.registerHandler('creator_studio', async (params, context) => {
      return this.creatorStudioService.executeTool(params || {}, context);
    });
