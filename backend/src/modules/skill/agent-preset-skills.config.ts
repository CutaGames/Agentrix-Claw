...
export const AGENT_PRESET_SKILLS: PresetSkill[] = [
  ...
  // ─── Connector(连接器/插件库)— 派 agent 办真事 ───
  {
    handlerName: 'connector_run',
    displayName: 'Connector Run',
    description: 'Use an installed connector to fetch real info (crypto prices, weather, etc.). Provide connectorId (e.g. "crypto-price", "weather") plus query args. Read-only, no reward.',
    category: 'tools',
    enabledByDefault: true,
    icon: '🔌',
  },
  {
    handlerName: 'connector_errand',
    displayName: 'Agent Errand',
    description: 'Send the user\'s agent on a real-world errand via a connector (e.g. check crypto price / weather). On success the agent earns AXP in Aeon and a city-news entry is posted. Provide connectorId + args. Use when the user asks the agent to "go do" a real task.',
    category: 'tools',
    enabledByDefault: true,
    icon: '🏃',
  },
