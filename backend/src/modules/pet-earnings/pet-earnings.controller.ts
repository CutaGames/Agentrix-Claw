...
export class PetEarningsController {
  constructor(
    ...
    private readonly autoEarn: PetAutoEarnService,
  ) {}
  ...
  // ── 半自主接活（需求 6）──
  @Get('opportunities')
  async opportunities(@Req() req: any, @Query('limit') limit?: string) {
    return this.autoEarn.listOpportunities(this.uid(req), limit ? Number(limit) : 20);
  }

  @Post('opportunities/:id/accept')
  async acceptOpportunity(@Req() req: any, @Param('id') id: string) {
    return this.autoEarn.acceptOpportunity(this.uid(req), id);
  }

  /** 飞轮指标（需求 7，admin only）。 */
  @Get('admin/metrics')
  @UseGuards(JwtAuthGuard, AdminGuard)
  async flywheelMetrics() {
    return this.earnings.getFlywheelMetrics();
  }
}
