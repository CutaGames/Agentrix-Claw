...
export class AgentAccountController {
  ...
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
    summary: '签发 Agent 密钥对 + API secret（私钥/secret 仅返回一次）',
  })
  @ApiParam({ name: 'id', description: 'Agent 账户 ID' })
  @ApiResponse({
    status: 200,
    description: '返回密钥对与 API secret（明文仅显示一次，后端只存哈希与公钥）',
  })
  async issueKeys(
    @Param('id') id: string,
    @Body('rotate') rotate?: boolean,
  ) {
    const result = await this.agentKeyService.issueKeys(id, {
      allowRotate: !!rotate,
    });
    return {
      success: true,
      data: result,
      message: '请立即保存私钥与 API secret，关闭后将无法再次查看。',
    };
  }
  ...
  @Get(':id/economic-status')
  @ApiOperation({ summary: '查询 Agent 经济身份各维度真实状态(H 组)' })
  @ApiParam({ name: 'id', description: 'Agent 账户 ID' })
  @ApiResponse({
    status: 200,
    description: '返回钱包/限额/信用/链上身份/能力的真实状态枚举(enabled/not_enabled/failed),与后端字段一致、无空占位',
  })
  async getEconomicStatus(@Param('id') id: string) {
    const result = await this.agentAccountService.getEconomicIdentityStatus(id);
    return {
      success: true,
      data: result,
    };
