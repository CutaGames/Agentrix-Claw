...
Execute this step now. Use the appropriate tool if specified. Report the result concisely.`;
   ...
   *
   * Same UUID guard as getContextUsage: client may pass a non-uuid
   * sessionId for anonymous / unauthenticated flows; we no-op in that
   * case to avoid Postgres "invalid input syntax for type uuid".
   */
   ...
   *
   * v1 client uses non-uuid sessionId format (`session-${timestamp}`) for
   * unauthenticated / anonymous flows. For those we short-circuit return
   * an empty usage block instead of letting Postgres throw "invalid input
   * syntax for type uuid". The error was surfacing to the chat surface as
   * repeating "Session not found" red rows.
   */
  async getContextUsage(sessionId: string, instanceId: string): Promise<{
    ...
    // UUID v1-v5 regex (8-4-4-4-12 hex). Loose enough for Postgres
    // gen_random_uuid() output. Anything else → empty result, no DB hit.
    const UUID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
  ...
  /**
   * Permanently delete a session (cascade messages + memories).
   *
   * v0.7.19 — added so the desktop chat history 🗑 button actually removes
   * the conversation from backend storage instead of only clearing the local
   * cache (which left the server-side session forever).
   *
   * Accepts either the row PK uuid OR the client-side `sessionId` string;
   * both lookups are scoped to the calling user so a stolen session id
   * cannot be deleted by another account.
   */
  async deleteSession(userId: string, sessionRef: string): Promise<{ ok: boolean; deletedMessages: number }> {
    const session = await this.sessionRepo.findOne({
      where: [
        { id: sessionRef, userId },
        { sessionId: sessionRef, userId },
    ...
    if (!session) return { ok: false, deletedMessages: 0 };

    // Delete child rows first (no DB cascade configured on these FKs).
    let deletedMessages = 0;
    try {
      const r = await this.messageRepo.delete({ sessionId: session.id });
      deletedMessages = r.affected ?? 0;
    } catch (e: any) {
      this.logger.warn(`deleteSession: message cleanup failed: ${e?.message || e}`);
    }
    try {
      await this.memoryRepo.delete({ sessionId: session.id });
    } catch (e: any) {
      this.logger.warn(`deleteSession: memory cleanup failed: ${e?.message || e}`);
    }

    await this.sessionRepo.delete({ id: session.id });
    this.activePlans.delete(session.sessionId);

    this.logger.log(`session deleted: ${session.id} (${deletedMessages} messages)`);
    return { ok: true, deletedMessages };
  }
