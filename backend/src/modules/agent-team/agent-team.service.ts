...
export interface ProvisionTeamDto {
  /**
   * Either `templateId` (uuid) OR `templateSlug` (human-readable slug
   * like 'agentrix-11'). One of them is required.
   * v1 W1 client uses slug; legacy callers use id.
   */
  templateId?: string;
  templateSlug?: string;
...
export class AgentTeamService {
  ...
  constructor(
    ...
    private readonly petTeamMemberRepo: Repository<PetTeamMember>,
  ) {}
  ...
  /**
   * 按 templateId(uuid)或 templateSlug 解析模板(provision 前置读取,用于规模/角色校验)。
   * 供 TeamProductizationService(任务 24)在 provisionTeam 前做白名单 + 1–20 规模校验。
   */
  async getTemplateForProvision(
    templateId?: string,
    templateSlug?: string,
    ...
    const where = templateId ? { id: templateId } : { slug: templateSlug };
  ...
  async provisionTeam(
    ...
    // v1: accept either templateId (uuid) or templateSlug — client uses slug
    if (!dto.templateId && !dto.templateSlug) {
      throw new BadRequestException('templateId or templateSlug required');
    }
    const where = dto.templateId
      ? { id: dto.templateId }
  ...
  // ─────────────────────────────────────────────────────────────────────
  // Multi-Agent v1 W3 — Pet bridge: living pet ↔ AgentAccount
  // ─────────────────────────────────────────────────────────────────────

  /**
   * Bind a list of LivingPets to AgentTeam membership. Each pet gets a
   * dedicated AgentAccount (owner = user) plus a PetTeamMember row so
   * `selectMember` can dispatch sub-tasks to it.
   *
   * Idempotent: pets already bound (living_pets.bound_agent_account_id
   * non-null) are skipped silently.
   *
   * Spec: design.md §11.1, §11.2; tasks.md W3.2
   */
  async bindLivingPets(
    ownerId: string,
    livingPetIds: string[],
  ): Promise<{ bound: number; skipped: number; members: PetTeamMember[] }> {
    if (!ownerId) throw new BadRequestException('ownerId required');
    if (!Array.isArray(livingPetIds) || livingPetIds.length === 0) {
      throw new BadRequestException('livingPetIds must be a non-empty array');
    }

    // W3.5 sentinel — `["*"]` means "bind all my pets"
    const allPetsRequested = livingPetIds.length === 1 && livingPetIds[0] === '*';
    const pets = allPetsRequested
      ? await this.livingPetRepo.find({ where: { userId: ownerId } })
      : await this.livingPetRepo.find({
          where: { id: In(livingPetIds), userId: ownerId },
        });
    if (pets.length === 0) {
      throw new NotFoundException('No matching pets found for this user');
    }

    const members: PetTeamMember[] = [];
    let bound = 0;
    let skipped = 0;
    ...
    for (const pet of pets) {
      ...
      // Derive role from soul template if available; fallback to 'general'.
      // Mapping: high_arpu → 'coder' / high_dau → 'researcher' / edu →
      // 'tutor' / viral → 'media' / web3 → 'treasury' / family → 'general'
      const role = this.deriveRoleFromSoulTemplate(pet.soulTemplateId);
      const displayName = pet.name || `Pet ${pet.id.slice(0, 8)}`;

      // 1. Create AgentAccount for the pet
      const agentUniqueId = `PET-AGT-${Date.now()}-${crypto.randomBytes(4).toString('hex')}`;
      const agent = this.agentRepo.create({
        agentUniqueId,
        name: displayName,
        description: `${displayName} 作为 AgentTeam 成员`,
        ownerId,
        agentType: AgentType.PERSONAL,
        status: AgentAccountStatus.ACTIVE,
        creditScore: 500,
        ...
        metadata: {
          ...
          livingPetId: pet.id,
          role,
          soulTemplateId: pet.soulTemplateId ?? null,
        },
      } as Partial<AgentAccount>);
      const savedAgent = await this.agentRepo.save(agent) as AgentAccount;

      // 2. Create PetTeamMember row (denormalized bound_agent_account_id
      //    for fast `selectMember` dispatch)
      const member = this.petTeamMemberRepo.create({
        parentLivingPetId: pet.id,
        userId: ownerId,
        role: role as PetTeamRole,
        soulTemplateId: pet.soulTemplateId ?? '',
        displayName,
        scope: {},
        dailyBudgetUsd: '2.00',
        walletAddress: null,
        status: 'active',
        boundAgentAccountId: savedAgent.id,
      });
      const savedMember = await this.petTeamMemberRepo.save(member);

      // 3. Stamp back-reference on LivingPet
      pet.boundAgentAccountId = savedAgent.id;
    ...
    this.logger.log(
      `bindLivingPets owner=${ownerId} requested=${livingPetIds.length} bound=${bound} skipped=${skipped}`,
    );

    return { bound, skipped, members };
  }

  /**
   * Soft-unbind a pet member. AgentAccount is preserved (revoked status).
   * LivingPet.bound_agent_account_id cleared, PetTeamMember status →
   * 'revoked'. History (agent_tasks, cost records) is preserved.
   *
   * Spec: design.md §11.5; tasks.md W3.9
   */
  async unbindLivingPet(
    ownerId: string,
    livingPetId: string,
  ): Promise<{ ok: true }> {
    const pet = await this.livingPetRepo.findOne({
      where: { id: livingPetId, userId: ownerId },
    });
    ...
    // Soft-revoke the PetTeamMember
    await this.petTeamMemberRepo.update(
      { parentLivingPetId: pet.id, userId: ownerId, status: 'active' },
      { status: 'revoked' },
    );

    // Clear back-reference but keep AgentAccount + cost history
    const agentAccountId = pet.boundAgentAccountId;
    pet.boundAgentAccountId = null;
    await this.livingPetRepo.save(pet);

    // Mark AgentAccount as revoked but preserve row for history
    await this.agentRepo.update(
      { id: agentAccountId },
      { status: AgentAccountStatus.REVOKED },
    );

    this.logger.log(
      `unbindLivingPet owner=${ownerId} pet=${livingPetId} agent=${agentAccountId} (preserved)`,
    );
    return { ok: true };
  }

  /**
   * Map soul template tier → AgentTeam role tag.
   * Conservative default 'general' for unknowns / NULL soul.
   * Spec: design.md §11.2
   */
  private deriveRoleFromSoulTemplate(soulTemplateId?: string | null): string {
    if (!soulTemplateId) return 'general';
    // The mapping is intentionally hard-coded for v1 (spec §11.2). When
    // pet-soul-templates are extended with explicit role tags we'll
    // promote this to a JOIN.
    const lower = soulTemplateId.toLowerCase();
    if (lower.includes('claw') || lower.includes('coder') || lower.includes('dev')) return 'coder';
    if (lower.includes('research') || lower.includes('learn')) return 'researcher';
    if (lower.includes('media') || lower.includes('viral')) return 'media';
    if (lower.includes('treasury') || lower.includes('web3')) return 'treasury';
    if (lower.includes('tutor') || lower.includes('edu')) return 'tutor';
    if (lower.includes('ops') || lower.includes('qa')) return 'qa_ops';
    return 'general';
  }
}
