...
export class AgentTeamController {
  ...
  // ─────────────────────────────────────────────────────────────────────
  // Multi-Agent v1 W3 — Pet bridge endpoints (LivingPet ↔ AgentAccount)
  // ─────────────────────────────────────────────────────────────────────

  @Post('bind-pets')
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: '将 LivingPet 绑定为 AgentTeam 成员 (W3.2)' })
  @ApiResponse({ status: 200, description: '绑定成功,返回新的 PetTeamMember 列表' })
  async bindPets(
    @Request() req,
    @Body() body: { livingPetIds: string[] },
  ) {
    const result = await this.agentTeamService.bindLivingPets(
      req.user.id,
      body.livingPetIds,
    );
    return {
      ...
      message:
        result.bound > 0
          ? `已将 ${result.bound} 只宠物绑定为团队成员。`
          : '所选宠物已经是团队成员,无新增。',
    };
  }

  @Post('unbind-pet/:livingPetId')
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: '解除 LivingPet 与 AgentAccount 的绑定 (W3.9)' })
  @ApiResponse({ status: 200, description: '解绑成功 (AgentAccount + 历史保留)' })
  async unbindPet(
    @Request() req,
    @Param('livingPetId') livingPetId: string,
  ) {
    const result = await this.agentTeamService.unbindLivingPet(
      req.user.id,
      livingPetId,
    );
    return {
