...
/** Options for {@link BedrockIntegrationService.generateImage}. */
export interface BedrockImageOptions {
  /** BYO AWS creds (same Bedrock account as the user's Sonnet); omit to use platform default chain. */
  userCredentials?: BedrockUserCredentials;
  /** Override region (image models are region-gated; defaults to creds/env region). */
  region?: string;
  /** Negative prompt (defaults applied). */
  negativePrompt?: string;
  /** Output size (portrait default 768x1152 for mobile vertical). */
  width?: number;
  height?: number;
  seed?: number;
}
...
export class BedrockIntegrationService {
  ...
  private static readonly MODEL_ID_REMAP: Record<string, string> = {
    ...
    'us.anthropic.claude-opus-4-8-v1:0':          'us.anthropic.claude-opus-4-1-20250805-v1:0',
    'us.anthropic.claude-opus-4-8-20260601-v1:0': 'us.anthropic.claude-opus-4-1-20250805-v1:0',
  ...
  private resolveModelId(modelId: string): string {
    ...
    // 4.7 / 4.8 are aspirational — alias to real Opus 4 until AWS hosts them.
    if (modelId.includes('opus-4-8') || modelId.includes('opus-4.8')) return 'us.anthropic.claude-opus-4-1-20250805-v1:0';
  ...
  /**
   * Generate an image via Bedrock (Amazon Titan Image Generator → Stability fallback).
   *
   * NOTE: Claude/Sonnet are text/vision models and CANNOT generate images — image
   * generation on Bedrock uses Titan Image Generator or Stability models. We reuse the
   * caller's AWS Bedrock credentials (same account as their BYO Sonnet) but route to an
   * image model id. Returns a base64-encoded PNG. Throws if no image model is accessible
   * in the region (caller should degrade gracefully).
   */
  async generateImage(prompt: string, opts: BedrockImageOptions = {}): Promise<string> {
    const region = this.normalizeRegion(
      opts.userCredentials?.region ||
        opts.region ||
        this.configService.get<string>('AWS_IMAGE_REGION') ||
    ...
    const client = new BedrockRuntimeClient({
      ...
      ...(opts.userCredentials
        ? {
            credentials: {
              accessKeyId: opts.userCredentials.accessKeyId,
              secretAccessKey: opts.userCredentials.secretAccessKey,
            },
          }
        : {}),
    });
    ...
    const seed = Math.abs(opts.seed ?? Math.floor(Math.random() * 2_000_000)) % 2_147_483_647;
    const text = (prompt || '').slice(0, 510);
    const negative = (opts.negativePrompt || 'text, watermark, logo, low quality, blurry, deformed').slice(0, 510);
    let lastErr: any;

    // 1) Amazon image models — Nova Canvas (current) → Titan (legacy fallback). Same body shape.
    for (const modelId of ['amazon.nova-canvas-v1:0', 'amazon.titan-image-generator-v2:0', 'amazon.titan-image-generator-v1']) {
      try {
        const body = {
          taskType: 'TEXT_IMAGE',
          textToImageParams: { text, negativeText: negative },
          imageGenerationConfig: { numberOfImages: 1, width, height, cfgScale: 8.0, seed, quality: 'standard' },
        };
        ...
        lastErr = e;
        this.logger.warn(`Titan image (${modelId}) failed: ${e?.name || ''} ${e?.message || e}`);
      }
    }

    // 2) Stability fallback (stable-image-core / SDXL). Body & response differ.
    for (const modelId of ['stability.stable-image-core-v1:1', 'stability.stable-image-core-v1:0', 'stability.sd3-5-large-v1:0']) {
      try {
        const body: any = { prompt: text, mode: 'text-to-image', aspect_ratio: '2:3', output_format: 'png', seed };
        if (opts.negativePrompt) body.negative_prompt = negative;
        const resp = await client.send(
          new InvokeModelCommand({ modelId, contentType: 'application/json', accept: 'application/json', body: JSON.stringify(body) }),
        );
        const data = JSON.parse(new TextDecoder().decode(resp.body));
        const b64 = data?.images?.[0] || (Array.isArray(data?.artifacts) ? data.artifacts[0]?.base64 : undefined);
        if (b64) return b64;
      } catch (e: any) {
        lastErr = e;
        this.logger.warn(`Stability image (${modelId}) failed: ${e?.name || ''} ${e?.message || e}`);
      }
    }

    throw new Error(
      `No accessible Bedrock image model in region ${region} (enable Titan Image Generator or Stability model access). Last error: ${lastErr?.message || lastErr}`,
    );
  }
  ...
  private async invokeWithPlatformToken(modelId: string, body: any, region: string): Promise<any> {
    ...
    const response = await axios.post(
      ...
      body,
      {
        ...
        httpsAgent,
        proxy: false,
        // v0.7.7 — Non-streaming Bedrock invoke. axios `timeout` is the time
        // between request start and response complete. For Claude with large
        // tool result inputs (audit task with multiple read_file outputs) the
        // server-side processing alone can take 30-90s; bump from 60s → 240s.
        timeout: 240_000,
      },
    );
   ...
   */
  async invokeModel(prompt: string, modelId: string = 'us.anthropic.claude-haiku-4-5-20251001-v1:0', userCredentials?: BedrockUserCredentials, maxTokens: number = 2000): Promise<string> {
    ...
    this.logger.log(`Calling Bedrock: ${finalModelId} (Region: ${region}, UserCreds: ${!!userCredentials}, maxTokens: ${maxTokens})`);
  ...
  private async invokeStreamingWithPlatformToken(
    ...
    const response = await axios.post(
      ...
      body,
      {
        ...
        httpsAgent,
        proxy: false,
        // v0.7.7 — `timeout` here is axios's request timeout AND because we
        // set `responseType: 'stream'`, it acts as the time-to-first-byte
        // ceiling. Bump 60s → 600s (10 min) so a long Claude reasoning phase
        // (e.g. Sonnet 4.6 chewing through large tool outputs) doesn't drop
        // the stream mid-think. Audit task with 5+ read_file rounds is the
        // canonical case.
        timeout: 600_000,
        responseType: 'stream',
      },
  ...
  async chatWithFunctions(messages: any[], options: any = {}): Promise<any> {
    ...
    try {
      ...
      if (options.tools && options.tools.length > 0) {
        ...
        // Tool-choice forwarding. Anthropic supports:
        //   {type: 'auto'}  - default, model picks
        //   {type: 'any'}   - model must call SOME tool (no plain-text reply)
        //   {type: 'tool', name: '...'} - force a specific tool
        // The caller (e.g. ClaudeIntegrationService) can pass either a string
        // ('auto'|'any') or an object. We forward verbatim; default is 'auto'.
  ...
  /**
   * Public vision-capable invocation entry point.
   *
   * Used by World Engine AI Interpreter to analyze 3D mesh + source photos
   * via Claude vision (Haiku 4.5 / Sonnet 4.6 / Opus 4.x).
   *
   * - With `userCredentials`: BYOK path, user's AWS account is charged
   * - Without: platform Bearer token path, billed to platform quota
   *
   * @param modelId Full Bedrock model ID or short alias (e.g. 'haiku', 'sonnet')
   * @param body Anthropic Messages API body (with image blocks in content array)
   * @param userCredentials Optional user AWS credentials for BYOK
   */
  async invokeVisionModel(
    modelId: string,
    body: any,
    userCredentials?: BedrockUserCredentials,
  ): Promise<any> {
    const finalModelId = this.resolveModelId(modelId);
    const region = this.normalizeRegion(
      userCredentials?.region || this.configService.get<string>('AWS_REGION'),
    );

    if (userCredentials) {
      return this.invokeWithUserCredentials(finalModelId, body, userCredentials);
    }
    return this.invokeWithPlatformToken(finalModelId, body, region);
  }
}
