...
export class Plugin {
  ...
  manifest?: {
    ...
    /**
     * Plugin-provided callable tools. `inputSchema` advertised to the LLM.
     * Execution binding (#2 ① 真执行):
     *   - { type:'http', endpoint, method?, headers? } → 直接 HTTP 调用(带鉴权头)
     *   - { type:'mcp', mcpServer } → 路由到本 plugin 声明的 mcpServers[name]
     *   - 无 exec → 不可执行(仅展示)
     */
    tools?: Array<{
      name: string;
      ...
      exec?:
        | { type: 'http'; endpoint: string; method?: string; headers?: Record<string, string> }
        | { type: 'mcp'; mcpServer: string };
    }>;
    permissions?: string[];
