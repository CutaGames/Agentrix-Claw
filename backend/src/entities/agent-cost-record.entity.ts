...
export class AgentCostRecord {
  ...
  /**
   * Multi-Agent v1 (W5) — sub-task linkage. NULL for plain LLM calls
   * outside a multi-agent flow. Spec: design.md §2.2 新增 4
   */
  ...
  /**
   * Multi-Agent v1 (W5) — discriminator for cost-record kind.
   *   'llm_call'           — raw LLM invocation cost (default)
   *   'sub_task_complete'  — written by worker on terminal status; sums
   *                          all llm_call rows under the same parent_task_id
   *   'tool_call' / 'override' — extension points for v2
   * Spec: design.md §2.2 新增 4
   */
