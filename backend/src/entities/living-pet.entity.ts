...
export class LivingPet {
  ...
  /**
   * Multi-Agent v1 (W3) — when set, this pet doubles as an AgentTeam
   * member that can be delegated sub-tasks via `agent_run` tool.
   * NULL = companion-only (default). FK ON DELETE SET NULL so deleting
   * the AgentAccount preserves the pet.
   *
   * Spec: design.md §2.2 新增 6, §11.1
   */
  @Column({ type: 'varchar', length: 64, nullable: true })
  @Index()
