...
export default function ConsolePresence(): React.ReactElement {
  ...
  const load = React.useCallback(async (): Promise<void> => {
    ...
    setPet(p);
    // Defensive: backend may return null (502/503), an envelope {data:[…]}, or a non-array.
    // Guard against `d.map is not a function` runtime errors in production.
    const handoffsList = Array.isArray(h)
      ? h
      : Array.isArray((h as any)?.data)
        ? ((h as any).data as HandoffRecord[])
        : Array.isArray((h as any)?.items)
          ? ((h as any).items as HandoffRecord[])
          : [];
    const approvalsList = Array.isArray(a)
      ? a
      : Array.isArray((a as any)?.data)
        ? ((a as any).data as ApprovalRequest[])
        : Array.isArray((a as any)?.items)
          ? ((a as any).items as ApprovalRequest[])
          : [];
    setHandoffs(handoffsList);
