...
export const getServerSideProps: GetServerSideProps<SkinDetailPageProps> = async (ctx) => {
  ...
  // Validate that response payload actually looks like a SkinListItem.
  // The /v1/marketplace/pets/:id and /v1/pet/skins/marketplace/:id endpoints
  // sometimes return null, an envelope, or a different shape — only treat the
  // result as a skin if the required keys are present.
  const isValidSkin = (s: unknown): s is SkinListItem => {
    if (!s || typeof s !== 'object') return false;
    const obj = s as Record<string, unknown>;
    return (
      typeof obj.id === 'string' &&
      typeof obj.displayName === 'string' &&
      typeof obj.clan === 'string'
    );
  };

  try {
    // Primary: /v1/market/skins?limit=200 + filter — returns canonical SkinListItem shape.
    const { data } = await axios.get(`${baseUrl}/v1/market/skins`, {
      params: { limit: 200 },
      timeout: 5000,
    });
    const items: SkinListItem[] = Array.isArray(data?.items) ? data.items : [];
    const found = items.find((s) => s.id === id);
    if (isValidSkin(found)) {
      skin = found;
    }
  } catch {
  ...
  if (!skin) {
    try {
      ...
      // Some legacy endpoints wrap the payload in {listing: ...} or {data: ...}
      const candidate = (data as any)?.listing ?? (data as any)?.data ?? data;
...
export default function SkinDetailPage({
  ...
  const [axpModalOpen, setAxpModalOpen] = useState(false);
  ...
  return (
    <MarketplaceLayout seo={seo} activeSection="skins">
            ...
            <div>
              ...
              <div className="mt-4 flex items-center gap-5 text-sm text-gray-400">
                <span className="flex items-center gap-1.5">
                  ...
                  {(skin.likeCount ?? 0).toLocaleString()}
                </span>
                <span className="flex items-center gap-1.5">
                  <Eye size={15} className="text-blue-400" />
                  {(skin.viewCount ?? 0).toLocaleString()}
                </span>
                <span className="flex items-center gap-1.5">
                  <GitBranch size={15} className="text-green-400" />
                  {(skin.remixCount ?? 0).toLocaleString()} {t({ zh: '次 Remix', en: 'remixes' })}
                </span>
              </div>
            </div>
            ...
            <div className="rounded-xl border border-gray-700 bg-gray-800/50 p-5">
              ...
              {skin.parentSkinId ? (
                ...
                <p className="mt-3 text-sm text-gray-500">
                  {(skin.remixCount ?? 0) > 0
                    ? t({
                        zh: `此皮肤已被 Remix ${skin.remixCount ?? 0} 次`,
                        en: `This skin has been remixed ${skin.remixCount ?? 0} times`,
                      })
      ...
      {/* AXP Purchase Modal — opened from the price/auction action buttons.
          Shows balance check, insufficient balance warning, login redirect,
          and POSTs to /v1/pet-skin/marketplace/:id/install-with-axp on confirm. */}
      <AxpPurchaseModal
        skin={skin}
        open={axpModalOpen}
        onClose={() => setAxpModalOpen(false)}
      />
    </MarketplaceLayout>
  );
}
