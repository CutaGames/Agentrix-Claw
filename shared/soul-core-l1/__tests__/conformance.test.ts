...
import { encodeProposalV2, proposalDigestV2, type ProposalV2 } from '../v1_1/proposal-digest-v2';
...
const golden: {
  ...
  proposalsV2: { name: string; proposal: ProposalV2; expectedEncodedHex: string; expectedProposalDigest: string }[];
...
describe('Soul Core L1 v1.1 cross-impl conformance (T17)', () => {
  ...
  it('every V2 proposal reproduces frozen length-prefixed bytes and digest', () => {
    for (const v of golden.proposalsV2) {
      expect(hex(encodeProposalV2(v.proposal))).toBe(v.expectedEncodedHex);
      expect(proposalDigestV2(v.proposal)).toBe(v.expectedProposalDigest);
    }
  });
