...
describe('Soul Core L1 v1.1 Profile Registry (T12)', () => {
  ...
  it('routes every proposal-capable Profile to V2 and rejects legacy V1 APDU downgrade', () => {
    for (const profileId of PROFILE_IDS) {
      expect(checkApdu(profileId, 'SIGN_PROPOSAL_V2').allowed).toBe(true);
      expect(checkApdu(profileId, 'SIGN_PROPOSAL').allowed).toBe(false);
    }
  });
