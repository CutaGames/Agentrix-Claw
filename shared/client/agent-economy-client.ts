...
function validateSettlementEvent(input: unknown): ValidationResult {
  ...
  if (!['debit_pending', 'debit_confirmed', 'refund_pending', 'refund_confirmed', 'reversal_confirmed', 'reconciliation_required'].includes(String(value.eventType))) errors.push('eventType: unknown');
