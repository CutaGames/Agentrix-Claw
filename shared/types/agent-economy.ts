...
export interface EconomyPaymentAcceptV1 {
  scheme: 'exact';
  network: string;
...
/** Authority-owned x402 conditions bound into a paid quote's integrity digest. */
export interface EconomyPaymentRequirementsV1 {
  x402Version: 1;
  accepts: EconomyPaymentAcceptV1[];
  error: string | null;
}
...
export interface ActionQuoteV1 {
  ...
  amount: Money;
  maximumAmount?: Money;
  /** Required for paid quotes; omitted only for zero-value or historical records. */
  paymentRequirements?: EconomyPaymentRequirementsV1;
  termsRefs: RecordRef[];
...
export interface SettlementEventV1 {
  ...
  settlementAuthorityRef: PartyRef;
  /** Source-owned payer/beneficiary bindings; absent only on legacy v1 events. */
  settlementParties?: PartyRef[];
  /** Canonical owning Remedy authority for refund/reversal facts. */
  remedyAuthorityRef?: PartyRef;
  /** Source-owned claimant/respondent settlement parties for Remedy facts. */
...
/**
 * Owner-scoped read envelope for the canonical receipt composer. A null receipt
 * is an intentional, honest state: consumers render the lane dispositions and
 * diagnostics without inferring execution or settlement success.
 */
export interface AgentEconomyActionReceiptResultV1 {
  schemaVersion: AgentEconomySchemaVersion;
  soulCoreId: string;
  actionId: string;
  accountableAgentId: string;
  receipt: ActionReceiptV1 | null;
  /**
   * Canonical read projection used by the receipt. It remains independently
   * visible when receipt composition is blocked by another missing source ref.
   */
  responsibilityLineage: ResponsibilityLineageV1 | null;
  /**
   * Null until the commission owning domain exposes an authoritative,
   * digest-bound receipt/checkpoint. Projector-only candidate rows are not
   * promoted to commission truth.
   */
  commissionAllocation: CommissionAllocationViewV1 | null;
  dispositions: {
    paymentDisposition: PaymentDispositionV1;
    settlementDisposition: SettlementDispositionV1;
    remedyDisposition: RemedyDispositionV1;
...
function isEvidenceRef(value: unknown): value is EvidenceRef {
  return isObject(value)
    ...
    && (EVIDENCE_KINDS as readonly unknown[]).includes(value.kind)
    && isDigestRef(value.digest)
    && (value.locator === undefined || isNonEmptyString(value.locator))
    && (value.issuer === undefined || isPartyRef(value.issuer))
    && (DATA_CLASSES as readonly unknown[]).includes(value.dataClass)
    && isNonEmptyString(value.createdAt);
}

function isEvidenceRefArray(value: unknown): value is EvidenceRef[] {
...
export function validateCommissionAllocationViewV1(input: unknown): AgentEconomyValidationResultV1 {
  const { value, errors } = validateBase(input, 'commissionAllocation');
  if (!value) return { valid: false, errors };
  for (const key of ['allocationViewId', 'actionId', 'generatedAt']) {
    if (!isNonEmptyString(value[key])) errors.push(`commissionAllocation.${key}: required`);
  }
  if (!isRecordRefV1(value.policyRef)) errors.push('commissionAllocation.policyRef: invalid');
  if (!isPartyRef(value.projectorRef) || value.projectorRef.kind !== 'projector') {
    errors.push('commissionAllocation.projectorRef: projector PartyRef required');
  }
  if (!Array.isArray(value.allocations)) {
    errors.push('commissionAllocation.allocations: required');
  } else {
    value.allocations.forEach((allocation, index) => {
      if (!isObject(allocation)) {
        errors.push(`commissionAllocation.allocations[${index}]: invalid`);
        return;
      }
      if (!isPartyRef(allocation.beneficiaryRef)) errors.push(`commissionAllocation.allocations[${index}].beneficiaryRef: invalid`);
      if (!isMoney(allocation.amount)) errors.push(`commissionAllocation.allocations[${index}].amount: invalid`);
      if (!['provider', 'referrer', 'platform', 'other'].includes(String(allocation.role))) errors.push(`commissionAllocation.allocations[${index}].role: unknown`);
      if (!['calculated', 'payable', 'paid', 'reversed'].includes(String(allocation.status))) errors.push(`commissionAllocation.allocations[${index}].status: unknown`);
      if (!isRecordRefV1(allocation.sourceSettlementRef)) errors.push(`commissionAllocation.allocations[${index}].sourceSettlementRef: invalid`);
      if (allocation.reversalRef !== undefined && !isRecordRefV1(allocation.reversalRef)) errors.push(`commissionAllocation.allocations[${index}].reversalRef: invalid`);
    });
  }
  if (!['projected', 'partially_paid', 'paid', 'reversed', 'unverifiable'].includes(String(value.status))) {
    errors.push('commissionAllocation.status: unknown');
  }
  return { valid: errors.length === 0, errors };
}
...
export function validateActionReceiptV1(input: unknown): AgentEconomyValidationResultV1 {
  ...
  for (const key of ['planRef', 'quoteRef', 'budgetReservationRef']) {
    if (value[key] !== undefined && !isRecordRefV1(value[key])) {
      errors.push(`actionReceipt.${key}: invalid`);
    }
  }
  ...
  if (!isEvidenceRefArray(value.evidenceRefs)) errors.push('actionReceipt.evidenceRefs: invalid');
  ...
  if (!Array.isArray(value.missingOrInvalidRefs)
    || !value.missingOrInvalidRefs.every(isNonEmptyString)) {
    errors.push('actionReceipt.missingOrInvalidRefs: non-empty string array required');
  }
