...
/** PATCH /v1/creations/:id — owner-scoped metadata edit; lifecycle uses dedicated routes. */
export interface UpdateCreationRequest {
  title?: string;
  summary?: string | null;
}

export interface UpdateCreationResponse {
  creation: Creation;
}

/** GET /v1/creations/:id/operator-status — authoritative owner operation surface. */
export interface CreationOperatorStatusResponse {
  schemaVersion: 1;
  creationId: string;
  ...
  allowedTransitions: CreationStatus[];
  capabilities: {
    edit: boolean;
...
export type FulfillmentVoucherStatus =
  | 'issued'
  | 'revocation_pending'
  | 'redeemed'
  | 'revoked';
