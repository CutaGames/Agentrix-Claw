...
export function AgentConsoleScreen() {
  ...
  const activeInstanceName = resolveAgentDisplayName(
    activeInstance,
    t({ en: 'My Agent', zh: '我的 Agent' }),
  );
    ...
    >
      ...
      <View style={styles.statusCard}>
        <View style={styles.statusLeft}>
          ...
          <View>
            <Text style={styles.instanceName} testID="agent-active-name" numberOfLines={1}>
              {activeInstanceName}
            </Text>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.instanceScroll}>
          {instances.map((inst, index) => (
            ...
            >
              ...
              <Text
                style={[styles.chipText, activeInstance.id === inst.id && { color: colors.accent }]}
                testID={`agent-instance-name-${index}`}
                numberOfLines={1}
              >
                {resolveAgentDisplayName(inst, t({ en: `Agent ${index + 1}`, zh: `Agent ${index + 1}` }))}
              </Text>
            </TouchableOpacity>
          ))}
        </ScrollView>
      ...
      {tab === 'overview' && (
        <>
          <TouchableOpacity
            ...
            onPress={() => navigation.navigate('AgentChat', { instanceId: activeInstance.id, instanceName: activeInstanceName })}
          >
            <Text style={styles.chatCtaEmoji}>💬</Text>
            <View>
              <Text style={styles.chatCtaTitle}>{t({ en: `Chat with ${activeInstanceName}`, zh: `与 ${activeInstanceName} 对话` })}</Text>
        ...
        <View style={styles.tasksSection}>
          <View style={styles.opsMiniCard}>
            ...
            <View style={{ flexDirection: 'row', gap: 8 }}>
              <TouchableOpacity
                style={styles.secondaryBtn}
                onPress={() => navigateToDestinationError(navigation as any, 'route_not_mounted')}
              >
