...
import { ReputationAssuranceCard } from '../../components/soul-core/ReputationAssuranceCard';
import { TrustActionTimelineCard } from '../../components/soul-core/TrustActionTimelineCard';
...
import {
  ...
  createMobileAgentEconomyReceiptClient,
  ...
  isMobileAgentEconomyRemedyExecutionEnabled,
...
import { extractX402PaymentOption } from '../../services/aggregatedMarket.api';
import {
  getManagedTransactionByIdempotencyKey,
  makeUserWalletX402Payer,
...
const MOBILE_DISCOVERY_KINDS = [...DISCOVERABLE_KINDS_V1];
...
export function AgentHomeScreen({ navigation }: any) {
  ...
  const actionsQ = useQuery({
    queryKey: context.kind === 'ready'
      ? ['mobile-v7', 'owner', ownerScope ?? 'no-owner', ...context.context.scope.queryKey, 'recent-actions']
      : ['mobile-v7', 'owner', ownerScope ?? 'no-owner', 'recent-actions', 'unresolved'],
    queryFn: () => directory.facade.listActions(soulCoreId as string, { enabled: economyEnabled }),
    enabled: !!ownerScope && !!soulCoreId,
    retry: 0,
  });
  const actionsState: MobileReadState<ActionTaskListV1> = actionsQ.data ?? (
    !economyEnabled
      ...
      : !ownerScope
        ? { kind: 'unauthorized', reason: 'owner_session_missing' }
        : !soulCoreId
  ...
  return (
    ...
    >
      ...
      <ReputationAssuranceCard
        soulCoreId={soulCoreId}
...
export function GoalComposerScreen(props: any) {
  const ownerScope = useAuthStore((state) => state.user?.id ?? null);
  return <GoalComposerOwnerScreen key={ownerScope ?? 'no-owner'} {...props} ownerScope={ownerScope} />;
}

function GoalComposerOwnerScreen({ navigation, route, ownerScope }: any) {
  const { t } = useI18n();
  const styles = useThemedStyles(makeStyles);
  const directory = useMobileAgentDirectory(route.params.agentId);
  const client = React.useMemo(() => createMobileAgentEconomyClient(ownerScope), [ownerScope]);
  ...
  const paidExecutionEnabled = isMobileAgentEconomyPaidExecutionEnabled();
  ...
  const proceed = async () => {
    if (pendingRef.current || !ownerScope || !economyEnabled || !soulCoreId) return;
    ...
    if (!submissionError?.uncertain) {
      ...
      const constraints: GoalConstraintsV1 = {
        allowedKinds: MOBILE_DISCOVERY_KINDS,
        ...(amountMinor ? {
  ...
  const canSubmit = !!ownerScope && economyEnabled && !!soulCoreId && !submitting;
  ...
  return (
    <ScrollView style={styles.screen} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" testID="goal-composer-screen">
      ...
      <TextInput value={budget} editable={!submitting && !immutableUnknownDraft} onChangeText={(value) => { setBudget(value); resetDraftMutation(); }} style={styles.input} keyboardType="decimal-pad" placeholder={t({
        en: paidExecutionEnabled ? 'Optional · canonical ceiling' : 'Optional · paid execution disabled',
        zh: paidExecutionEnabled ? '可选 · canonical 金额上限' : '可选 · 付费执行已关闭',
      })} placeholderTextColor="#8c8279" testID="goal-budget" />
      ...
      <View style={styles.softCard} testID="safe-v1-payment-boundary"><Text style={styles.cardTitle}>{t({ en: 'Release boundary', zh: '发布边界' })}</Text><Text style={styles.muted}>{t({
        en: paidExecutionEnabled
          ? 'Live discovery is enabled. Paid Creation offerings require canonical x402 terms, explicit authority and the bound user wallet.'
          : 'Live discovery is enabled; paid execution remains disabled in this build.',
        zh: paidExecutionEnabled
          ? '实时发现已开启；付费 Creation offering 必须具备 canonical x402 条款、明确授权与已绑定用户钱包。'
          : '实时发现已开启；此构建仍关闭付费执行。',
      })}</Text></View>
...
export function CandidateCompareScreen(props: any) {
  const ownerScope = useAuthStore((state) => state.user?.id ?? null);
  return <CandidateCompareOwnerScreen key={ownerScope ?? 'no-owner'} {...props} ownerScope={ownerScope} />;
}

function CandidateCompareOwnerScreen({ navigation, route, ownerScope }: any) {
  const { t } = useI18n();
  const styles = useThemedStyles(makeStyles);
  const directory = useMobileAgentDirectory(route.params.agentId);
  const client = React.useMemo(() => createMobileAgentEconomyClient(ownerScope), [ownerScope]);
  ...
  const loadCandidates = React.useCallback(async (reconcileOnly = false) => {
    if (pendingRef.current || !ownerScope || !economyEnabled || !soulCoreId || !route.params.actionId) return;
    pendingRef.current = true;
    setLoading(true);
    setError(undefined);
    try {
      ...
      if (!reconcileOnly && latest.candidates.length === 0) {
        ...
        const outcome = await runMobileEconomyMutation(() => client.discoverCandidates(
          soulCoreId,
          latest,
          { kinds: MOBILE_DISCOVERY_KINDS, limit: 3 },
  ...
  }, [client, economyEnabled, lineageError, ownerScope, route.params.actionId, route.params.goalId, soulCoreId]);

  React.useEffect(() => {
    const key = `${ownerScope ?? 'no-owner'}:${soulCoreId ?? 'unresolved'}:${route.params.actionId ?? 'missing'}`;
    if (!ownerScope || !economyEnabled || !soulCoreId || !route.params.actionId || startedRef.current === key) return;
    startedRef.current = key;
    void loadCandidates(false);
  }, [economyEnabled, loadCandidates, ownerScope, route.params.actionId, soulCoreId]);

  const chooseCandidate = async (candidate: DiscoveryCandidateV1) => {
    if (pendingRef.current || !ownerScope || !soulCoreId || !['eligible', 'selected'].includes(candidate.status)) return;
    pendingRef.current = true;
    ...
    try {
      ...
      setWorkflow(latest);
      const paidQuote = isMobilePaidQuote(latest.quote);
      if (!isMobileZeroUsdQuote(latest.quote) && (!paidQuote || !paidExecutionEnabled)) {
        setError({
          kind: 'paid_execution_disabled',
          title: 'Paid execution disabled',
          detail: paidQuote
            ? 'This build keeps user-wallet payment disabled.'
            : 'The authority did not supply usable canonical x402 requirements.',
          uncertain: false,
        });
        return;
      }
        ...
        return (
          <View key={candidate.candidateId} style={[styles.candidateCard, !candidateEligible && styles.candidateCardDisabled]} testID={`candidate-${candidate.candidateId}`}>
            ...
            <TouchableOpacity style={[styles.primaryButton, !candidateEligible && styles.disabled]} disabled={!candidateEligible || pendingRef.current} onPress={() => chooseCandidate(candidate)} testID={`candidate-select-${candidate.candidateId}`}>
              {pendingCandidateId === candidate.candidateId ? <ActivityIndicator color="#fff" /> : <Text style={styles.primaryButtonText}>{t({
                en: paidExecutionEnabled ? 'Select & request canonical quote' : 'Select & request 0 USD quote',
                zh: paidExecutionEnabled ? '选择并请求 canonical 报价' : '选择并请求 0 USD 报价',
              })}</Text>}
            </TouchableOpacity>
          </View>
        );
...
function useActionRead(ownerScope: string | null, soulCoreId: string | undefined, actionId: string) {
  const facade = React.useMemo(() => createMobileV6QueryFacade(), []);
  const enabled = isMobileV6FeatureEnabled('mobile.agent_economy_v1');
  const query = useQuery({
    queryKey: ['mobile-v7', 'owner', ownerScope ?? 'no-owner', 'action', soulCoreId ?? 'unresolved', actionId],
    queryFn: () => facade.getAction(soulCoreId as string, actionId, { enabled }),
    enabled: !!ownerScope && !!soulCoreId && !!actionId,
    retry: 0,
  });
...
export function AuthorityReviewScreen(props: any) {
  const ownerScope = useAuthStore((state) => state.user?.id ?? null);
  return <AuthorityReviewOwnerScreen key={ownerScope ?? 'no-owner'} {...props} ownerScope={ownerScope} />;
}

function AuthorityReviewOwnerScreen({ navigation, route, ownerScope }: any) {
  const { t } = useI18n();
  const styles = useThemedStyles(makeStyles);
  const directory = useMobileAgentDirectory(route.params.agentId);
  const client = React.useMemo(() => createMobileAgentEconomyClient(ownerScope), [ownerScope]);
  ...
  const loadWorkflow = React.useCallback(async () => {
    if (!ownerScope || !enabled || !soulCoreId || !route.params.actionId || pendingRef.current) return;
    pendingRef.current = true;
    setLoading(true);
    setError(undefined);
    try {
  ...
  }, [client, enabled, ownerScope, route.params.actionId, soulCoreId]);

  React.useEffect(() => {
    const key = `${ownerScope ?? 'no-owner'}:${soulCoreId ?? 'unresolved'}:${route.params.actionId}`;
    if (!ownerScope || !enabled || !soulCoreId || startedRef.current === key) return;
    startedRef.current = key;
    void loadWorkflow();
  }, [enabled, loadWorkflow, ownerScope, route.params.actionId, soulCoreId]);
  ...
  const authorize = async () => {
    if (pendingRef.current || !ownerScope || !soulCoreId) return;
    pendingRef.current = true;
    setLoading(true);
    setError(undefined);
    try {
      ...
      if (!isMobileZeroUsdQuote(latest.quote)
        && (!paidExecutionEnabled || !isMobilePaidQuote(latest.quote))) {
        setWorkflow(latest);
        setError({
          kind: 'paid_execution_disabled',
          title: 'Paid execution disabled',
          detail: paidExecutionEnabled
            ? 'Canonical x402 payment requirements are unavailable.'
            : 'This build keeps user-wallet payment disabled.',
          uncertain: false,
        });
        return;
  ...
  const payableQuote = isMobilePaidQuote(quote);
  const activeMandate = isMobileEconomyMandateActive(workflow?.mandate);
  const canAuthorize = activeMandate || (!!ownerScope && enabled
    && !!soulCoreId
    && !!quote
    && quote.status === 'offered'
    && !quoteExpired
    && (!paidQuote || (paidExecutionEnabled && payableQuote))
    && !loading);
  ...
  return (
    <ScrollView style={styles.screen} contentContainerStyle={styles.content} testID="authority-review-screen">
      ...
      {quote ? (
        <>
          <View style={styles.softCard}>
            <Text style={styles.eyebrow}>{t({
              en: paidQuote ? 'CANONICAL X402 AUTHORITY' : '0 USD RELEASE GATE',
              zh: paidQuote ? 'CANONICAL X402 授权' : '0 USD 发布门',
            })}</Text>
          ...
          {paidQuote && (!paidExecutionEnabled || !payableQuote) ? <View style={styles.blockedCard} testID="paid-quote-blocked"><Text style={styles.noticeTitle}>{t({ en: 'Paid quote blocked', zh: '付费报价已阻断' })}</Text><Text style={styles.noticeText}>{t({
            en: paidExecutionEnabled ? 'Canonical x402 requirements are missing or unsupported.' : 'This build keeps user-wallet paid execution disabled.',
            zh: paidExecutionEnabled ? 'Canonical x402 支付要求缺失或不受支持。' : '此构建保持用户钱包付费执行关闭。',
          })}</Text></View> : null}
      ...
      <TouchableOpacity style={[styles.primaryButton, !canAuthorize && styles.disabled]} disabled={!canAuthorize || pendingRef.current} onPress={authorize} testID="authority-authorize">
        {loading ? <ActivityIndicator color="#fff" /> : <Text style={styles.primaryButtonText}>{activeMandate
          ? t({ en: 'View authorized Action', zh: '查看已授权行动' })
          : paidQuote
            ? t({ en: 'Authorize paid Action', zh: '授权付费行动' })
            : t({ en: 'Authorize 0 USD Action', zh: '授权 0 USD 行动' })}</Text>}
      </TouchableOpacity>
...
function ReceiptRefList({
  ...
  return (
    <View>
      ...
      {refs.length > 0
        ? refs.map((ref) => (
          <Text key={`${label}:${ref.type}:${ref.id}`} style={styles.muted}>
...
/**
 * Owner-scoped, receipt-only reconciliation. This component has no payment or
 * remedy mutation client; terminal execution alone can never manufacture a
 * receipt, settlement, verification or Remedy success state.
 */
function CanonicalActionReceiptPanel({
  soulCoreId,
  actionId,
}: {
  soulCoreId?: string;
  ...
  const styles = useThemedStyles(makeStyles);
  const ownerScope = useAuthStore((state) => state.user?.id ?? null);
  const featureEnabled = isMobileAgentEconomyEnabled()
    && process.env.EXPO_PUBLIC_MOBILE_AGENT_ECONOMY_RECEIPT === '1';
  const client = React.useMemo(
    () => createMobileAgentEconomyReceiptClient(ownerScope),
    [ownerScope],
  );
  const query = useQuery({
    queryKey: [
      'mobile-v7',
      'economy-action-receipt',
      ownerScope ?? 'no-owner',
      soulCoreId ?? 'unresolved',
      actionId ?? 'missing',
    ],
    queryFn: () => client.getActionReceipt(soulCoreId as string, actionId as string),
    enabled: featureEnabled && !!ownerScope && !!soulCoreId && !!actionId,
    retry: 0,
  });

  if (!featureEnabled) {
    return (
      <View style={styles.notice} testID="canonical-receipt-disabled">
        <Text style={styles.noticeTitle}>{t({ en: 'Canonical ActionReceipt disabled', zh: 'Canonical ActionReceipt 已关闭' })}</Text>
        <Text style={styles.noticeText}>{t({ en: 'The Agent Economy feature gate is off. No receipt state is inferred.', zh: 'Agent Economy 功能门已关闭；不会推断任何凭证状态。' })}</Text>
      </View>
    );
  }
  if (!ownerScope || !soulCoreId || !actionId) {
    return (
      <View style={styles.notice} testID="canonical-receipt-unresolved">
        <Text style={styles.noticeTitle}>{t({ en: 'Canonical ActionReceipt unavailable', zh: 'Canonical ActionReceipt 当前不可用' })}</Text>
        <Text style={styles.noticeText}>{t({ en: 'An authenticated owner session plus canonical Soul Core and Action bindings are required.', zh: '需要已认证 owner 会话以及 canonical Soul Core 与 Action 绑定。' })}</Text>
      </View>
    );
  }
  if (query.isLoading) {
    return (
      <View style={styles.loadingCard} testID="canonical-receipt-loading">
        <ActivityIndicator />
        <Text style={styles.muted}>{t({ en: 'Reading canonical ActionReceipt…', zh: '正在读取 canonical ActionReceipt…' })}</Text>
      </View>
    );
  }
  if (query.isError) {
    const error = describeEconomyClientError(query.error, false);
    return (
      <View style={styles.notice} testID="canonical-receipt-error">
        <Text style={styles.noticeTitle}>{t({ en: 'Canonical ActionReceipt unavailable', zh: 'Canonical ActionReceipt 当前不可用' })}</Text>
        <Text style={styles.noticeText}>{t({ en: 'No receipt or terminal state was inferred from this read failure.', zh: '读取失败时不会推断凭证或任何终态。' })}</Text>
        <Text style={styles.helper}>{error.kind}</Text>
        <TouchableOpacity onPress={() => query.refetch()}><Text style={styles.textLink}>{t({ en: 'Try again', zh: '重试' })}</Text></TouchableOpacity>
      </View>
    );
  }
  ...
  const lineage = result.responsibilityLineage;
  const commission = result.commissionAllocation;
  return (
    <View style={styles.softCard} testID={receipt ? 'canonical-receipt-ready' : 'canonical-receipt-incomplete'}>
      <Text style={styles.eyebrow}>{t({ en: 'CANONICAL ACTION RECEIPT', zh: 'CANONICAL 行动凭证' })}</Text>
      <SummaryRow label={t({ en: 'Payment', zh: '付款' })} value={result.dispositions.paymentDisposition} />
      <SummaryRow label={t({ en: 'Settlement', zh: '结算' })} value={result.dispositions.settlementDisposition} />
      <SummaryRow label={t({ en: 'Remedy', zh: '补救' })} value={result.dispositions.remedyDisposition} />
      <SummaryRow label={t({ en: 'Accountable Agent', zh: '责任 Agent' })} value={shortId(result.accountableAgentId)} />

      {lineage ? (
        <View style={styles.evidenceBox} testID="canonical-responsibility-lineage">
          <Text style={styles.cardTitle}>{t({ en: 'Canonical responsibility parties', zh: 'Canonical 责任参与方' })}</Text>
          <SummaryRow label={t({ en: 'Lineage', zh: '责任链' })} value={`${lineage.status} · ${shortId(lineage.responsibilityLineageId)}`} warning={lineage.status !== 'complete'} />
          <SummaryRow label={t({ en: 'Principal', zh: '委托主体' })} value={`${lineage.principalRef.kind}:${shortId(lineage.principalRef.id)}`} />
          <SummaryRow label={t({ en: 'Authorizer', zh: '授权方' })} value={`${lineage.authorizerRef.kind}:${shortId(lineage.authorizerRef.id)}`} />
          <Text style={styles.cardTitle}>{t({ en: 'Providers', zh: 'Provider' })}</Text>
          {lineage.providerParties.map((party, index) => (
            <Text key={`provider:${party.kind}:${party.id}:${index}`} style={styles.muted}>{party.kind}:{shortId(party.id)}</Text>
          ))}
          <Text style={styles.cardTitle}>{t({ en: 'Executors', zh: 'Executor' })}</Text>
          {lineage.executorParties.map((party, index) => (
            <Text key={`executor:${party.kind}:${party.id}:${index}`} style={styles.muted}>{party.kind}:{shortId(party.id)}</Text>
          ))}
          <Text style={styles.cardTitle}>{t({ en: 'Settlement parties', zh: '结算参与方' })}</Text>
          {lineage.settlementAuthorityRef ? <Text style={styles.muted}>{lineage.settlementAuthorityRef.kind}:{shortId(lineage.settlementAuthorityRef.id)}</Text> : null}
          {lineage.settlementParties.length > 0
            ? lineage.settlementParties.map((party, index) => <Text key={`settlement:${party.kind}:${party.id}:${index}`} style={styles.muted}>{party.kind}:{shortId(party.id)}</Text>)
            : <Text style={styles.muted}>unavailable</Text>}
          <Text style={styles.cardTitle}>{t({ en: 'Remedy parties', zh: '补救参与方' })}</Text>
          {lineage.remedyAuthorityRef ? <Text style={styles.muted}>{lineage.remedyAuthorityRef.kind}:{shortId(lineage.remedyAuthorityRef.id)}</Text> : null}
          {lineage.remedyParties.length > 0
            ? lineage.remedyParties.map((party, index) => <Text key={`remedy:${party.kind}:${party.id}:${index}`} style={styles.muted}>{party.kind}:{shortId(party.id)}</Text>)
            : <Text style={styles.muted}>unavailable</Text>}
          {lineage.missingOrAmbiguousRefs.map((value) => <Text key={`lineage:${value}`} style={styles.warningText}>{value}</Text>)}
        </View>
      ) : (
        <View style={styles.notice} testID="canonical-responsibility-lineage-unavailable">
          <Text style={styles.noticeTitle}>{t({ en: 'Responsibility lineage unavailable', zh: '责任参与方当前不可用' })}</Text>
          <Text style={styles.noticeText}>{t({ en: 'Mobile does not infer provider, executor, principal, authorizer, settlement or remedy parties.', zh: '移动端不会推导 provider、executor、principal、authorizer、结算或补救参与方。' })}</Text>
        </View>
      )}

      {commission ? (
        <View style={styles.evidenceBox} testID="canonical-commission-allocation">
          <Text style={styles.cardTitle}>{t({ en: 'Canonical commission allocation', zh: 'Canonical 佣金分配' })}</Text>
          <SummaryRow label={t({ en: 'Status', zh: '状态' })} value={commission.status} warning={commission.status !== 'paid'} />
          {commission.allocations.map((allocation, index) => (
            <Text key={`commission:${allocation.role}:${allocation.beneficiaryRef.id}:${index}`} style={styles.muted}>
              {allocation.role} · {allocation.beneficiaryRef.kind}:{shortId(allocation.beneficiaryRef.id)} · {formatMoney(allocation.amount)} · {allocation.status}
            </Text>
          ))}
        </View>
      ) : (
        <View style={styles.notice} testID="canonical-commission-unavailable">
          <Text style={styles.noticeTitle}>{t({ en: 'Commission authority unavailable', zh: '佣金权威数据当前不可用' })}</Text>
          <Text style={styles.noticeText}>{t({
            en: 'No owning-domain commission receipt/checkpoint is available. Referrer, platform, paid and reversed states are not inferred from projector rows.',
            zh: '当前没有 owning-domain 佣金 receipt/checkpoint；不会从 projector 行推导 referrer、platform、paid 或 reversed 状态。',
          })}</Text>
        </View>
      )}
      ...
      {receipt ? (
        <>
          <SummaryRow label={t({ en: 'Receipt', zh: '凭证' })} value={`${receipt.status} · ${shortId(receipt.actionReceiptId)}`} warning={receipt.status !== 'complete'} />
          <SummaryRow label={t({ en: 'Generated', zh: '生成时间' })} value={receipt.generatedAt} />
          <ReceiptRefList label={t({ en: 'Responsibility lineage', zh: '责任链' })} refs={[receipt.responsibilityLineageRef]} />
          <ReceiptRefList label={t({ en: 'Outcome', zh: '结果' })} refs={[receipt.outcomeRef]} />
          <ReceiptRefList label={t({ en: 'Execution', zh: '执行' })} refs={receipt.executionRefs} />
          <ReceiptRefList label={t({ en: 'Task proof', zh: '任务证明' })} refs={receipt.proofRefs} />
          <ReceiptRefList label={t({ en: 'Verification', zh: '验证' })} refs={receipt.verificationRefs} />
          <ReceiptRefList label={t({ en: 'Payment attempts', zh: '付款尝试' })} refs={receipt.paymentAttemptRefs} />
          <ReceiptRefList label={t({ en: 'Settlement refs', zh: '结算引用' })} refs={receipt.settlementRefs} />
          <ReceiptRefList label={t({ en: 'Remedy refs', zh: '补救引用' })} refs={receipt.remedyRefs} />
          {receipt.missingOrInvalidRefs.length > 0 ? (
            <View style={styles.notice}>
              <Text style={styles.noticeTitle}>{t({ en: 'Receipt is incomplete', zh: '凭证尚不完整' })}</Text>
              {receipt.missingOrInvalidRefs.map((value) => <Text key={value} style={styles.noticeText}>{value}</Text>)}
            </View>
          ) : null}
        </>
      ) : (
        <View style={styles.notice} testID="canonical-receipt-not-composable">
          <Text style={styles.noticeTitle}>{t({ en: 'Receipt not yet composable', zh: '凭证尚无法组合' })}</Text>
          <Text style={styles.noticeText}>{t({
            en: 'Execution terminality is not a receipt. Missing owning-domain facts remain explicit below.',
            zh: '执行终态不等于凭证；缺失的 owning-domain 事实明确列于下方。',
          })}</Text>
        </View>
      )}

      {result.diagnostics.map((value) => <Text key={value} style={styles.muted}>{value}</Text>)}
      <TouchableOpacity style={styles.secondaryButton} onPress={() => query.refetch()}>
        <Text style={styles.secondaryButtonText}>{t({ en: 'Refresh receipt', zh: '刷新凭证' })}</Text>
      </TouchableOpacity>
    </View>
  );
}

function EconomyActionTrackingScreen({ route, ownerScope }: any) {
  const { t } = useI18n();
  const styles = useThemedStyles(makeStyles);
  const directory = useMobileAgentDirectory(route.params.agentId);
  const client = React.useMemo(() => createMobileAgentEconomyClient(ownerScope), [ownerScope]);
  ...
  const [error, setError] = React.useState<EconomyUiError>();
  const [mutating, setMutating] = React.useState<'revoke' | 'release' | 'pay' | 'refund' | 'reversal'>();
  const pendingRef = React.useRef(false);
  const revokeKeyRef = React.useRef<string | undefined>(undefined);
  const releaseKeyRef = React.useRef<string | undefined>(undefined);
  const refundKeyRef = React.useRef<string | undefined>(undefined);
  const reversalKeyRef = React.useRef<string | undefined>(undefined);
  const [paymentCheckpoint, setPaymentCheckpoint] = React.useState<MobileEconomyPaymentCheckpoint | null>(null);
  const [paymentCheckpointLoaded, setPaymentCheckpointLoaded] = React.useState(false);
  const soulCoreId = directory.model.agents
    .find((agent) => agent.agentId === route.params.agentId)?.soulCoreId;
  const paymentCheckpointScope = React.useMemo(() => (
    ownerScope && soulCoreId && route.params.actionId
      ? { ownerId: ownerScope, soulCoreId, actionId: route.params.actionId as string }
      : null
  ), [ownerScope, route.params.actionId, soulCoreId]);
  React.useEffect(() => {
    let active = true;
    setPaymentCheckpointLoaded(false);
    setPaymentCheckpoint(null);
    if (!paymentCheckpointScope) {
      setPaymentCheckpointLoaded(true);
      return () => { active = false; };
    }
    void loadMobileEconomyPaymentCheckpoint(paymentCheckpointScope)
      .then((checkpoint) => {
        if (active) setPaymentCheckpoint(checkpoint);
      })
      .catch(() => {
        if (!active) return;
        const now = new Date().toISOString();
        setPaymentCheckpoint({
          schemaVersion: 1,
          ...paymentCheckpointScope,
          quoteId: '',
          idempotencyKey: '',
          state: 'recovery_blocked',
          createdAt: now,
          updatedAt: now,
          recoveryError: 'checkpoint_read_failed',
        });
      })
      .finally(() => { if (active) setPaymentCheckpointLoaded(true); });
    return () => { active = false; };
  }, [paymentCheckpointScope]);
  const enabled = isMobileAgentEconomyEnabled();
  const paidExecutionEnabled = isMobileAgentEconomyPaidExecutionEnabled();
  const remedyExecutionEnabled = isMobileAgentEconomyRemedyExecutionEnabled();
  const query = useQuery({
    queryKey: ['mobile-v7', 'owner', ownerScope ?? 'no-owner', 'economy-workflow', soulCoreId ?? 'unresolved', route.params.actionId],
    queryFn: () => client.getWorkflow(soulCoreId as string, route.params.actionId),
    enabled: enabled && !!ownerScope && !!soulCoreId && !!route.params.actionId,
    retry: 0,
  });
  ...
  React.useEffect(() => {
    if (
      !paymentCheckpointScope
      || !paymentCheckpoint
      || paymentCheckpoint.state === 'recovery_blocked'
      || !workflow
      || !['settled', 'refunded', 'reversed'].includes(workflow.workflowStatus)
    ) return;
    let active = true;
    void clearMobileEconomyPaymentCheckpoint(
      paymentCheckpointScope,
      paymentCheckpoint.idempotencyKey,
    ).then(() => {
      if (active) setPaymentCheckpoint(null);
    }).catch(() => {
      if (active) setError({
        kind: 'payment_recovery_blocked',
        title: 'Payment checkpoint cleanup requires review',
        detail: 'Canonical state is terminal, but the durable payment checkpoint could not be safely cleared.',
        uncertain: true,
      });
    });
    return () => { active = false; };
  }, [paymentCheckpoint, paymentCheckpointScope, workflow]);
  ...
  const paidAmount = isMobilePaidQuote(workflow?.quote);
  const authorityReceiptId = workflow?.paymentAttempts
    .filter((attempt) => attempt.status === 'succeeded')
    .flatMap((attempt) => attempt.authorityReceiptRefs)
    .find((ref) => ref.type === 'settlement')?.id;
  const hasCanonicalPaymentAttempt = !!workflow?.paymentAttempts.some(
    (attempt) => ['pending', 'unknown_outcome', 'succeeded'].includes(attempt.status),
  );
  const checkpointMatchesQuote = !paymentCheckpoint
    || paymentCheckpoint.quoteId === workflow?.quote?.quoteId;
  const canRecoverPayment = paidExecutionEnabled
    && paymentCheckpointLoaded
    && paidAmount
    && isMobileEconomyMandateActive(workflow?.mandate)
    && workflow?.reservation?.status === 'reserved'
    && checkpointMatchesQuote
    && paymentCheckpoint?.state === 'intent_persisted'
    && !mutating;
  const canPay = paidExecutionEnabled
    && paymentCheckpointLoaded
    && paidAmount
    && isMobileEconomyMandateActive(workflow?.mandate)
    && workflow?.reservation?.status === 'reserved'
    ...
    && paymentCheckpoint?.state !== 'intent_persisted'
    && paymentCheckpoint?.state !== 'recovery_blocked'
    && (!!paymentCheckpoint?.proof || (!!owner?.walletAddress && !hasCanonicalPaymentAttempt))
    && !mutating;
  const canRequestRemedy = remedyExecutionEnabled
    && (workflow?.workflowStatus === 'settled'
      || workflow?.workflowStatus === 'reconciliation_required')
    && !!authorityReceiptId
    && !mutating;

  const refresh = async () => {
    if (!ownerScope || !soulCoreId) return;
    setError(undefined);
  ...
  const mutateAuthority = async (operation: 'revoke' | 'release') => {
    ...
    setError(undefined);
    try {
      if (!paymentCheckpointLoaded || !paymentCheckpointScope) {
        setError({
          kind: 'payment_recovery_blocked',
          title: 'Payment checkpoint is still loading',
          detail: 'Authority changes remain blocked until durable payment state has been loaded.',
          uncertain: true,
        });
        return;
      }
      // Re-read durable storage inside the mutation fence. React state alone is
      // insufficient because the mount-time AsyncStorage read may race a tap.
      let durableCheckpoint: MobileEconomyPaymentCheckpoint | null;
      try {
        durableCheckpoint = await loadMobileEconomyPaymentCheckpoint(paymentCheckpointScope);
      } catch {
        setError({
          kind: 'payment_recovery_blocked',
          title: 'Payment checkpoint could not be verified',
          detail: 'Authority changes remain blocked until durable payment state can be read.',
          uncertain: true,
        });
        return;
      }
      if (durableCheckpoint) {
        setPaymentCheckpoint(durableCheckpoint);
        setError({
          kind: 'payment_recovery_blocked',
          title: 'Payment reconciliation required',
          detail: 'A durable wallet payment checkpoint exists. Revoke and release are blocked until chain and authority state are reconciled.',
          uncertain: true,
        });
        return;
      }
  ...
  const executePaidAction = async () => {
    ...
    try {
      let latest = await client.getWorkflow(soulCoreId, route.params.actionId);
      if (latest.workflowStatus === 'settled') {
        if (checkpoint && checkpoint.state !== 'recovery_blocked') {
          await clearMobileEconomyPaymentCheckpoint(
            paymentCheckpointScope,
      ...
      if (
        !isMobilePaidQuote(latest.quote)
        || !isMobileEconomyMandateActive(latest.mandate)
        || latest.reservation?.status !== 'reserved'
      ) {
        setOverride(latest);
        setError({
          kind: 'payment_not_ready',
          title: 'Payment is not ready',
          detail: 'A live paid quote, active mandate and reserved allocation are required.',
          uncertain: false,
        });
        return;
      }
      if (checkpoint?.state === 'recovery_blocked'
        || (checkpoint && checkpoint.quoteId !== latest.quote.quoteId)) {
        setError({
          kind: 'payment_recovery_blocked',
          title: 'Payment recovery requires review',
          detail: 'A durable payment checkpoint is corrupt or belongs to another quote. The wallet remains blocked.',
          uncertain: true,
        });
        return;
      }

      const paymentOption = extractX402PaymentOption(
        latest.quote.paymentRequirements,
        latest.quote.amount,
      );
      if (!paymentOption) {
        setError({
          kind: 'payment_route_unsupported',
          title: 'Payment route unavailable',
          detail: 'The canonical amount, asset, decimals or explicit testnet route is not supported by this wallet build.',
          uncertain: false,
        });
        return;
      }

      if (checkpoint?.state === 'intent_persisted') {
        const recovered = await getManagedTransactionByIdempotencyKey(
          checkpoint.idempotencyKey,
        );
        if (
          recovered?.txHash
          && (recovered.status === 'submitted' || recovered.status === 'confirmed')
        ) {
          checkpoint = {
            ...checkpoint,
            walletInvocationId: recovered.invocationId,
          ...
          await saveMobileEconomyPaymentCheckpoint(checkpoint);
          setPaymentCheckpoint(checkpoint);
        } else {
          setError({
            kind: 'payment_recovery_blocked',
            title: 'Wallet invocation is still unresolved',
            detail: recovered?.reason
              ?? 'The durable wallet invocation has no transaction hash. Do not pay again.',
            uncertain: true,
          });
          return;
        }
      ...
      if (!checkpoint) {
        const canonicalAttemptExists = latest.paymentAttempts.some(
          (attempt) => ['pending', 'unknown_outcome', 'succeeded'].includes(attempt.status),
        );
        if (canonicalAttemptExists) {
          setOverride(latest);
          setError({
            kind: 'payment_recovery_blocked',
            title: 'Canonical payment requires review',
            detail: 'The backend already records a payment attempt but this device has no durable proof. The wallet remains blocked.',
            uncertain: true,
          });
          return;
        }
        ...
        if (!walletAddress) {
          setError({
            kind: 'wallet_payment_failed',
            title: 'User wallet required',
            detail: 'A bound wallet is required before creating a payment intent.',
            uncertain: false,
          });
          return;
        }
        ...
        checkpoint = {
          schemaVersion: 1,
          ...paymentCheckpointScope,
          quoteId: latest.quote.quoteId,
          idempotencyKey: createMobileEconomyIdempotencyKey(`payment-${latest.actionId}`),
          state: 'intent_persisted',
          createdAt: now,
          updatedAt: now,
        };
        // This write must finish before invoking the wallet. Failure leaves the
        // wallet untouched and therefore cannot create an untracked transfer.
        await saveMobileEconomyPaymentCheckpoint(checkpoint);
        setPaymentCheckpoint(checkpoint);

        const payer = makeUserWalletX402Payer({
          walletAddress,
          userId: ownerScope,
          agentId: route.params.agentId,
        });
        const paid = await payer({
          ...
          token: paymentOption.usdcToken,
          to: paymentOption.payTo,
          amountHuman: paymentOption.amountHuman,
          idempotencyKey: checkpoint.idempotencyKey,
        });
        if (!paid.txHash || paid.status === 'failed') {
          // Keep the pre-wallet checkpoint. A reported failure does not prove
          // that no chain transaction escaped before a process/transport fault.
          setError({
            kind: 'wallet_payment_failed',
            title: 'Wallet outcome requires review',
            detail: paid.reason ?? 'No transaction hash was returned. The wallet is blocked from paying again.',
            uncertain: true,
          });
          return;
        }
        checkpoint = {
          ...checkpoint,
          walletInvocationId: paid.invocationId,
          state: 'proof_persisted',
          proof: {
            txHash: paid.txHash,
            network: paymentOption.network,
      ...
      if (checkpoint.state !== 'proof_persisted' || !checkpoint.proof) {
        setError({
          kind: 'payment_recovery_blocked',
          title: 'Payment proof unavailable',
          detail: 'A pre-wallet checkpoint exists without a durable transaction proof. Do not pay again; reconcile manually.',
          uncertain: true,
        });
        return;
      }
      const outcome = await runMobileEconomyMutation(() => client.executePayment(
        soulCoreId,
        latest,
      ...
      // The wallet transfer may already be final. Never pay again and never
      // generate a new key; only read canonical state after an uncertain submit.
      const mutationError = outcome.error;
      try {
        latest = await client.getWorkflow(soulCoreId, route.params.actionId);
        setOverride(latest);
      ...
      setError(describeEconomyClientError(paymentError, checkpoint !== null));
    } finally {
      pendingRef.current = false;
      setMutating(undefined);
    }
  ...
  const requestRemedy = async (kind: 'refund' | 'reversal') => {
    if (pendingRef.current || !ownerScope || !soulCoreId || !remedyExecutionEnabled) return;
    pendingRef.current = true;
    setMutating(kind);
    setError(undefined);
    try {
      let latest = await client.getWorkflow(soulCoreId, route.params.actionId);
      const authorityReceiptId = latest.paymentAttempts
        .filter((attempt) => attempt.status === 'succeeded')
        .flatMap((attempt) => attempt.authorityReceiptRefs)
        .find((ref) => ref.type === 'settlement')?.id;
      if (
        !authorityReceiptId
        || (latest.workflowStatus !== 'settled'
          && latest.workflowStatus !== 'reconciliation_required')
      ) {
        setOverride(latest);
        setError({
          kind: 'remedy_not_ready',
          title: 'Remedy is not ready',
          detail: 'A canonical settled payment receipt is required.',
          uncertain: false,
        });
        return;
      }
      const keyRef = kind === 'refund' ? refundKeyRef : reversalKeyRef;
      keyRef.current ??= createMobileEconomyIdempotencyKey(`${kind}-${latest.actionId}`);
      const outcome = await runMobileEconomyMutation(() => kind === 'refund'
        ? client.requestRefund(
            soulCoreId,
            latest,
            authorityReceiptId,
            keyRef.current!,
            'mobile_owner_requested',
          )
        : client.requestReversal(
            soulCoreId,
            latest,
            authorityReceiptId,
            keyRef.current!,
      ...
      if (outcome.ok === true) {
        setOverride(outcome.result.workflow);
        return;
      }
      const mutationError = outcome.error;
      try {
      ...
      if (!['refunded', 'reversed'].includes(latest.workflowStatus)) {
        setError(mutationError);
      }
    } catch (remedyError) {
      setError(describeEconomyClientError(remedyError, true));
    } finally {
      pendingRef.current = false;
      setMutating(undefined);
    }
  ...
  return (
    <ScrollView style={styles.screen} contentContainerStyle={styles.content} refreshControl={<RefreshControl refreshing={query.isFetching} onRefresh={refresh} />} testID="economy-action-tracking-screen">
      <Text style={styles.pageTitle}>{paidAmount
        ? t({ en: 'Paid Action', zh: '付费行动' })
        : t({ en: 'Non-paid Action', zh: '非付费行动' })}</Text>
      ...
      {workflow ? (
        <>
          ...
          <SummaryRow label={t({ en: 'Payment', zh: '付款' })} value={zeroAmount
            ? t({ en: 'Not required (0 USD)', zh: '无需付款（0 USD）' })
            : workflow.workflowStatus === 'settled'
              ? t({ en: 'Canonical settlement confirmed', zh: 'Canonical 结算已确认' })
              : paidExecutionEnabled
                ? t({ en: 'User-wallet payment required', zh: '需要用户钱包付款' })
                : t({ en: 'Blocked by build gate', zh: '已被构建门禁阻断' })} warning={!zeroAmount && workflow.workflowStatus !== 'settled'} />
          <SummaryRow label={t({ en: 'Provider order', zh: 'Provider 订单' })} value={workflow.workflowStatus === 'settled'
            ? t({ en: 'See execution receipt below', zh: '请查看下方执行凭证' })
            : t({ en: 'Not yet fulfilled', zh: '尚未履约' })} />
          <SummaryRow label={t({ en: 'Authority receipt', zh: '权威付款凭证' })} value={shortId(authorityReceiptId)} />
          ...
          {paidAmount && !paidExecutionEnabled ? <View style={styles.blockedCard} testID="economy-paid-gate-disabled"><Text style={styles.noticeTitle}>{t({ en: 'Paid execution disabled', zh: '付费执行已关闭' })}</Text><Text style={styles.noticeText}>{t({ en: 'This build cannot sign or submit a payment.', zh: '此构建不能签名或提交付款。' })}</Text></View> : null}
          {paidAmount && paidExecutionEnabled && !owner?.walletAddress && paymentCheckpoint?.state !== 'intent_persisted' ? <View style={styles.blockedCard} testID="economy-wallet-unavailable"><Text style={styles.noticeTitle}>{t({ en: 'User wallet required', zh: '需要用户钱包' })}</Text><Text style={styles.noticeText}>{t({ en: 'A bound MPC wallet address and local signing shard are required.', zh: '需要已绑定的 MPC 钱包地址和本地签名分片。' })}</Text></View> : null}
          {paymentCheckpoint && workflow.workflowStatus !== 'settled' ? <View style={styles.notice} testID="economy-payment-reconciliation"><Text style={styles.noticeTitle}>{paymentCheckpoint.state === 'proof_persisted' ? t({ en: 'Payment submitted · reconciliation required', zh: '付款已提交 · 需要对账' }) : paymentCheckpoint.state === 'intent_persisted' ? t({ en: 'Payment recovery available', zh: '可恢复付款状态' }) : t({ en: 'Payment attempt fenced · manual verification required', zh: '付款尝试已封锁 · 需要人工核验' })}</Text><Text style={styles.noticeText}>{t({ en: 'The wallet will not pay again. Refresh canonical state using the same Action and idempotency key.', zh: '钱包不会再次付款；请使用同一 Action 和幂等键刷新 canonical 状态。' })}</Text>{paymentCheckpoint.proof ? <Text style={styles.helper}>{shortId(paymentCheckpoint.proof.txHash)}</Text> : null}</View> : null}
          {canRecoverPayment ? <TouchableOpacity style={styles.primaryButton} disabled={!!mutating} onPress={executePaidAction} testID="economy-recover-payment"><Text style={styles.primaryButtonText}>{mutating === 'pay' ? t({ en: 'Recovering…', zh: '正在恢复…' }) : t({ en: 'Recover payment', zh: '恢复付款' })}</Text></TouchableOpacity> : null}
          {canPay ? <TouchableOpacity style={styles.primaryButton} disabled={!!mutating} onPress={executePaidAction} testID="economy-execute-payment"><Text style={styles.primaryButtonText}>{mutating === 'pay' ? t({ en: 'Paying…', zh: '正在付款…' }) : t({ en: `Pay ${formatMoney(workflow.quote?.amount)} with wallet`, zh: `使用钱包支付 ${formatMoney(workflow.quote?.amount)}` })}</Text></TouchableOpacity> : null}
          {(workflow.workflowStatus === 'settled' || workflow.workflowStatus === 'reconciliation_required') && !remedyExecutionEnabled ? <Text style={styles.helper} testID="economy-remedy-gate-disabled">{t({ en: 'Refund/reversal mutations are disabled in this build.', zh: '此构建已关闭退款/冲正 mutation。' })}</Text> : null}
          {canRequestRemedy ? <View style={styles.badgeRow} testID="economy-remedy-actions"><TouchableOpacity style={styles.secondaryButton} disabled={!!mutating} onPress={() => requestRemedy('refund')} testID="economy-request-refund"><Text style={styles.secondaryButtonText}>{mutating === 'refund' ? t({ en: 'Requesting…', zh: '正在请求…' }) : t({ en: 'Request refund', zh: '申请退款' })}</Text></TouchableOpacity><TouchableOpacity style={styles.secondaryButton} disabled={!!mutating} onPress={() => requestRemedy('reversal')} testID="economy-request-reversal"><Text style={styles.secondaryButtonText}>{mutating === 'reversal' ? t({ en: 'Requesting…', zh: '正在请求…' }) : t({ en: 'Request reversal', zh: '申请冲正' })}</Text></TouchableOpacity></View> : null}
          {workflow.workflowStatus === 'reserved' && zeroAmount ? <View style={styles.notice}><Text style={styles.noticeTitle}>{t({ en: 'Authorized for provider handoff', zh: '已授权，可交给服务方' })}</Text><Text style={styles.noticeText}>{t({ en: 'The 0 USD authority loop is complete. A canonical execution receipt is still required before claiming execution success.', zh: '0 USD 授权闭环已完成；在声明执行成功前，仍需要 canonical execution receipt。' })}</Text></View> : null}
          ...
          {workflow.mandate?.status === 'active' && workflow.reservation?.status === 'reserved' && paymentCheckpointLoaded && !authorityReceiptId && !paymentCheckpoint ? <TouchableOpacity style={styles.dangerButton} disabled={!!mutating} onPress={() => mutateAuthority('revoke')} testID="economy-revoke"><Text style={styles.dangerButtonText}>{mutating === 'revoke' ? t({ en: 'Revoking…', zh: '正在撤销…' }) : t({ en: 'Revoke authority', zh: '撤销授权' })}</Text></TouchableOpacity> : null}
          {reservationExpired && workflow.reservation?.status === 'reserved' && paymentCheckpointLoaded && !paymentCheckpoint ? <TouchableOpacity style={styles.secondaryButton} disabled={!!mutating} onPress={() => mutateAuthority('release')} testID="economy-timeout-release"><Text style={styles.secondaryButtonText}>{mutating === 'release' ? t({ en: 'Releasing…', zh: '正在释放…' }) : t({ en: 'Release expired reservation', zh: '释放过期 Reservation' })}</Text></TouchableOpacity> : null}
        </>
      ) : null}
      <CanonicalActionReceiptPanel soulCoreId={soulCoreId} actionId={route.params.actionId} />
      <TrustActionTimelineCard soulCoreId={soulCoreId} actionId={route.params.actionId} />
      <TouchableOpacity style={[styles.primaryButton, (!ownerScope || !!mutating) && styles.disabled]} disabled={!ownerScope || !!mutating} onPress={refresh}><Text style={styles.primaryButtonText}>{t({ en: 'Reconcile canonical state', zh: '重新对账 canonical 状态' })}</Text></TouchableOpacity>
...
function ActionRuntimeTrackingScreen({ route, ownerScope }: any) {
  const { t } = useI18n();
  const styles = useThemedStyles(makeStyles);
  const directory = useMobileAgentDirectory(route.params.agentId);
  const soulCoreId = directory.model.agents
    .find((agent) => agent.agentId === route.params.agentId)?.soulCoreId;
  const { state, query } = useActionRead(ownerScope, soulCoreId, route.params.actionId);
  ...
  return (
    <ScrollView style={styles.screen} contentContainerStyle={styles.content} refreshControl={<RefreshControl refreshing={query.isFetching} onRefresh={() => query.refetch()} />} testID="action-tracking-screen">
      ...
      <CanonicalActionReceiptPanel soulCoreId={soulCoreId} actionId={route.params.actionId} />
      <TrustActionTimelineCard soulCoreId={soulCoreId} actionId={route.params.actionId} />
      <TouchableOpacity style={[styles.primaryButton, !ownerScope && styles.disabled]} disabled={!ownerScope} onPress={() => query.refetch()}><Text style={styles.primaryButtonText}>{t({ en: 'Reconcile canonical state', zh: '重新对账 canonical 状态' })}</Text></TouchableOpacity>
...
export function ActionTrackingScreen(props: any) {
  const ownerScope = useAuthStore((state) => state.user?.id ?? null);
  const screenKey = `${ownerScope ?? 'no-owner'}:${props.route.params?.actionId ?? 'missing'}`;
  return props.route.params?.origin === 'economy'
    ? <EconomyActionTrackingScreen key={`economy:${screenKey}`} {...props} ownerScope={ownerScope} />
    : <ActionRuntimeTrackingScreen key={`runtime:${screenKey}`} {...props} ownerScope={ownerScope} />;
}
...
export function ActionsHomeScreen({ navigation, route }: any) {
  ...
  const query = useQuery({
    queryKey: context.kind === 'ready'
      ? ['mobile-v7', 'owner', ownerScope ?? 'no-owner', ...context.context.scope.queryKey, 'actions']
      : ['mobile-v7', 'owner', ownerScope ?? 'no-owner', 'actions', 'unresolved'],
    queryFn: () => directory.facade.listActions(soulCoreId as string, { enabled }),
    enabled: !!ownerScope && !!soulCoreId,
    retry: 0,
  });
  const state: MobileReadState<ActionTaskListV1> = query.data ?? (!enabled ? { kind: 'unavailable', capability: 'action.list_v1', reason: 'feature_disabled' } : !ownerScope ? { kind: 'unauthorized', reason: 'owner_session_missing' } : !soulCoreId ? { kind: 'unavailable', capability: 'action.list_v1', reason: 'agent_context_unresolved' } : { kind: 'unknown', reason: 'loading' });
  return (
