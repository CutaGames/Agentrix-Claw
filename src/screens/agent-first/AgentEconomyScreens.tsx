...
function GoalComposerOwnerScreen({ navigation, route, ownerScope }: any) {
  ...
  return (
    <ScrollView style={styles.screen} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" testID="goal-composer-screen">
      ...
      <View style={styles.softCard} testID="safe-v1-zero-usd-boundary"><Text style={styles.cardTitle}>{t({ en: 'First release boundary', zh: '首版边界' })}</Text><Text style={styles.muted}>{t({ en: 'Live skill, task, resource, product, service and Agent candidates; only canonical 0 USD quotes can be authorized. Paid execution stays disabled.', zh: '实时发现技能、任务、资源、产品、服务和 Agent 候选；仅允许授权 canonical 0 USD 报价，付费执行保持关闭。' })}</Text></View>
