...
export function WorldUgcRuleSetsScreen() {
  ...
  const onPlay = useCallback((rs: WorldGameRuleSet) => {
    // 用"我的玩法"开打:去对战选择页挑角色,把玩法规则透传到对战(后端据 shareCode 应用规则)。
    navigation.navigate('WorldBattlePicker', {
      ruleSetShareCode: rs.shareCode,
      ruleSetName: rs.name,
    });
  }, [navigation]);
        ...
        items.map((rs) => (
          <View key={rs.id} style={styles.ruleCard}>
            ...
            <View style={styles.ruleActions}>
              <TouchableOpacity style={styles.playBtn} onPress={() => onPlay(rs)}>
                <Text style={styles.playBtnText}>▶️ 开打</Text>
              </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  playBtn: { backgroundColor: '#22c55e', borderRadius: 8, paddingVertical: 6, paddingHorizontal: 16 },
