...
export default function MyWorldScreen() {
  ...
  const onPublish = useCallback(
    (c: Creation) => {
      Alert.alert(
        t({ en: 'Publish creation', zh: '发布创作' }),
        t({
          en: `Run moderation and the quality gate for "${c.title}"?`,
          zh: `对「${c.title}」执行审核与质量门并发布吗？`,
        }),
        [
          { text: t({ en: 'Cancel', zh: '取消' }), style: 'cancel' },
          {
            text: t({ en: 'Publish', zh: '发布' }),
            onPress: async () => {
              setPublishingId(c.id);
              try {
                const result = await publishCreation(c.id);
                if (!result.published) {
                  Alert.alert(
                    t({ en: 'Publish blocked', zh: '发布被拦截' }),
                    result.error?.detail ?? t({ en: 'Review or quality gate rejected this Creation.', zh: '审核或质量门未通过。' }),
                  );
                  return;
                }
                await load();
      ...
      {loading ? (
        ...
        mine.map((c) => (
          <View key={c.id} style={styles.creationRow}>
            ...
            {(c.status === 'draft' || c.status === 'unpublished') ? (
              <TouchableOpacity
                style={[styles.publishBtn, publishingId === c.id && styles.btnDisabled]}
                onPress={() => onPublish(c)}
                disabled={publishingId === c.id}
                testID={`my-world-publish-${c.id}`}
              >
                <Text style={styles.publishText}>
                  {publishingId === c.id ? '…' : t({ en: 'Publish', zh: '发布' })}
                </Text>
              </TouchableOpacity>
...
const styles = themedStyles(() => StyleSheet.create({
  ...
  publishBtn: { marginLeft: 8, backgroundColor: colors.accent, borderRadius: 8, paddingHorizontal: 12, paddingVertical: 7 },
