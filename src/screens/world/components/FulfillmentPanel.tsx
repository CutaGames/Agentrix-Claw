            ...
            {(o.fulfillmentType === 'agent' || o.fulfillmentType === 'manual') && o.status === 'paid' ? (
              <Text style={styles.hint}>
                {o.fulfillmentType === 'agent'
                  ? t({ zh: '🤖 Agent 履约进行中；Mobile 完成/放款入口在持久化幂等门通过前保持关闭。', en: '🤖 Agent fulfillment is running; Mobile completion/release stays closed until the durable idempotency gate passes.' })
                  : t({ zh: '✍️ 待手动交付；Mobile 完成/放款入口在交付证据与持久化幂等门通过前保持关闭。', en: '✍️ Awaiting manual delivery; Mobile completion/release stays closed until evidence and durable idempotency gates pass.' })}
              </Text>
            ) : null}
          </View>
        );
