...
export default function WorldInteractiveBattleScreen() {
  ...
  const [maxHp, setMaxHp] = useState<{ challenger: number; defender: number }>({ challenger: 100, defender: 100 });
  const [skills, setSkills] = useState<{ name: string; damageBase?: number; skillIndex: number }[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const r = training
          ? await startTrainingBattle({ challengerAssetId, difficulty: difficulty ?? 'normal', ruleSetShareCode })
          : await startInteractiveBattle({ challengerAssetId, defenderAssetId, ruleSetShareCode });
        if (cancelled) return;
        setBattleId(r.battleId);
        setState(r.state);
        // 记录初始 HP 作为血条满值
        setMaxHp({ challenger: r.state?.challenger?.hp ?? 100, defender: r.state?.defender?.hp ?? 100 });
        // 取攻击型技能,保留"原始下标"(后端 stepRound 用的是完整技能列表的 index)。
        // 之前用过滤后数组的 index 会错位(选 Surge 实际打成 Guard)。
        const offensive = (r.challengerSkills || [])
          .map((s, idx) => ({ name: s.name, damageBase: s.damageBase ?? 0, skillIndex: idx }))
          .filter((s) => s.damageBase > 0);
        setSkills(
          offensive.length > 0
            ? offensive
            : [{ name: '基础攻击', damageBase: 10, skillIndex: 0 }],
        );
      } catch (e: any) {
    ...
    return () => { cancelled = true; };
  }, [challengerAssetId, defenderAssetId, training, difficulty, ruleSetShareCode]);
  ...
  const onAction = useCallback(
    (action: BattleActionType) => {
      if (action === 'attack') {
        ...
        submit({ action: 'attack', skillIndex: skills[0]?.skillIndex ?? 0 });
      } else {
        submit({ action });
      }
    },
  ...
  // 最近一回合,用于伤害飘字 + 受击闪烁(让回合有"打中了"的反馈)。
  const lastRound = log[0];
    ...
    <View style={styles.container}>
      ...
      <View style={styles.combatants}>
        <Combatant
          ...
          fallbackEmoji="🦊"
          hp={c?.hp ?? 0}
          maxHp={maxHp.challenger}
          energy={c?.energy ?? 0}
          charge={c?.charge ?? 0}
          defending={c?.defending}
          side="me"
          hitFlash={!!lastRound && lastRound.defenderDamageDealt > 0}
          popDamage={lastRound?.defenderDamageDealt ?? null}
          popCrit={lastRound?.defenderCrit}
        />
        ...
        <Combatant
          ...
          hp={d?.hp ?? 0}
          maxHp={maxHp.defender}
          energy={d?.energy ?? 0}
          charge={d?.charge ?? 0}
          defending={d?.defending}
          side="foe"
          hitFlash={!!lastRound && lastRound.challengerDamageDealt > 0}
          popDamage={lastRound?.challengerDamageDealt ?? null}
          popCrit={lastRound?.challengerCrit}
        />
      </View>
...
function Combatant({ label, portraitUrl, spriteClan, fallbackEmoji, hp, maxHp, energy, charge, defending, side, hitFlash, popDamage, popCrit }: {
  label: string; portraitUrl?: string | null; spriteClan?: 'A' | 'B' | 'C' | 'D' | 'E' | 'F'; fallbackEmoji: string; hp: number; maxHp: number; energy: number; charge: number; defending?: boolean; side: 'me' | 'foe'; hitFlash?: boolean; popDamage?: number | null; popCrit?: boolean;
}) {
  const hpPct = Math.max(0, Math.min(100, maxHp > 0 ? (hp / maxHp) * 100 : 0));
  const hpColor = hpPct > 50 ? '#22c55e' : hpPct > 20 ? '#f59e0b' : '#ef4444';
  return (
    <View style={[styles.combatant, hitFlash && styles.combatantHit]}>
      <View style={styles.combatantAvatar}>
        ...
        {popDamage != null && popDamage > 0 ? (
          <Text style={[styles.dmgPop, popCrit && styles.dmgPopCrit]}>-{popDamage}{popCrit ? '!' : ''}</Text>
        ) : null}
      </View>
      <Text style={styles.combatantLabel} numberOfLines={1}>{label}</Text>
      <View style={styles.hpBarTrack}>
        <View style={[styles.hpBarFill, { width: `${hpPct}%`, backgroundColor: hpColor }]} />
      </View>
      <Text style={styles.hpText}>HP {Math.max(0, hp)}</Text>
...
const styles = StyleSheet.create({
  ...
  combatantHit: { backgroundColor: '#3a1a1e', borderWidth: 1, borderColor: 'rgba(239,68,68,0.5)' },
  ...
  dmgPop: { position: 'absolute', top: -14, alignSelf: 'center', color: '#ef4444', fontSize: 18, fontWeight: '900', textShadowColor: '#000', textShadowRadius: 3 },
  dmgPopCrit: { color: '#f59e0b', fontSize: 22 },
  combatantEmoji: { fontSize: 36 },
  combatantLabel: { color: '#aaa', fontSize: 12, marginTop: 2 },
  hpBarTrack: { width: '90%', height: 6, borderRadius: 3, backgroundColor: '#2d2d44', overflow: 'hidden', marginTop: 6 },
