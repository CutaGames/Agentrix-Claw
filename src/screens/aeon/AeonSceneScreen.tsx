...
export default function AeonSceneScreen() {
  ...
  /** 可选:从功能建筑(餐厅/会场)进来时直接指定房间 id,绕过"取地块默认房间"。 */
  const explicitRoomId: string | undefined = route.params?.roomId;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        // 显式房间(从功能建筑进入)优先;否则取/建该地块的默认 public 房间。
        let room: { id: string; kind?: string } | undefined;
        if (explicitRoomId) {
          try {
            const r = await getRoomWithPresence(explicitRoomId);
            room = { id: r.id, kind: (r as any).kind };
          } catch {
            room = undefined;
          }
        }
  ...
  }, [plotId, explicitRoomId, mySnapshot, applyServerEvent, startPolling]);
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.actionBar} contentContainerStyle={styles.actionBarContent}>
        <TouchableOpacity style={styles.actionChip} onPress={() => navigation.navigate('AeonPlaza')}>
          <Text style={styles.actionEmoji}>🎪</Text>
          <Text style={styles.actionLabel}>公共广场</Text>
          <Text style={styles.actionHint}>全服实时群聊</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.actionChip} onPress={() => navigation.navigate('AeonEvents')}>
          <Text style={styles.actionEmoji}>🎤</Text>
          <Text style={styles.actionLabel}>活动现场</Text>
          <Text style={styles.actionHint}>脱口秀/预约/打赏</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.actionChip} onPress={() => navigation.navigate('ConnectorHub')}>
          <Text style={styles.actionEmoji}>🔌</Text>
          <Text style={styles.actionLabel}>连接器</Text>
          <Text style={styles.actionHint}>派agent办真事赚AXP</Text>
        </TouchableOpacity>
        ...
        <TouchableOpacity style={styles.actionChip} onPress={() => navigation.navigate('AeonCompany')}>
          <Text style={styles.actionEmoji}>🏢</Text>
          <Text style={styles.actionLabel}>公司运营</Text>
          <Text style={styles.actionHint}>开公司/雇agent/发薪</Text>
        </TouchableOpacity>
