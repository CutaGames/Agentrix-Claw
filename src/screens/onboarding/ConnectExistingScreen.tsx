...
} from '../../services/openclaw-migration-outcome';
...
export function ConnectExistingScreen() {
  ...
  const handleRunMigration = useCallback(async () => {
    // `loading` also guards a same-frame double tap on the retry button.
    if (!boundInstanceId || loading) return;
    setLoading(true);
    ...
    try {
      ...
      setMigrationOutcome(deriveMigrationOutcome(result));
      Animated.timing(migrateProgress, { toValue: 100, duration: 400, useNativeDriver: false }).start();
      setStep('done');
    } catch (err: any) {
      // The instance is bound, but the read did not complete. Never advance the
      // progress bar to 100% and never fall through to the success copy: the
      // server-side outcome is unknown, so this is reported as a failure the
      // user can retry.
      setMigrationResult(null);
      setMigrationOutcome('failed');
      setMigrationError(presentableMigrationError(err?.message));
      Animated.timing(migrateProgress, { toValue: 60, duration: 200, useNativeDriver: false }).start();
      setStep('done');
    } finally {
      setLoading(false);
    }
  }, [boundInstanceId, loading, migrateProgress]);
  ...
  const handleRetryMigration = useCallback(() => {
    if (!boundInstanceId || loading) return;
    // Clear the previous outcome and its result together, so a retry in flight
    // can never render last attempt's counts under this attempt's header.
    setMigrationOutcome(null);
    setMigrationResult(null);
    setMigrationError(null);
    handleRunMigration();
  }, [boundInstanceId, loading, handleRunMigration]);
  ...
  // While a retry is in flight the outcome is cleared, so the copy falls back to
  // the in-progress descriptor rather than showing the previous attempt's result.
  const outcomeCopy = describeMigrationOutcome(loading ? null : migrationOutcome);
  // Shown for partial *and* nothing_read: which categories returned nothing is
  // the actionable detail. Empty for succeeded, and for failed there is no result.
  const incompleteCategories = loading ? [] : incompleteMigrationCategories(migrationResult);
      ...
      {step === 'discover' && (
        <View>
          ...
          <Text style={styles.subtitle}>
            将您现有的 OpenClaw 实例接入 Agentrix。绑定与数据迁移是两个步骤，迁移结果会逐类别显示。
          </Text>
      ...
      {step === 'done' && (
        <View>
          <View style={styles.successBadge}>
            <Text style={styles.successIcon}>{outcomeCopy.icon}</Text>
          </View>
          <Text style={styles.title}>{outcomeCopy.title}</Text>
          <Text style={styles.subtitle}>{outcomeCopy.body}</Text>

          {migrationOutcome === 'failed' && migrationError && (
            <View style={styles.migrationNotice}>
              <Text style={styles.migrationNoticeTitle}>失败原因</Text>
              <Text style={styles.migrationNoticeBody}>{migrationError}</Text>
            </View>
          )}

          {incompleteCategories.length > 0 && (
            <View style={styles.migrationNotice}>
              <Text style={styles.migrationNoticeTitle}>以下类别未读取到内容</Text>
              {incompleteCategories.map((entry) => (
                <Text key={entry.category} style={styles.migrationNoticeBody}>
                  {`• ${migrationCategoryLabel(entry.category)}${
                    entry.status === 'skipped' ? '（源实例未提供或读取失败）' : ''
                  }`}
                </Text>
              ))}
            </View>
          ...
          {outcomeCopy.showsReadCounts && migrationResult && (
            <View style={styles.summaryGrid}>
              {[
                { icon: '⚡', label: '读取到技能', value: String(migrationResult.skills.length) },
                { icon: '💾', label: '读取到记忆', value: String(migrationResult.memoryEntries.length) },
                { icon: '💬', label: '读取到对话', value: String(migrationResult.sessionSummaries.length) },
                { icon: '🔧', label: '读取到配置', value: migrationResult.config ? '是' : '否' },
              ].map((t) => (
          ...
          {outcomeCopy.offersRetry && (
            <TouchableOpacity
              style={[styles.primaryBtn, loading && styles.primaryBtnDisabled]}
              onPress={handleRetryMigration}
              disabled={loading}
            >
              <Text style={styles.primaryBtnText}>{loading ? '正在重试…' : '重试读取'}</Text>
            </TouchableOpacity>
          )}

          <TouchableOpacity
            style={migrationOutcome === 'succeeded' ? styles.primaryBtn : styles.secondaryBtn}
            onPress={handleFinish}
            disabled={loading}
          >
            <Text
              style={
                migrationOutcome === 'succeeded' ? styles.primaryBtnText : styles.secondaryBtnText
              }
            >
              {migrationOutcome === 'succeeded' ? '开始使用 →' : '跳过，先开始使用 →'}
            </Text>
          </TouchableOpacity>
        </View>
      )}
