...
export function TeamDashboardScreen({ navigation }: Props) {
  ...
  // MTR-R03 (2026-09-15): `DesktopControl` is registered in no navigator, so
  // the old `navigate('Agent', { screen: 'DesktopControl' })` was a silent
  // no-op. Until approvals land in the Work tab, say so out loud.
  const openApprovalSurface = useCallback(() => {
    navigateToDestinationError(
      (navigation as any).getParent?.() ?? (navigation as any),
      'route_not_mounted',
    );
  }, [navigation]);
      ...
      >
        ...
        <View style={styles.opsPanel} testID="mobile-agent-ops-control">
          <View style={styles.opsHeader}>
            ...
            <TouchableOpacity style={styles.opsLinkBtn} onPress={openApprovalSurface}>
          ...
          {(operationsContinuity?.pendingApprovals || []).slice(0, 2).map((approval) => (
            <TouchableOpacity key={approval.approvalId} style={styles.opsItem} onPress={openApprovalSurface} activeOpacity={0.75}>
              <View style={{ flex: 1 }}>
