...
const X402_TOKEN_DECIMALS: Record<string, number> = {
  USDC: 6,
};

function atomicToDecimalString(atomic: string, decimals: number): string | null {
  if (!/^[1-9][0-9]*$/.test(atomic) || !Number.isInteger(decimals) || decimals < 0) {
    return null;
  }
...
export function extractX402PaymentOption(
  paymentRequirements: any,
  expected?: { amountMinor: string; currency: string; decimals: number },
): X402PaymentOption | null {
  if (
    paymentRequirements?.x402Version !== 1
    || paymentRequirements?.error !== null
    || !Array.isArray(paymentRequirements.accepts)
    || paymentRequirements.accepts.length !== 1
  ) return null;
  const accept = paymentRequirements.accepts[0];
  if (!accept || typeof accept !== 'object' || accept.scheme !== 'exact') return null;
  const network = String(accept.network ?? '').trim().toLowerCase();
  const payTo = String(accept.payTo ?? '').trim();
  const asset = String(accept.asset ?? '').trim().toUpperCase();
  const atomic = String(accept.maxAmountRequired ?? '').trim();
  if (!network || !payTo || !asset) return null;

  // Bare aliases such as `bsc`/`injective` are intentionally rejected: they
  // do not uniquely identify testnet vs mainnet and could route real funds to
  // the wrong chain.
  const chainId = X402_NETWORK_CHAIN[network];
  const decimals = X402_TOKEN_DECIMALS[asset];
  if (!chainId || decimals === undefined) return null;
  const chain = getLsmChain(chainId);
  if (!chain || asset !== 'USDC') return null;

  const amountHuman = atomicToDecimalString(atomic, decimals);
  if (!amountHuman) return null;
  if (expected && (
    expected.amountMinor !== atomic
    || expected.currency.trim().toUpperCase() !== asset
...
export async function payWithUserWalletAndReplay(
  ...
  const option = extractX402PaymentOption(pending.paymentRequirements);
  if (!option) {
