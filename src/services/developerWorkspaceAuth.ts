...
export type DeveloperWorkspaceAuthOptions = {
  ...
  online?: boolean;
  /**
   * Mobile's stable agent relation (`SoulCoreRefV1.agentAccountId`, a uuid).
   * Sent as the `agentAccountId` query on every request so the backend's
   * `claimedAgentFromRequest` → `resolveOwnerContext` can scope the owner's
   * tenant assignment; without any agent hint the six `/v1/developer/**`
   * routes answer 503 `canonical_tenant_agent_required` regardless of flags
   * (coordination board §0.35; REAL-checked against the :3011 canary on
   * 2026-09-17). It is a routing hint the backend still resolves and checks
   * against the JWT owner — not a caller-asserted identity, which is why it
   * is not one of `IDENTITY_HEADERS`.
   */
  agentAccountId?: string;
};

export const DEVELOPER_WORKSPACE_AGENT_ACCOUNT_QUERY = "agentAccountId";

/**
 * Headers the client must never send. `x-agent-id` is the backend's
 * `agentUniqueId` (`AGT-…`) hint; Mobile only holds the account uuid, and the
 * backend rejects a uuid there with 503 `canonical_tenant_agent_unknown`, so
 * Mobile scopes by query instead of ever asserting an agent id header.
 */
...
export function createDeveloperWorkspaceAuthTransport(
  ...
  const agentAccountId =
    typeof options.agentAccountId === "string" && options.agentAccountId.trim()
      ? options.agentAccountId.trim()
      : undefined;
