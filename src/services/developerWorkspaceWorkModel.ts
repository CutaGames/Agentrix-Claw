...
/**
 * Read-state snapshot for "we know which agent, but the DRW scope cannot be
 * established for it" — e.g. the owner directory has no `agentUniqueId` for
 * Mobile's `agentAccountId` (M2 slice A, board §0.35). Same eight-state card
 * as every other unresolved agent, never a blank screen.
 */
export function developerWorkspaceUnresolvedAgentSnapshot(
  resolution: Extract<WorkSurfaceAgentResolution, { ok: false }>,
): DeveloperWorkspaceSnapshot {
  return unresolvedAgentSnapshot(resolution);
}
