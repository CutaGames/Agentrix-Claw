...
import type {
  ...
  CreationOperatorStatusResponse,
  // §2 generate
...
/** Owner-scoped metadata edit; lifecycle transitions remain dedicated operations. */
export async function updateCreation(
  id: string,
  req: UpdateCreationRequest,
): Promise<UpdateCreationResponse> {
  return unified<UpdateCreationResponse>(`/${encodeURIComponent(id)}`, {
    method: 'PATCH',
    body: JSON.stringify(req),
  });
}

/** Authoritative owner operation status; clients must not infer unavailable actions. */
export async function getCreationOperatorStatus(
  id: string,
): Promise<CreationOperatorStatusResponse> {
  return unified<CreationOperatorStatusResponse>(
    `/${encodeURIComponent(id)}/operator-status`,
  );
}
...
export async function discoverCreations(
  ...
  if (USE_UNIFIED_CREATION_BACKEND) {
    return unified<DiscoverCreationsResponse>(`/discover${toQuery(query as unknown as Record<string, unknown>)}`);
  }
  ...
  return unified<DiscoverAgentSearchResponse>(`/discover${toQuery(query as unknown as Record<string, unknown>)}`);
}
...
export async function discoverCreationsPublic(
  ...
  return apiFetch<DiscoverCreationsResponse>(
    `${BASE}/public/discover${toQuery(query as unknown as Record<string, unknown>)}`,
  );
}
