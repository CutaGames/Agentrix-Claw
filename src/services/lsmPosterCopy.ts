...
export function buildWorldCupPosterCopy(input: WorldCupPosterInput): WorldCupPosterCopy {
  ...
  const description = zh
    ? `${statusText}${scorePart} · 🎁 新人限量前 ${b.limit} 名：注册即得 ${b.axp} AXP + ${b.usdc} USDC 站内体验金（测试网·可直接下注开仓）。扫码或复制链接注册即玩。`
    : `${statusText}${scorePart} · 🎁 First ${b.limit} sign-ups get ${b.axp} AXP + ${b.usdc} USDC in-app trial credits (testnet · ready to bet). Scan or copy the link to register & play.`;
...
export function worldCupHeroTagline(zh: boolean): string {
  const b = SIGNUP_BONUS_DISPLAY;
  return zh
    ? `🎁 新人限量前 ${b.limit} 名：注册即领 ${b.axp} AXP + ${b.usdc} USDC 站内体验金（测试网）`
    : `🎁 First ${b.limit} sign-ups: ${b.axp} AXP + ${b.usdc} USDC in-app trial credits (testnet)`;
}
