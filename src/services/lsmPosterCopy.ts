...
export function buildWorldCupPosterCopy(input: WorldCupPosterInput): WorldCupPosterCopy {
  ...
  // 利益点分开表述（AXP + USDC 各自列出，无固定兑换比例）。
  const description = zh
    ? `${statusText}${scorePart} · 🎁 新人限量前 ${b.limit} 名：注册即领 ${b.axp} AXP 与 ${b.usdc} USDC 体验金，直接下注开仓、体验杠杆赔率。扫码或复制链接注册即玩。`
    : `${statusText}${scorePart} · 🎁 First ${b.limit} sign-ups get ${b.axp} AXP and ${b.usdc} USDC trial credits — bet and open positions right away. Scan or copy the link to register & play.`;

  const ctaLabel = zh
    ? `扫码注册 · 领 ${b.axp} AXP + ${b.usdc} USDC`
    : `Scan to register · Get ${b.axp} AXP + ${b.usdc} USDC`;

  const categoryLabel = zh ? '世界杯' : 'World Cup';

  const tags = zh
    ? ['世界杯', `限量${b.limit}名`, '注册即玩', input.sport || 'soccer']
    : ['WorldCup', `First${b.limit}`, 'PlayFree', input.sport || 'soccer'];
...
export function worldCupHeroTagline(zh: boolean): string {
  const b = SIGNUP_BONUS_DISPLAY;
  return zh
    ? `🎁 新人限量前 ${b.limit} 名：注册即领 ${b.axp} AXP + ${b.usdc} USDC 体验金`
    : `🎁 First ${b.limit} sign-ups: ${b.axp} AXP + ${b.usdc} USDC trial credits`;
}
