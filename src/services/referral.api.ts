...
export const referralApi = {
  ...
  // 分享文案(纯函数;CreateLink 带目标信息,Promote / SkillDetail 走通用文案)
  generateShareText(target?: {
    name?: string;
    price?: number | string;
    priceUnit?: string;
    shortUrl?: string;
  }): string {
    const url = target?.shortUrl || 'https://agentrix.top';
    if (target?.name && target.price && target.priceUnit) {
      return `🔥 Check out "${target.name}" — only $${target.price}/${target.priceUnit}!\n👉 ${url}`;
    }
    if (target?.name) {
      return `🚀 Discover "${target.name}" on Agentrix — try it now!\n👉 ${url}`;
    }
    return `🚀 Join me on Agentrix — your AI agent economy.\n👉 ${url}`;
  },
