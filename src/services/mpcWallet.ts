...
export async function getManagedTransactionByIdempotencyKey(
  idempotencyKey: string,
): Promise<{
  invocationId?: string;
  txHash: string;
  ...
  if (!idempotencyKey || idempotencyKey !== idempotencyKey.trim()) {
    throw new Error('WALLET_IDEMPOTENCY_KEY_INVALID');
  }
  return apiFetch(
    `/mpc-wallet/sign-and-send/${encodeURIComponent(idempotencyKey)}`,
    { method: 'GET' },
  );
}
