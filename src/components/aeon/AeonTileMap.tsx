...
export function AeonTileMap({ width, height, gridSize = 7, roomKind, children }: AeonTileMapProps) {
  ...
  return (
    <View style={[styles.root, { width, height }]} pointerEvents="box-none">
      {tiles.map((t) => {
        // 略放大渲染并居中,消除等距砖之间的发丝级缝隙(透明边距导致)。
        const render = tilePx * 1.08;
        const pad = (render - tilePx) / 2;
        const left = cx0 + (t.x - t.y) * halfW - pad;
        const top = cy0 + (t.x + t.y) * quarterH - pad;
        return (
          <Image
            key={`${t.x}-${t.y}`}
            source={t.src}
            style={[styles.tile, { left, top, width: render, height: render }]}
            resizeMode="contain"
            // `pointerEvents` is honoured by the native Image view but is not
            // part of ImageProps in the current RN typings.
