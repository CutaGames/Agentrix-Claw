 ...
 * 由统一 `mobile.trust_loop` feature flag 门控。无卡、读取失败和证据不足严格区分；
 * 硬件保障与经济保障各自独立呈现，不并入行为信誉。只读。
 */
...
export function ReputationAssuranceCard({ soulCoreId, subjectKind = 'agent', subjectId, context }: ReputationAssuranceCardProps) {
  ...
  const repQ = useQuery({
    queryKey: ['trust-reputation-mobile', ownerScope ?? 'no-owner', soulCoreId, subjectKind, subjectId, context],
    queryFn: () => fetchReputationCard(soulCoreId as string, subjectKind, subjectId as string, context),
    enabled,
    retry: 0,
  });
  const asrQ = useQuery({
    queryKey: ['trust-assurance-mobile', ownerScope ?? 'no-owner', soulCoreId, subjectKind, subjectId],
  ...
  const card = repQ.data ?? null;
  const assurance = asrQ.data ?? null;

  return (
    <View style={styles.wrap} testID="trust-reputation-mobile">
      {repQ.isLoading ? (
        <View style={styles.card} testID="trust-reputation-mobile-loading">
          <ActivityIndicator />
          <Text style={styles.muted}>{t({ en: 'Reading canonical reputation…', zh: '正在读取 canonical 信誉…' })}</Text>
        </View>
      ) : repQ.isError ? (
        <View style={styles.card} testID="trust-reputation-mobile-error">
          <Text style={styles.title}>{t({ en: 'Contextual reputation', zh: '情境信誉' })}</Text>
          <Text style={styles.chipMuted}>unknown / unavailable</Text>
          <Text style={styles.muted}>
            {t({ en: 'Canonical reputation could not be verified. No score is inferred.', zh: '无法验证 canonical 信誉；不会推断任何分数。' })}
          </Text>
        </View>
      ) : card ? (
        <View style={styles.card} testID="trust-reputation-mobile-card">
          ...
          {card.dimensions.map((dimension) => (
            <Text key={dimension.dimension} style={styles.row}>
              {dimension.dimension}: {dimension.value === null ? t({ en: 'insufficient', zh: '样本不足' }) : `${dimension.value} (n=${dimension.sampleSize})`}
            </Text>
          ))}
        </View>
      ) : (
      ...
      {asrQ.isLoading ? (
        <View style={styles.card} testID="trust-assurance-mobile-loading">
          <ActivityIndicator />
          <Text style={styles.muted}>{t({ en: 'Reading canonical assurance…', zh: '正在读取 canonical 保障…' })}</Text>
        </View>
      ) : asrQ.isError ? (
        <View style={styles.card} testID="trust-assurance-mobile-error">
          <Text style={styles.title}>{t({ en: 'Assurance (not reputation)', zh: '保障画像（非行为信誉）' })}</Text>
          <Text style={styles.chipMuted}>unknown / unavailable</Text>
          <Text style={styles.muted}>{t({ en: 'Assurance data could not be verified.', zh: '保障数据无法验证。' })}</Text>
        </View>
      ) : assurance ? (
        <View style={styles.card} testID="trust-assurance-mobile">
          <Text style={styles.title}>{t({ en: 'Assurance (not reputation)', zh: '保障画像（非行为信誉）' })}</Text>
          {assurance.profile.enforcementLayers.map((layer, index) => (
            <Text key={`${layer.layer}-${index}`} style={styles.row}>
