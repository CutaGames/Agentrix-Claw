...
function normalizeAuthUserInstances(user: AuthUser): AuthUser {
  if (!user.openClawInstances) return user;
  return {
    ...user,
    openClawInstances: user.openClawInstances.map((instance, index) => ({
      ...instance,
      name: resolveAgentDisplayName(instance, `Agent ${index + 1}`),
    })),
  };
}
  ...
  persist(
    (set, get) => ({
      ...
      setAuth: async (user, token) => {
        ...
        // Normalize API and persisted legacy rows before they become UI state.
        const normalizedUser = normalizeAuthUserInstances(user);
        const previousUserId = get().user?.id;
        const currentActive = get().activeInstance;
        const previousActiveId = currentActive?.id ?? normalizedUser.activeInstanceId;
        const refreshedActive = previousActiveId
          ? normalizedUser.openClawInstances?.find((instance) => instance.id === previousActiveId) ?? null
          : null;
        const firstInstance = normalizedUser.openClawInstances?.[0] ?? null;
        const activeInstance = refreshedActive ?? firstInstance ?? (
          currentActive
            ? { ...currentActive, name: resolveAgentDisplayName(currentActive) }
            : null
        );
        set({
          user: normalizedUser,
          ...
          isGuest: false,
          activeInstance,
          hasValidInvitation: previousUserId && previousUserId !== normalizedUser.id ? false : get().hasValidInvitation,
        });
      },
      ...
      addInstance: (instance) => {
        ...
        nextInstance.name = resolveAgentDisplayName(nextInstance, `Agent ${existing.length + 1}`);
      ...
      updateUser: (updates) => {
        const user = get().user;
        if (!user) return;
        set({ user: normalizeAuthUserInstances({ ...user, ...updates }) });
      },
    }),
    {
