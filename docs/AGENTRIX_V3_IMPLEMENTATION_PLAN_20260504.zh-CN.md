...
| P0-W2-7 | Web Console `/console/**` 路由分区 + 登录/Agent 总览/Presence v3/Wallet read-only/Stripe | dev | � 修订 2026-05-05 | 🧪 路由 + ConsoleLayout + middleware 已落；2026-05-05 重做：Dashboard/Wallet/Presence 实接 `/api/v1/{pet,wallet/projection,handoff,approval}`；新增 Marketplace/Wallet 子页/Developer/Merchant/Family/Settings 多级 IA（[WEB_REMEDIATION_PLAN](WEB_REMEDIATION_PLAN_20260505.zh-CN.md) R1）；Stripe Console 联调留 W23 R3-8 | 2026-05-04 / 2026-05-05 修订 | [frontend/pages/console/](../frontend/pages/console/) + [components/console/ConsoleLayout.tsx](../frontend/components/console/ConsoleLayout.tsx) + [lib/api/v1.api.ts](../frontend/lib/api/v1.api.ts) |
...
| P1-8 | Web Console 团队/企业后台 + SplitPlan UI + BudgetPool UI + 钱包台账 + 合规审计 | dev | � 后端完成 / 前端 W21 修订 | 🧪 后端：`POST /api/v1/split-plans` 70/20/10 + `/preview $1000` 精确分账；BudgetPool 月限额 + 超限 400；`/audit` 4 条；smoke 2026-05-04 通过。前端：2026-05-05 落 [/console/wallet/split-plans](../frontend/pages/console/wallet/split-plans.tsx) + [/budgets](../frontend/pages/console/wallet/budgets.tsx) + [/auto-earn](../frontend/pages/console/wallet/auto-earn.tsx) 实接，audit/checkout/commission 走 redirect 收敛旧页面；编辑器/Stripe 联调留 W23 R3 | 2026-05-04 / 2026-05-05 |
...
| P2-6 | Web 开发者后台 + Shortcut 模板编辑器 + 市场后台 | dev | � 后端完成 / 前端 50% | 🧪 后端：`POST /api/v1/skill-listings` draft → submit → review approve → invoke 3× 200cents；80/20 精确（dev=480/platform=120/total=600）；`/me/earnings` 聚合；duplicate slug → 400；smoke 2026-05-04 通过。前端：2026-05-05 落 [/console/marketplace/skills](../frontend/pages/console/marketplace/skills.tsx)（SSR 拉真 listings） + [/marketplace/tasks](../frontend/pages/console/marketplace/tasks.tsx)（接旧 TaskMarketplace 组件） + [/developer](../frontend/pages/console/developer/index.tsx) 入口；earnings 仪表盘 / publish 编辑器 留 W22-W23 | 2026-05-04 / 2026-05-05 |
...
| P3-5 | Web 家庭账号后台（Family Account / Pet / Household Agent）+ 5 端管理 + i18n + A11y | dev | � 后端完成 / 前端骨架 | 🧪 后端：`POST /api/v1/family` 创建 Smith Family；invite member/child code；setup family pet Buddy + emotion happy；create household agents Alfred (butler) + Mr.Owl (tutor)；visible_to_roles 过滤生效；smoke 2026-05-04 通过。前端：2026-05-05 落 [/console/family](../frontend/pages/console/family/index.tsx) 列表+创建 + members/pet/agents/allowance 子页；i18n / A11y / 真实 RBAC 留 W24 R4 | 2026-05-04 / 2026-05-05 |
